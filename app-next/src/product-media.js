...
const BUILD='20260828-product-media-2mockups-shorts-v2-final';
...
function bindGallery(images,{mug=false}={}){
  ...
  if(mug){media.dataset.mugMediaReady='1';document.documentElement.classList.add('mug-media-ready')}
  ...
  thumbs.innerHTML=images.map((image,index)=>`<button type="button" class="product-media-thumb${index===0?' active':''}" data-product-media-src="${escapeHtml(image)}" aria-label="${mug?`Ver mockup ${index+1}`:`Ver foto ${index+1}`}" aria-pressed="${index===0?'true':'false'}"><img loading="lazy" decoding="async" src="${escapeHtml(image)}" alt="${mug?`Mockup ${index+1} da caneca`:`Foto ${index+1} do produto`}"><span>${mug?`Vista ${index+1}`:`Foto ${index+1}`}</span></button>`).join('');
  media.appendChild(thumbs);
  thumbs.addEventListener('click',event=>{const button=event.target.closest('[data-product-media-src]');if(!button)return;main.src=button.dataset.productMediaSrc;thumbs.querySelectorAll('.product-media-thumb').forEach(item=>{const active=item===button;item.classList.toggle('active',active);item.setAttribute('aria-pressed',active?'true':'false')})});
}
...
html.mug-product-route .product-detail-media>img{display:block!important}html.mug-product-route .product-detail-media>.image-thumbs{display:grid!important}html.mug-product-route .product-detail-media:not(:has(.da-mug-showcase)){min-height:0!important;display:block!important;background:transparent!important;border-radius:0!important}html.mug-product-route .product-detail-media:not(:has(.da-mug-showcase))::before{content:none!important;display:none!important}
.product-media-thumbs{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:10px}.product-media-thumb{min-height:56px;border:1px solid #e3e4df;background:#fff;border-radius:13px;padding:5px;cursor:pointer;overflow:hidden;position:relative;touch-action:manipulation}.product-media-thumb.active{border-color:#20231f;box-shadow:0 0 0 1px #20231f}.product-media-thumb img{display:block;width:100%;aspect-ratio:1;object-fit:contain;border-radius:9px;background:#f8f8f6}.product-media-thumb span{display:block;padding:5px 3px 2px;font-size:10px;font-weight:800;color:#62665f}
...
.product-art-open{display:block;width:100%;min-height:44px;border:0;background:#f4f5f1;border-radius:14px;padding:8px;cursor:zoom-in;touch-action:manipulation}.product-art-open img{display:block;width:100%;aspect-ratio:2.5/1;object-fit:contain;border-radius:10px}.product-art-open>span{display:block;margin-top:6px;font-size:10px;font-weight:750;color:#6c716a}
.product-real-video[data-video-ratio="short"]{display:grid;justify-items:center}.product-real-video[data-video-ratio="short"] .product-extra-heading{width:min(100%,390px)}.product-video-poster,.product-youtube-iframe{display:block;border:0;border-radius:16px;background:#111;overflow:hidden}.product-video-poster{position:relative;padding:0;cursor:pointer;width:100%;min-height:200px;touch-action:manipulation}.product-real-video[data-video-ratio="short"] .product-video-poster,.product-real-video[data-video-ratio="short"] .product-youtube-iframe{width:min(100%,390px);aspect-ratio:9/16}.product-real-video[data-video-ratio="wide"] .product-video-poster,.product-real-video[data-video-ratio="wide"] .product-youtube-iframe{width:100%;aspect-ratio:16/9}.product-video-poster img{display:block;width:100%;height:100%;object-fit:cover;opacity:.9}.product-video-play{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:64px;height:64px;border-radius:50%;background:rgba(255,255,255,.94);color:#111;display:grid;place-items:center;font-size:25px;padding-left:4px;box-shadow:0 8px 26px rgba(0,0,0,.22)}.product-video-cta{position:absolute;left:50%;bottom:18px;transform:translateX(-50%);padding:7px 12px;border-radius:999px;background:rgba(0,0,0,.68);color:#fff;font-size:11px;font-weight:850}.product-youtube-iframe{border:0}
@media(max-width:700px){.product-extra-media{gap:14px;margin:14px 0 22px}.product-print-art,.product-real-video{padding:11px;border-radius:15px}.product-extra-heading{display:block}.product-extra-heading strong,.product-extra-heading span{display:block}.product-extra-heading span{margin-top:2px;text-align:left}.product-media-thumbs{gap:6px}.product-media-thumb{border-radius:10px;min-height:64px}.product-video-play{width:58px;height:58px}.product-real-video[data-video-ratio="short"] .product-video-poster,.product-real-video[data-video-ratio="short"] .product-youtube-iframe{width:min(88vw,360px)}}`;document.head.appendChild(style)}
