const BUILD = '20260826-site-mug-make-client-guard-v3-accepted-poll';
...
async function waitFinalProduct(payload, fetcher){
  ...
  while (Date.now() < deadline) {
    ...
    } catch (error) { console.warn('[Canecas públicas] aguardando Firebase após Accepted:', error); }
    await sleep(POLL_MS);
  }
  return null;
}
...
function install(){
  ...
  window.fetch = async function publicMugMakeFetch(input, init){
    ...
    if (response.ok && /^accepted\.?$/i.test(text(raw)) && payload?.action === 'finalize_mug_product') {
      console.info('[Canecas públicas] Make respondeu Accepted; aguardando produto final no Firebase.', payload.request_id);
      const recovered = await waitFinalProduct(payload, previousFetch);
      if (recovered) return jsonResponse(recovered,200);
      return jsonResponse({ok:false,error:'Sua caneca continua sendo finalizada. Aguarde alguns instantes e tente abrir novamente esta criação.',accepted:true,request_id:payload.request_id,client_contract:BUILD},504);
    }
