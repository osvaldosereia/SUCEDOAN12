import { CONFIG } from './config.js?v=20260826-canecas-clean-v16';

const BUILD = '20260826-site-canecas-clean-v16';
...
const PLACEHOLDER = Object.freeze({ art:'__MUG_ART__', m1:'__MUG_MOCKUP_1__', m2:'__MUG_MOCKUP_2__', m3:'__MUG_MOCKUP_3__' });
const DAILY_LIMIT = 3;
...
function normalizeFields(config={}) {
  return (Array.isArray(config.campos) ? config.campos : [])
    ...
    .map((field,index) => ({
      id:text(field.id || `campo_${index+1}`), tipo:text(field.tipo || 'texto'), label:text(field.label || `Campo ${index+1}`),
      obrigatorio:field.obrigatorio === true, placeholder:text(field.placeholder), valor_padrao:text(field.valor_padrao), ajuda:text(field.ajuda),
      opcoes:Array.isArray(field.opcoes) ? field.opcoes.map(text).filter(Boolean) : [], ordem:Number(field.ordem ?? index)
    }))
...
async function firebaseWrite(path, payload, method='PUT') {
  const response = await fetch(`${FIREBASE}/${path}.json`, { method, headers:{'Content-Type':'application/json',Accept:'application/json'}, body:JSON.stringify(payload) });
  if (!response.ok) throw new Error(`Firebase ${response.status}`);
...
function limitData() {
  const data=readLocal(LIMIT_KEY,{});
  return data && typeof data === 'object' ? data : {};
}

function creationsToday(whatsapp) {
  const phone=canonicalWhatsapp(whatsapp);
  const entries=phone ? limitData()?.[dayKey()]?.[phone] : null;
...
function registerCreation(whatsapp,id) {
  ...
  Object.keys(data).forEach(key => { if (key !== today) delete data[key]; });
  writeLocal(LIMIT_KEY,data);
}

function installStyles() {
  if (document.getElementById('mugPublicCleanStylesV16')) return;
  const style=document.createElement('style');
  style.id='mugPublicCleanStylesV16';
  style.textContent=`
...
function updateLimitUi() {
  ...
  const phone=canonicalWhatsapp(input?.value);
  if (!phone) {
  ...
  const remaining=remainingToday(phone);
...
function renderPanel(product,config) {
  ...
  section.querySelector('#mugPublicCustomerName')?.addEventListener('input',saveCustomerDraft);
...
function saveCustomerDraft() {
  writeLocal(CUSTOMER_KEY,{ name:text(document.querySelector('#mugPublicCustomerName')?.value), whatsapp:text(document.querySelector('#mugPublicCustomerWhatsapp')?.value) });
}
...
function collect({validate=true}={}) {
  ...
  if (validate && remainingToday(whatsapp)<=0) throw new Error('Este número já atingiu o limite de 3 criações de hoje neste aparelho.');
  const values=[];
  const files=[];
...
function fileData(file) {
  return new Promise((resolve,reject)=>{ const reader=new FileReader(); reader.onload=()=>resolve(String(reader.result||'')); reader.onerror=()=>reject(new Error('Não foi possível ler uma das fotos.')); reader.readAsDataURL(file); });
}

function loadImage(src) {
  return new Promise((resolve,reject)=>{ const image=new Image(); if (/^https?:/i.test(src)) image.crossOrigin='anonymous'; image.onload=()=>resolve(image); image.onerror=()=>reject(new Error('Não foi possível abrir uma imagem gerada.')); image.src=src; });
}
...
async function cropMaster(src) {
  const image=await loadImage(src); const target=2.5; const ratio=image.naturalWidth/image.naturalHeight;
...
async function cropReference(master,mode) {
  const image=await loadImage(master); const width=1344;
...
async function waitFinalProduct(payload,onTick=()=>{}) {
  const id=text(payload?.request_id);
  if (!id) throw new Error('A automação aceitou a geração sem informar o código da criação.');
  ...
  while (Date.now()<deadline) {
    ...
    onTick(attempt);
    try {
      const product=await firebaseGet(`${NODES.products}/${encodeURIComponent(id)}`);
      ...
      console.debug('[Canecas públicas] aguardando finalização:',error?.message||error);
    }
    await sleep(POLL_MS);
  }
  throw new Error('A automação aceitou a criação, mas não publicou as 4 imagens em até 3 minutos. A tentativa foi encerrada para não ficar travada.');
}
...
async function callMake(payload,timeout=180000) {
  ...
  try {
    const response=await fetch(MAKE,{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({payload:JSON.stringify({...payload,client_contract:BUILD})}),signal:controller.signal});
    ...
    if (response.ok && /^accepted\.?$/i.test(text(raw)) && payload.action==='finalize_mug_product') {
      return waitFinalProduct(payload,attempt=>progress(Math.min(82,58+attempt),'Finalizando as três prévias…'));
    }
    const snippet=text(raw).replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').slice(0,160);
    throw new Error(snippet?`Automação respondeu conteúdo inválido (${response.status}): ${snippet}`:`Automação não devolveu JSON (${response.status}).`);
  } catch (error) {
    if (error?.name==='AbortError') throw new Error('A automação demorou mais de 3 minutos e foi encerrada para não deixar a tela travada.');
    throw error;
  } finally { clearTimeout(timer); }
}

function primaryText(result,ids=[]) {
  for (const id of ids) { const item=result.values.find(value=>value.id===id&&value.value); if (item) return item.value; }
  return '';
}

function requestId() { return `cp-${Date.now()}-${Math.random().toString(36).slice(2,10)}`; }
function resultUrl(id) { const url=new URL('/ceneca10/resultado.html',location.origin); url.searchParams.set('id',id); return url.href; }
...
function productTemplate(id,result,publicUrl) {
  ...
  const phraseValue=primaryText(result,['frase','mensagem','texto']);
  const price=salePrice();
  const name=`Caneca de Porcelana Personalizada${displayName?` ${displayName}`:''} - 350ml`;
  return {
    id,firebaseKey:id,codigo:`CANP-${id.slice(-8).toUpperCase()}`,nome:name,categoria:'Caneca de Porcelana',subcategoria:'Personalizadas',ncm:'69111090',
    preco_custo:10,preco:price,estoque:0,situacao:'I',status:'I',ativo:false,visivel:false,modelo_caneca:true,modelo_publico:false,personalizacao_publica:false,
    material:'Porcelana',capacidade:'350ml',unidade:'UN',descricao:`${name}. Criação personalizada identificada pelo WhatsApp ${displayWhatsapp(result.whatsapp)}.`,tags:['caneca personalizada','caneca com foto','presente personalizado'],
    url_imagem:PLACEHOLDER.m1,imagem:PLACEHOLDER.m1,imagem_url:PLACEHOLDER.m1,imagens:[PLACEHOLDER.m1,PLACEHOLDER.m2,PLACEHOLDER.m3],imagens_site:[PLACEHOLDER.m1,PLACEHOLDER.m2,PLACEHOLDER.m3],mockup_1:PLACEHOLDER.m1,mockup_2:PLACEHOLDER.m2,mockup_3:PLACEHOLDER.m3,arte_personalizacao:PLACEHOLDER.art,arte_horizontal:PLACEHOLDER.art,arte_impressao:{url:PLACEHOLDER.art,width:2400,height:960,dimensao_real:'24 × 9,5 cm',formato:'webp'},
    origem_cadastro:'site_publico_canecas_clean_v16',tipo_produto:'caneca_personalizada',geracao_status:'concluido',geracao_versao:BUILD,cliente_nome:result.customerName,cliente_whatsapp:result.whatsapp,
    personalizacao_cliente:{nome:result.customerName,whatsapp:result.whatsapp,whatsapp_local:result.whatsappLocal,nome_destaque:displayName,frase:phraseValue,modelo_key:STATE.key,modelo_nome:STATE.product?.nome||'',campos:Object.fromEntries(result.values.map(item=>[item.id,item.value])),resultado_url:publicUrl,codigo_criacao:id},
    configuracao_arte:{modo:'template_dinamico_site_v16',modelo_key:STATE.key,width:2400,height:960},criado_em:now,updated_at:now,last_update:Date.now()
  };
}
...
function materializeProduct(template,urls) {
  ...
  product.mockup_2=urls.m2; product.mockup_3=urls.m3; product.imagens=[urls.m1,urls.m2,urls.m3]; product.imagens_site=[urls.m1,urls.m2,urls.m3];
  product.arte_personalizacao=product.arte_horizontal=urls.art; product.arte_impressao.url=urls.art;
  return product;
}
...
async function saveCreation(id,result,urls,publicUrl,makeSaved) {
  const product=materializeProduct(productTemplate(id,result,publicUrl),urls);
  if (!makeSaved) await firebaseWrite(`${NODES.products}/${encodeURIComponent(id)}`,product);
  else await firebaseWrite(`${NODES.products}/${encodeURIComponent(id)}`,{
    ...
    mockup_1:urls.m1,mockup_2:urls.m2,mockup_3:urls.m3,url_imagem:urls.m1,imagem:urls.m1,imagem_url:urls.m1,imagens:[urls.m1,urls.m2,urls.m3],imagens_site:[urls.m1,urls.m2,urls.m3],arte_horizontal:urls.art,arte_personalizacao:urls.art,
    origem_cadastro:'site_publico_canecas_clean_v16',geracao_versao:BUILD,updated_at:new Date().toISOString(),last_update:Date.now()
  },'PATCH');
  const displayName=primaryText(result,['nome','nome_1','nome_na_caneca','nome_destaque']);
  const phraseValue=primaryText(result,['frase','mensagem','texto']);
  const now=new Date().toISOString();
  const publicRecord={id,nome_publico:displayName?`Caneca personalizada ${displayName}`:'Caneca personalizada',nome_destaque:displayName,frase:phraseValue,modelo_nome:STATE.product?.nome||'',modelo_key:STATE.key,arte_horizontal:urls.art,mockup_1:urls.m1,mockup_2:urls.m2,mockup_3:urls.m3,produto_key:id,criado_em:now,expira_em:new Date(Date.now()+7*86400000).toISOString()};
  ...
  const model={id,product_key:id,nome:product.nome,imagem:urls.m1,mockup_1:urls.m1,mockup_2:urls.m2,mockup_3:urls.m3,arte_horizontal:urls.art,frase:phraseValue,nome_destaque:displayName,modelo_publico:false,personalizacao_publica:false,origem:BUILD,atualizado_em:now};
  await Promise.all([firebaseWrite(`${NODES.public}/${encodeURIComponent(id)}`,publicRecord),firebaseWrite(`${NODES.private}/${encodeURIComponent(id)}`,privateRecord),firebaseWrite(`${NODES.models}/${encodeURIComponent(id)}`,model)]);
  return product;
}

function normalizedCartProduct(product,urls) {
  return {id:String(product.id),firebaseKey:String(product.id),codigo:String(product.codigo||product.id),name:String(product.nome||'Caneca personalizada'),slug:String(product.id),price:Number(product.preco||salePrice()),oldPrice:Number(product.preco||salePrice()),stock:1,situacao:'A',categoria:String(product.categoria||'Caneca de Porcelana'),subcategoria:String(product.subcategoria||'Personalizadas'),subsubcategoria:'',marca:'Dona Antônia',embalagem:'350ml',descricao:String(product.descricao||''),gtin:'',ean:'',gondola:'',prateleira:'',localizacao:'',preco_oferta:0,validade_oferta:'',validade:'',images:[urls.m1,urls.m2,urls.m3],img:urls.m1,url_imagem:urls.m1,raw:{...product,situacao:'I',ativo:false,checkout_personalizado:true}};
}
...
function addToCart(product,urls,result) {
  ...
  window.dispatchEvent(new CustomEvent('da:mug-personalized-added',{detail:{id:localProduct.id}}));
  return Boolean(state?.productMap instanceof Map);
}

function renderResult(id,urls,result,cartAdded) {
  const box=document.querySelector('#mugPublicResult'); if (!box) return;
  const publicUrl=resultUrl(id); const phone=displayWhatsapp(result.whatsapp);
  box.hidden=false;
  box.innerHTML=`<h3>Sua caneca ficou pronta ✨</h3><p>Confira a arte horizontal e as três prévias.</p><div class="mug-result-art"><span>Arte horizontal para impressão</span><img src="${escapeHtml(urls.art)}" alt="Arte horizontal"></div><div class="mug-result-mockups"><span>3 prévias da sua caneca</span><div class="mug-result-grid"><img src="${escapeHtml(urls.m1)}" alt="Mockup 1"><img src="${escapeHtml(urls.m2)}" alt="Mockup 2"><img src="${escapeHtml(urls.m3)}" alt="Mockup 3"></div></div><div class="mug-public-saved">Criação salva para <strong>${escapeHtml(phone)}</strong>.${cartAdded?' Ela já foi adicionada à sua compra.':''}</div><div class="mug-public-result-actions"><button type="button" id="mugPublicOpenCart">Ver minha compra / checkout</button><a href="${escapeHtml(publicUrl)}">Abrir página com as 4 imagens</a><button type="button" class="secondary" id="mugPublicAnother">Criar outra</button></div>`;
  box.querySelector('#mugPublicAnother')?.addEventListener('click',resetAfterResult);
  box.querySelector('#mugPublicOpenCart')?.addEventListener('click',()=>document.getElementById('open-cart')?.click());
...
async function generate() {
  ...
  saveCustomerDraft();
  STATE.id ||= requestId();
  const id=STATE.id; const publicUrl=resultUrl(id);
  STATE.busy=true; updateLimitUi();
  ...
  try {
    ...
    const generatedSource=text(generated.art_source_url||generated.art_url||generated.result_url||generated.art_source_base64);
    if (!isImageSource(generatedSource)) throw new Error('A automação não devolveu a nova arte.');
    progress(40,'Preparando a arte final…');
    const master=await cropMaster(generatedSource);
    ...
    const template=productTemplate(id,result,publicUrl);
    const final=await callMake({action:'finalize_mug_product',request_id:id,image_base64:master,mockup_left_base64:left,mockup_right_base64:right,mockup_center_base64:center,product_name:template.nome,prompt_mockup_1:mockPrompt(1),prompt_mockup_2:mockPrompt(2),prompt_mockup_3:mockPrompt(3),quality:'high',firebase_url:FIREBASE,products_node:NODES.products,firebase_template_json:JSON.stringify(template),origin:'site_publico'});
    ...
    console.error('[Canecas públicas clean V16]',error);
    setError(error?.message||String(error)||'Não foi possível concluir a caneca.');
    const progressBox=document.querySelector('#mugPublicProgress'); if (progressBox) progressBox.hidden=true;
  } finally { STATE.busy=false; updateLimitUi(); }
}
...
async function syncRoute() {
  ...
  if (!key) {
    document.querySelector('#mug-public-personalizer')?.remove();
    STATE.key=''; STATE.product=null; STATE.config=null; STATE.id='';
    return;
  }
  ...
  try {
    const product=await firebaseGet(`${NODES.products}/${encodeURIComponent(key)}`);
    if (epoch!==STATE.routeEpoch || routeKey()!==key) return;
    ...
    if (!enabled) { document.querySelector('#mug-public-personalizer')?.remove(); STATE.key=key; STATE.product=null; STATE.config=null; return; }
    STATE.key=key; STATE.product=product; STATE.config=config; STATE.id='';
    let attempts=0;
    const mount=()=>{
      if (epoch!==STATE.routeEpoch || routeKey()!==key) return;
      if (renderPanel(product,config)) return;
      if (++attempts<50) setTimeout(mount,80);
    };
    mount();
  } catch (error) { if (epoch===STATE.routeEpoch) console.error('[Canecas públicas clean V16] Falha ao carregar modelo:',error); }
}

let syncTimer=0;
function scheduleSync(delay=0) { clearTimeout(syncTimer); syncTimer=setTimeout(syncRoute,delay); }
window.addEventListener('hashchange',()=>scheduleSync(0));
window.addEventListener('da:route-rendered',()=>scheduleSync(0));
window.addEventListener('da:catalog-ready',()=>scheduleSync(20));
window.addEventListener('da:catalog-refreshed',()=>scheduleSync(20));
new MutationObserver(()=>{ if (routeKey() && !document.querySelector('#mug-public-personalizer') && document.querySelector('.product-detail')) scheduleSync(30); }).observe(document.documentElement,{childList:true,subtree:true});
