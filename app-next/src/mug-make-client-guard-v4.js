const BUILD = '20260826-site-mug-make-client-guard-v4-accepted-poll';
...
async function waitFinalProduct(payload, fetcher){
  ...
  while (Date.now() < deadline) {
    ...
    } catch (error) { console.warn('[Canecas públicas V4] aguardando Firebase:', error); }
    await sleep(POLL_MS);
  }
  return null;
}
...
function install(){
  ...
  window.fetch = async function publicMugMakeFetch(input, init){
    ...
    if (response.ok && /^accepted\.?$/i.test(text(raw)) && payload?.action === 'finalize_mug_product') {
      console.info('[Canecas públicas V4] Make respondeu Accepted; aguardando produto final no Firebase.', payload.request_id);
