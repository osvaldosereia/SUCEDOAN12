(() => {
  ...
  const BUILD = '20260903-canecafacil-ui-runtime-v1-stable';
  ...
  function installStyle() {
    if (document.getElementById('cfUiRuntimeStyle')) return;
    const style = document.createElement('style');
    style.id = 'cfUiRuntimeStyle';
    style.textContent = `
...
@media(max-width:767px){
  ...
  .menu.superior>ul.nivel-um>li>a>i.icon-chevron-down{
    position:static!important;
  ...
  .menu.superior>ul.nivel-um>li>a>i.icon-chevron-down:before{
    content:'▾'!important;
    display:block!important;
    color:#777!important;
...
`;
  ...
  function shareButton() {
    ...
    if (label && label.textContent !== 'Compartilhar no WhatsApp') label.textContent = 'Compartilhar no WhatsApp';
    if (button.getAttribute('aria-label') !== 'Compartilhar no WhatsApp') button.setAttribute('aria-label', 'Compartilhar no WhatsApp');
  ...
  function positionShareButton() {
    ...
    if (MOBILE.matches) {
      ...
      const thumbs = document.querySelector('.produto-thumbs');
      const anchor = nativeShare || thumbs;
