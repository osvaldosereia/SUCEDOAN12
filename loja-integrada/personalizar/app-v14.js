const BUILD = '20260901-loja-integrada-personalizador-v6-async-preview-email';
...
function creationCode() {
  ...
  const random = (globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`).replace(/[^a-z0-9]/gi,'').toUpperCase().slice(-12);
