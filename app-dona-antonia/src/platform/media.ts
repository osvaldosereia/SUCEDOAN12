...
export function validateHomologationMedia(value: MediaFixtureInput): MediaAttachment {
  if (!TEST_MEDIA_ID.test(value.id)) {
    throw new Error('homologation media id must use TEST-MEDIA-*');
  }
  ...
  if (value.kind === 'photo') {
    if (value.source !== 'photo_picker' && value.source !== 'camera') {
      throw new Error('photo source must be photo_picker or camera');
    }
    if (!PHOTO_MIME_TYPES.has(value.mimeType)) {
      throw new Error('unsupported photo mime type');
    }
    if (value.sizeBytes > MAX_PHOTO_BYTES) {
      throw new Error('photo exceeds homologation size limit');
    }
    if (value.durationSeconds !== null) {
      throw new Error('photo cannot have duration');
    }
  } else {
    if (value.source !== 'microphone') {
      throw new Error('audio source must be microphone');
    }
    if (!AUDIO_MIME_TYPES.has(value.mimeType)) {
      throw new Error('unsupported audio mime type');
    }
    if (value.sizeBytes > MAX_AUDIO_BYTES) {
      throw new Error('audio exceeds homologation size limit');
    }
    if (
      value.durationSeconds === null
      || !Number.isFinite(value.durationSeconds)
      || value.durationSeconds <= 0
      || value.durationSeconds > MAX_AUDIO_SECONDS
    ) {
      throw new Error('audio duration is outside homologation limit');
    }
  }
...
export function createHomologationMediaGateway(options: {
  ...
  async function next(queue: MediaFixtureInput[]): Promise<MediaAttachment | null> {
    ...
    return value ? validateHomologationMedia(value) : null;
  }

  return {
    pickPhoto() {
      return next(pickedPhotos);
    },

    takePhoto() {
      return next(cameraPhotos);
    },

    async recordAudio() {
      recording = true;
      const result = await next(audioRecordings);
      recording = false;
      return result;
    },
