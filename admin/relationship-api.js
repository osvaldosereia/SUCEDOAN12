...
async function whatsappDirectAdminCall(body,timeoutMs=30000){
  const token=getCustomerOsAccessToken();
  if(!token){
    const error=new Error('Entre com o PIN para acessar o diagnóstico Meta.');
    error.code='missing_token';
    throw error;
  }
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    const response=await fetch(`${CONFIG.supabaseUrl}/functions/v1/admin-whatsapp-direct-v1`,{
      method:'POST',
      headers:{
        apikey:CONFIG.supabasePublishableKey,
        Authorization:`Bearer ${token}`,
    ...
    if(response.status===401)clearCustomerOsSession();
    if(!response.ok||data?.ok===false){
      const error=new Error(data?.detail||data?.error||'Não foi possível executar o diagnóstico Meta.');
      error.code=String(data?.error||'request_failed');
      error.status=response.status;
      error.data=data;
      throw error;
    ...
    if(error?.name==='AbortError')throw new Error('A verificação Meta demorou demais. Tente novamente.');
    throw error;
  }finally{clearTimeout(timer)}
}
...
export const runMetaDiagnosticsReadonly=()=>whatsappDirectAdminCall({action:'meta_diagnostics_readonly'},40000);
