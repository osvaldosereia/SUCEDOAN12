 ...
 * R1 foundation only: describes navigation/ownership without changing runtime gates.
 * New shell work must consume this registry instead of duplicating menus per page.
 */
...
const module=(value)=>Object.freeze({
  kind:'page',
  status:'production',
...
export const ADMIN_MODULES=Object.freeze([
  ...
  module({id:'customers',label:'Clientes',group:'operation',route:{type:'hash',value:'customers'},order:30,gate:'customerOs'}),
...
export function modulesByGroup(){
  return ADMIN_GROUPS.map(group=>({
    ...group,
    modules:ADMIN_MODULES.filter(item=>item.group===group.id).sort((a,b)=>a.order-b.order)
  }));
}

export function getAdminModule(id){
  return ADMIN_MODULES.find(item=>item.id===id)||null;
}

export function validateAdminRegistry(){
  const errors=[];
  const ids=new Set();
  const groupIds=new Set(ADMIN_GROUPS.map(x=>x.id));
  for(const item of ADMIN_MODULES){
    if(ids.has(item.id))errors.push(`duplicate module id: ${item.id}`);
    ids.add(item.id);
    if(!groupIds.has(item.group))errors.push(`unknown group ${item.group}: ${item.id}`);
    if(!item.route?.type||!item.route?.value)errors.push(`invalid route: ${item.id}`);
  }
