...
const state = {
  products: [], productMap: new Map(), cart: {}, cartOrder: [], cartPromotions: {}, campaigns: [],
  campaign: null, campaignStatus: 'none', campaignRef: '', route: { name: 'home', values: [], query: new URLSearchParams() }
};
...
function normalizeCampaign(raw = {}) {
  return {
    id: String(raw.id || '').trim(), token: String(raw.token || '').trim(), name: String(raw.name || raw.nome || 'Campanha').trim(),
    code: String(raw.code || raw.codigo || raw.id || '').trim().toUpperCase(), active: raw.active !== false && raw.ativo !== false,
    discountPercent: Math.max(0, Math.min(30, Number(raw.discountPercent ?? raw.desconto ?? 0) || 0)),
    scope: raw.scope === 'all' ? 'all' : 'destination',
    destination: { type: String(raw.destination?.type || raw.tipo || 'home').trim().toLowerCase(), value: String(raw.destination?.value || raw.destino || '').trim() },
    startsAt: String(raw.startsAt || raw.inicio || '').trim(), expiresAt: String(raw.expiresAt || raw.fim || '').trim(), note: String(raw.note || raw.observacao || '').trim()
  };
}
async function loadCampaigns() {
  ...
  } catch (error) { console.warn('Não foi possível carregar as campanhas do mini catálogo:', error); state.campaigns = []; }
}
function parseRoute() {
...
function campaignIsActive(campaign, now = new Date()) {
  ...
  const start = safeDate(campaign.startsAt, false); const end = safeDate(campaign.expiresAt, true);
...
function resolveCampaign(route) {
  ...
  state.campaignRef = reference; state.campaign = null; state.campaignStatus = reference ? 'invalid' : 'none';
  if (!reference) return;
  const separator = reference.indexOf('.');
  if (separator <= 0) return;
  const id = reference.slice(0, separator); const token = reference.slice(separator + 1);
  const campaign = state.campaigns.find(item => item.id === id && item.token === token);
  if (!campaign) return;
  const validity = campaignIsActive(campaign); state.campaignStatus = validity.status;
  if (validity.ok) state.campaign = campaign;
}
function campaignQuery() { return state.campaignRef ? `c=${encodeURIComponent(state.campaignRef)}` : ''; }
function routeHref(base) {
  const clean = String(base || '#/'); const query = campaignQuery();
  return query ? `${clean}${clean.includes('?') ? '&' : '?'}${query}` : clean;
}
function navigate(base) { location.hash = routeHref(base); }
function readCart() {
  try {
...
function persistCart() { try { localStorage.setItem(CART_KEY, JSON.stringify({ cart: state.cart, cartOrder: state.cartOrder, cartPromotions: state.cartPromotions, savedAt: Date.now() })); } catch {} }
function cleanCart() {
  const nextOrder = [], nextCart = {}, nextPromotions = {};
  state.cartOrder.forEach(id => {
    const product = state.productMap.get(String(id)); const qty = Math.min(Number(product?.stock || 0), Math.max(0, Number(state.cart[id] || 0)));
    if (product && isAvailable(product) && qty > 0) {
      nextOrder.push(String(id)); nextCart[String(id)] = qty;
      if (itemCampaign(id, product)) nextPromotions[String(id)] = state.cartPromotions[String(id)];
    }
  });
  state.cartOrder = nextOrder; state.cart = nextCart; state.cartPromotions = nextPromotions; persistCart();
}
function productMatchesSearch(product, query) {
  const wanted = norm(query); if (!wanted) return false;
...
function campaignMatchesProductFor(campaign, product) {
  ...
  const type = campaign.destination.type, value = campaign.destination.value;
...
function activeCampaignForProduct(product) { return state.campaign && campaignMatchesProductFor(state.campaign, product) ? state.campaign : null; }
...
function productPricing(product, campaign = activeCampaignForProduct(product)) {
  const current = Number(product.price || 0), discount = Number(campaign?.discountPercent || 0);
  const final = discount ? roundMoney(current * (1 - discount / 100)) : current;
  return { current, final, discount, code: campaign?.code || campaign?.name || '', campaignId: campaign?.id || '' };
}
function cartPricing() {
  const items = state.cartOrder.map(id => {
    const product = state.productMap.get(String(id)), qty = Number(state.cart[id] || 0);
    if (!product || qty <= 0) return null;
    const pricing = productPricing(product, itemCampaign(id, product));
    return { id: String(id), product, qty, unit: pricing.final, baseUnit: pricing.current, discountPercent: pricing.discount, discountCode: pricing.code, campaignId: pricing.campaignId, total: roundMoney(pricing.final * qty) };
  }).filter(Boolean);
...
function setQty(id, qty) {
  const key = String(id), product = state.productMap.get(key); if (!product) return;
  const next = Math.max(0, Math.min(Number(product.stock || 0), Number(qty || 0)));
  if (next <= 0) { delete state.cart[key]; delete state.cartPromotions[key]; state.cartOrder = state.cartOrder.filter(item => item !== key); }
  else { state.cart[key] = next; if (!state.cartOrder.includes(key)) state.cartOrder.push(key); savePromotionForItem(key, product); }
  persistCart(); updateShell(); renderCurrentControls(key); if (checkoutDrawer.classList.contains('open')) renderCheckout();
}
function addProduct(id) { setQty(id, Number(state.cart[String(id)] || 0) + 1); showToast('Produto adicionado ao complemento.'); }
function quantityControl(product, detail = false) {
  const id = String(product.id), qty = Number(state.cart[id] || 0);
...
function productRoute(product) { return encodeURIComponent(product.firebaseKey || product.id || product.codigo || product.slug); }
function productCard(product) {
  const pricing = productPricing(product), id = String(product.id);
  const original = pricing.discount ? pricing.current : Number(product.oldPrice || product.price || 0), showOld = original > pricing.final;
  return `<article class="product-card" data-product-card="${escapeHtml(id)}"><div class="product-card-media"><a href="${routeHref(`#/produto/${productRoute(product)}`)}" aria-label="Ver ${escapeHtml(product.name)}"><img loading="lazy" decoding="async" width="300" height="300" src="${escapeHtml(product.img)}" data-fallback="${escapeHtml(product.images?.slice(1).join('|') || '')}" alt="${escapeHtml(product.name)}"></a>${pricing.discount ? `<span class="discount-badge">-${pricing.discount}%</span>` : product.discountPercent > 0 ? `<span class="discount-badge">-${product.discountPercent}%</span>` : ''}</div><div class="product-card-body"><div class="product-packaging">${escapeHtml(product.embalagem || 'Unidade')}</div><a class="product-name" href="${routeHref(`#/produto/${productRoute(product)}`)}" title="${escapeHtml(product.name)}">${escapeHtml(product.name)}</a><div class="product-expiry">${product.validade && formatDateBR(product.validade) ? `Val. ${formatDateBR(product.validade)}` : '&nbsp;'}</div><div class="product-card-footer"><div class="product-price">${showOld ? `<s>${fmt(original)}</s>` : ''}<strong>${fmt(pricing.final)}</strong>${pricing.discount ? `<span class="link-discount-note">Campanha ${escapeHtml(pricing.code)}</span>` : ''}</div><div data-control-slot="${escapeHtml(id)}">${quantityControl(product)}</div></div></div></article>`;
}
function campaignBanner() {
  ...
  const messages = { invalid: 'Este link não é válido. Nenhum desconto foi aplicado.', inactive: 'Esta campanha foi desativada. Nenhum desconto foi aplicado.', scheduled: 'Esta campanha ainda não começou. Nenhum desconto foi aplicado.', expired: 'Esta campanha terminou. Nenhum desconto foi aplicado.' };
...
function pageHeader(title, subtitle = '', back = '#/') { return `<header class="page-header">${back ? `<a class="back-button" href="${routeHref(back)}" aria-label="Voltar">←</a>` : ''}<div><h1>${escapeHtml(title)}</h1>${subtitle ? `<p>${escapeHtml(subtitle)}</p>` : ''}</div></header>`; }
function emptyState(title, text) { return `<div class="empty-state"><strong>${escapeHtml(title)}</strong><span>${escapeHtml(text)}</span><a class="primary-button" href="${routeHref('#/categorias')}">Ver categorias</a></div>`; }
function productGrid(products) { return products.length ? `<div class="product-grid">${products.map(productCard).join('')}</div>` : emptyState('Nenhum produto disponível', 'Tente outra categoria ou faça uma nova busca.'); }
function availableCategories() {
  const map = new Map();
  state.products.filter(isAvailable).forEach(product => { const key = product.categoria || 'Outros'; if (!map.has(key)) map.set(key, product); });
  return [...map.entries()].sort((a, b) => a[0].localeCompare(b[0], 'pt-BR'));
}
function categoryStrip(limit = 12) { return `<div class="quick-category-strip">${availableCategories().slice(0, limit).map(([name]) => `<a class="chip" href="${routeHref(`#/categoria/${encodeURIComponent(name)}`)}">${escapeHtml(name)}</a>`).join('')}</div>`; }
function homePage() {
  ...
  return `<div class="page-container home-page"><section class="campaign-hero"><div><small>Pedido já realizado?</small><h1>Acrescente mais produtos em poucos cliques</h1><p>Escolha os itens, revise o complemento e envie direto no WhatsApp. Sem cadastro e sem refazer o pedido.</p></div><div class="campaign-hero-mark"><div><strong>+ itens</strong><span>no seu pedido</span></div></div></section>${campaignBanner()}<section class="content-section"><div class="section-heading"><div><h2>Categorias</h2><p>Encontre rapidamente o que faltou.</p></div><a href="${routeHref('#/categorias')}">Ver todas</a></div>${categoryStrip()}</section>${offers.length ? `<section class="content-section"><div class="section-heading"><div><h2>Ofertas para aproveitar</h2><p>Produtos disponíveis agora.</p></div><a href="${routeHref('#/ofertas')}">Ver todas</a></div>${productGrid(offers)}</section>` : ''}<section class="content-section"><div class="section-heading"><div><h2>Mais produtos</h2><p>Adicione ao pedido que você já fez.</p></div></div>${productGrid(products)}</section></div>`;
}
...
function renderRoute() {
  const route = parseRoute(); state.route = route; resolveCampaign(route);
  let html = '';
  ...
  app.innerHTML = html; searchInput.value = route.name === 'search' ? route.values.join(' ') : '';
  document.getElementById('search-clear').hidden = !searchInput.value; updateNavigation(route.name); updateShell(); window.scrollTo({ top: 0, behavior: 'instant' });
}
function updateNavigation(name) {
  ...
  document.querySelectorAll('a[data-nav]').forEach(link => { const base = link.dataset.nav === 'categories' ? '#/categorias' : link.dataset.nav === 'offers' ? '#/ofertas' : '#/'; link.setAttribute('href', routeHref(base)); });
...
function renderCurrentControls(id) {
  const product = state.productMap.get(String(id)); if (!product) return;
...
function updateShell() {
  const pricing = cartPricing(), count = pricing.items.reduce((sum, item) => sum + item.qty, 0);
  ...
  document.querySelectorAll('[data-direct-whatsapp]').forEach(button => { button.disabled = count <= 0; button.setAttribute('aria-label', count > 0 ? `Enviar ${count} item(ns), total ${fmt(pricing.total)}, no WhatsApp` : 'Adicione produtos para enviar no WhatsApp'); });
  const label = document.querySelector('[data-direct-label]'); if (label) label.textContent = count > 0 ? 'Enviar no WhatsApp' : 'Adicione produtos';
}
function openCheckout() { renderCheckout(); checkoutDrawer.classList.add('open'); checkoutDrawer.setAttribute('aria-hidden', 'false'); overlay.classList.add('show'); document.body.classList.add('drawer-open'); }
function closeCheckout() { checkoutDrawer.classList.remove('open'); checkoutDrawer.setAttribute('aria-hidden', 'true'); overlay.classList.remove('show'); document.body.classList.remove('drawer-open'); }
function renderCheckout() {
  const pricing = cartPricing();
  if (!pricing.items.length) { checkoutContent.innerHTML = `<div class="empty-state"><strong>Nenhum produto escolhido</strong><span>Adicione os itens que deseja incluir no pedido já realizado.</span></div>`; return; }
...
function whatsappMessage() {
  ...
  const message = ['Olá! Quero acrescentar estes produtos ao pedido que já fiz:', '', ...lines, '', `Subtotal: ${fmt(pricing.subtotal)}`, ...(pricing.discount > 0 ? [`Desconto da campanha: -${fmt(pricing.discount)}`] : []), `Total do complemento: ${fmt(pricing.total)}`, ...(campaigns.length ? [`Campanhas: ${campaigns.join(', ')}`] : []), '', 'Por favor, confirme a inclusão no meu pedido.', `Link usado: ${location.href}`].join('\n');
  return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`;
}
function sendWhatsapp() { const pricing = cartPricing(); if (pricing.items.length) window.open(whatsappMessage(), '_blank', 'noopener'); }
...
function clearCart() { state.cart = {}; state.cartOrder = []; state.cartPromotions = {}; persistCart(); updateShell(); renderCheckout(); renderRoute(); }
function bindEvents() {
  window.addEventListener('hashchange', renderRoute);
  document.addEventListener('click', event => {
    const button = event.target.closest('[data-action]'); if (!button) return;
    event.preventDefault(); const id = button.dataset.id;
  ...
  document.addEventListener('error', event => {
    ...
    const fallbacks = String(image.dataset.fallback || '').split('|').filter(Boolean), next = fallbacks.shift();
    image.dataset.fallback = fallbacks.join('|'); image.src = next || '../img/logoantonia5.png';
  }, true);
  ...
  document.querySelector('[data-close-drawer]').addEventListener('click', closeCheckout); overlay.addEventListener('click', closeCheckout);
  document.addEventListener('keydown', event => { if (event.key === 'Escape') closeCheckout(); });
  let searchTimer;
  searchInput.addEventListener('input', () => { clearTimeout(searchTimer); const query = searchInput.value.trim(); document.getElementById('search-clear').hidden = !query; searchTimer = setTimeout(() => { if (query.length >= 2) navigate(`#/busca/${encodeURIComponent(query)}`); }, 450); });
  document.getElementById('search-form').addEventListener('submit', event => { event.preventDefault(); const query = searchInput.value.trim(); if (query) navigate(`#/busca/${encodeURIComponent(query)}`); });
  document.getElementById('search-clear').addEventListener('click', () => { searchInput.value = ''; document.getElementById('search-clear').hidden = true; navigate('#/'); });
}
async function init() {
  bindEvents(); readCart(); app.innerHTML = '<div class="loading-shell"><div></div><div></div><div></div></div>';
  try {
    ...
    state.productMap = new Map(state.products.map(product => [String(product.id), product])); cleanCart(); renderRoute();
  } catch (error) {
    console.error(error); app.innerHTML = `<div class="page-container"><div class="empty-state"><strong>Não conseguimos carregar o catálogo.</strong><span>${escapeHtml(error.message || 'Tente novamente em alguns instantes.')}</span></div></div>`;
