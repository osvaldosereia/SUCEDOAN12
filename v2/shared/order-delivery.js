...
export function orderFingerprint(order){
  const basis={cliente:order?.cliente?.telefone||'',itens:(order?.itens||[]).map(item=>[item.firebaseKey||item.id,item.quantidade,item.precoUnitario]),total:Number(order?.total||0)};
...
export function clearOutbox(){localStorage.removeItem(OUTBOX_KEY);return [];}
...
export function buildDeliveryPreview(envelope){
  return Object.freeze({
    ...
    firebase:Object.freeze({path:`${APP_CONFIG.firebase.nodes.orders}/${envelope.id}`,payload:envelope.order,enabled:false}),
