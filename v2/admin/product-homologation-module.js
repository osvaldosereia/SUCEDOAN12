...
function statusClass(value) {
  if (value === HOMOLOGATION_STATUS.APPROVED) return 'ok';
  if (value === HOMOLOGATION_STATUS.READY) return 'ok';
...
function renderList() {
  ...
  const cards = drafts.map(saved => {
    ...
    const evaluation = record?.evaluation || evaluateProductHomologation(saved.draft, record?.confirmations || {});
    const displayStatus = record?.status || evaluation.status;
    return `<button class="homologation-card" type="button" data-homologation-open="${escapeHtml(saved.id)}"><span><strong>${escapeHtml(saved.draft?.nome || saved.draft?.gtin || 'Produto sem nome')}</strong><small>EAN ${escapeHtml(saved.draft?.gtin || '—')} · salvo ${escapeHtml(saved.savedAt ? new Date(saved.savedAt).toLocaleString('pt-BR') : 'sem data')}</small></span><span class="homologation-card-status"><b class="score ${statusClass(displayStatus)}">${escapeHtml(statusLabel(displayStatus))}</b><small>${evaluation.automaticPassed}/${evaluation.automaticTotal} automáticos · ${evaluation.humanPassed}/${evaluation.humanTotal} humanos</small></span></button>`;
  }).join('');
...
function openReview(productId) {
  ...
  state.records = [...readProductHomologations()];
  const record = state.records.find(item => item.productId === saved.id) || null;
  renderReview(saved, record);
}
