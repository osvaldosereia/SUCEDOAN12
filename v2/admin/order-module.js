...
function localSessionRow(session) {
  ...
  return `<button class="order-row local-dispatch-row" type="button" data-local-order-detail="${esc(session.id)}"><span><strong>${esc(session.envelopeId)}</strong><small>${esc(order.cliente?.nome || 'Cliente não informado')} · ${esc(order.cliente?.telefone || 'Sem telefone')}</small></span><span class="score ${dispatchStatusClass(session.status)}">${esc(dispatchStatusLabel(session.status))}</span><span>${itemCount} un.</span><span>${money(order.total)}</span><span>${session.channels?.whatsapp?.attempts || 0} abertura(s)</span></button>`;
}

function localSessionDetail(session) {
  const order = session.envelope?.order || {};
  const history = (session.history || []).slice(0, 20).map(item => `<article><strong>${esc(item.action || 'evento')}</strong><span>${esc(item.at ? new Date(item.at).toLocaleString('pt-BR') : '')}</span>${item.detail ? `<small>${esc(item.detail)}</small>` : ''}</article>`).join('');
...
function renderLocalSessions() {
  ...
  content.innerHTML = `<section class="module-hero"><span class="eyebrow">HOMOLOGAÇÃO LOCAL</span><h2>Pedidos preparados pelo checkout V2</h2><p>Sessões de teste protegidas por fingerprint, sem mistura com os pedidos reais do Firebase.</p></section>${renderViewTabs()}<section class="metrics">${metric(state.localSessions.length, 'SESSÕES')}${metric(waiting, 'AGUARDANDO WHATSAPP')}${metric(opened, 'WHATSAPP ABERTO')}${metric(errors, 'COM ERRO')}</section><section class="panel"><div class="order-head"><span>Pedido</span><span>Status</span><span>Itens</span><span>Total</span><span>WhatsApp</span></div><div class="order-list">${state.localSessions.map(localSessionRow).join('') || '<div class="empty">Nenhum pedido de homologação preparado neste navegador.</div>'}</div><div class="notice">Os registros são locais deste navegador e servem para testar o fluxo antes da liberação dos canais externos.</div></section>`;
}
...
document.addEventListener('click', event => {
  ...
  if (viewButton) {
    state.view = viewButton.dataset.orderView;
    state.selectedId = '';
    state.selectedLocalId = '';
    render();
    return;
