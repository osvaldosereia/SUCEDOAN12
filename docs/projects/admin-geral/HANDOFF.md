...
Shell V2 do Admin principal está integrado. Gôndolas, Estúdio Criativo, Pedidos e Marketing usam migração direta; Nomes dos Produtos e Imagens IA usam bootstrap allowlisted/fail-safe. Marketing mantém autenticação e gates externos intactos. Foi criada a camada `admin-context-nav-v2.js/css` para workspaces especializados com navegação interna própria, ainda sem ligação. Próximo lote: conectar essa camada aditivamente em Relacionamento e Atendimento sem substituir suas tabs/sidebars internas; depois tratar Inteligência/Aprendizados de forma controlada por ainda usarem a família visual antiga. Manter R3 aberta até cobertura segura das superfícies principais em desktop/tablet/mobile.
