...
## R3 — estado acumulado
- preflight mais recente: branch 37 commits à frente e 0 atrás de `main`; merge-base = HEAD de main;
...
- `admin/gondolas.html`, `admin/creative-studio.html` e agora `admin/pedidos.html` usam o shell compartilhado preservando CSS/JS funcional local;
- Pedidos ganhou menu mobile acessível (`aria-controls`, `aria-expanded`, backdrop e Escape via shell) sem alterar sua API, filtros, impressão ou PDF;
- contrato `tests/admin-subpage-shell-v2-contract.test.mjs` ampliado para cobrir Pedidos e garantir preservação de `pedidos-v2.css/js`;
- mounts gated continuam seguros: o Navigation Contract pode expô-los somente quando a flag correspondente estiver ativa, e o shell de subpágina não cria href para `route.type=mount`, evitando links quebrados fora do index;
- nenhuma flag/runtime externo alterado.

## R3 — próximo lote
1. migrar progressivamente `nomes-produtos.html`, `imagens-ia.html` e demais subpáginas modernas que já usam `styles.css`, sem reescrever sua lógica funcional;
