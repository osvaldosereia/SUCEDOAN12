...
export function renderProductDetail(product,quantity=1){
  const p=product||{};const meta=[p.brand,p.packaging].filter(Boolean).join(' · ');const expiry=validity(p.validity_date);const qty=Math.max(0,Math.floor(Number(quantity)||0));const stock=Math.max(0,Math.floor(Number(p.stock||0)));
  return `<div class="product-detail"><div class="product-detail-head"><h2>Produto</h2><button class="icon-button" type="button" data-close-product aria-label="Fechar">×</button></div><div class="product-detail-photo">${p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.name||'Produto')}" decoding="async">`:'<span aria-hidden="true">□</span>'}</div><div class="product-detail-copy"><h3>${esc(p.name||'Produto')}</h3>${meta?`<p>${esc(meta)}</p>`:''}<strong>${money(p.price)}</strong>${expiry?`<div class="product-validity"><span>Validade</span><b>${esc(expiry)}</b></div>`:''}</div><div class="product-detail-actions"><div class="qty-control product-detail-qty"><button type="button" data-modal-extra-minus aria-label="Diminuir">−</button><b data-modal-extra-qty>${qty}</b><button type="button" data-modal-extra-plus aria-label="Aumentar" ${qty>=stock?'disabled':''}>+</button></div><button class="primary-button product-detail-save" type="button" data-modal-save-product>${qty===0?'Remover do pedido':'Adicionar ao pedido'}</button></div></div>`;
}
