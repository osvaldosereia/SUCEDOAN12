...
export function renderBasketDetail(basket,items=[],total=null){
  ...
  return `<section class="basket-detail basket-selected"><button class="text-button" type="button" data-home>← Voltar para as cestas</button><div class="basket-detail-head"><div class="basket-detail-photo">${basket?.image_url?`<img src="${esc(basket.image_url)}" alt="" decoding="async">`:'<span>□</span>'}</div><div><p class="eyebrow">Cesta escolhida</p><h1>${esc(basket?.name||'Cesta')}</h1><p>Agora você pode ajustar as quantidades permitidas e adicionar outros produtos.</p><strong>${money(basket?.base_price)}</strong></div></div>${notice}<div class="basket-actions"><div class="basket-selected-total"><span>Total do pedido</span><strong>${money(currentTotal)}</strong></div><button class="primary-button" type="button" data-open-cart>Ver pedido</button><button class="secondary-button" type="button" data-add-products>Adicionar mais produtos</button></div><div class="basket-items"><h2>Personalize sua cesta</h2><p class="basket-items-help">O total do pedido é atualizado conforme você altera as quantidades.</p>${items.map(renderBasketItem).join('')}</div></section>`;
}
