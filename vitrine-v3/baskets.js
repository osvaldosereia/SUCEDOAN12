...
export function renderBasketPreview(basket,items=[],selectedBasketId=''){
  ...
  return `<section class="basket-detail basket-preview"><button class="text-button" type="button" data-home>← Voltar</button><div class="basket-detail-head"><div class="basket-detail-photo">${basket?.image_url?`<img src="${esc(basket.image_url)}" alt="" decoding="async">`:'<span>□</span>'}</div><div><p class="eyebrow">Conheça a cesta</p><h1>${esc(basket?.name||'Cesta')}</h1><p>${esc(basket?.description||'Confira os produtos da cesta.')}</p><strong>${money(basket?.base_price)}</strong></div></div>${notice}<div class="basket-preview-cta"><button class="primary-button full" type="button" data-select-basket="${esc(basket?.id||'')}">${esc(cta)}</button><p>${esc(helper)}</p></div><div class="basket-items"><h2>Produtos da cesta</h2><p class="basket-items-help">Confira a composição antes de escolher. Depois você poderá ajustar as quantidades permitidas.</p>${items.map(renderBasketPreviewItem).join('')}</div></section>`;
}

export function renderBasketDetail(basket,items=[],total=null){
  const notice=basket?.ready===false?'<div class="notice">A disponibilidade final será confirmada ao fechar o pedido.</div>':'';
  const currentTotal=total==null?Number(basket?.base_price||0):Number(total||0);
  return `<section class="basket-detail basket-selected"><button class="text-button" type="button" data-home>← Voltar para as cestas</button><div class="basket-detail-head"><div class="basket-detail-photo">${basket?.image_url?`<img src="${esc(basket.image_url)}" alt="" decoding="async">`:'<span>□</span>'}</div><div><p class="eyebrow">Cesta escolhida</p><h1>${esc(basket?.name||'Cesta')}</h1><p>Ajuste o que precisar e continue comprando logo abaixo.</p><strong>${money(basket?.base_price)}</strong></div></div>${notice}<div class="basket-inline-total"><span>Total até agora</span><strong>${money(currentTotal)}</strong><small>O botão Pedido fica disponível para revisar e finalizar quando quiser.</small></div><div class="basket-items"><h2>Personalize sua cesta</h2><p class="basket-items-help">O total é atualizado automaticamente conforme você altera as quantidades.</p>${items.map(renderBasketItem).join('')}</div></section>`;
}
