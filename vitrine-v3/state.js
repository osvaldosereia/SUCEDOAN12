...
const safeProduct=p=>p?{id:p.id,name:p.name,price:Number(p.price||0),stock:Number(p.stock||99),image_url:p.image_url||null,brand:p.brand||null,category:p.category||null,packaging:p.packaging||null}:null;
...
export function estimatedTotal(){if(basketChanged())return null;return Number(state.basket?.base_price||0)+extraSubtotal()}
