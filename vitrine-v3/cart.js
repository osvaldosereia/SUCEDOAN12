...
function basketRow(item){
  const p=item.product||{};const qty=Number(item.quantity||0);const stock=Math.max(0,Number(p.stock||0));const min=Math.max(0,Number(item.min_quantity||0));const max=item.max_quantity==null?Math.max(min,stock):Math.max(min,Number(item.max_quantity));
