(() => {
  ...
  const BUILD='20260904-canecafacil-generator-parity-v2';
  ...
  function installUi(){
    ...
    section.innerHTML=`
      <div class="cf-command-head"><div><strong>Comandos padrão</strong><small>Mesmo padrão do Gerador do Admin Canecas. Selecione um ou vários para somar à próxima criação.</small></div><button type="button" class="mini-button" id="cfCommandRefresh">Atualizar</button></div>
      <form id="cfCommandForm" class="cf-command-form">
        ...
        <div><button type="submit" class="mini-button" id="cfCommandSave">Salvar comando</button><button type="button" class="mini-button" id="cfCommandCancel" hidden>Cancelar</button></div>
      </form>
    ...
    note.innerHTML=`<strong>Automação conectada ao Gerador Canecas</strong><small>Webhook em uso: <code>${esc(hook)}</code></small><small>Saída: arte horizontal 2400×960 + 1 mockup mestre vertical 1200×1600 com duas vistas da mesma caneca.</small>`;
    ...
    positioning.innerHTML='<strong>Posicionamento para sublimação</strong><span>Elementos principais dentro da faixa segura central. Não colocar rosto, palavra principal ou detalhe essencial nos 10% extremos laterais. A arte deve continuar natural na curvatura da caneca e os dois lados precisam ser legíveis no mockup.</span>';
  ...
  async function loadCommands(force=false){if(state.commands.length&&!force)return render();try{state.commands=normalizeCommands(await fbGet(COMMANDS_NODE));state.selected=new Set([...state.selected].filter(id=>state.commands.some(c=>c.id===id)));persist();render()}catch(e){$('#cfCommandList').innerHTML=`<small>Erro ao carregar: ${esc(e.message||e)}</small>`}}
  ...
  function resetForm(){state.editingId='';$('#cfCommandForm')?.reset();$('#cfCommandSave').textContent='Salvar comando';$('#cfCommandCancel').hidden=true}
  ...
  function bindUi(){
    ...
    $('#cfCommandForm').onsubmit=saveCommand;
  ...
  function boot(){installStyles();installUi();loadCommands();document.documentElement.dataset.generatorParity=BUILD}
