(()=>{
  ...
  const MIN_DELAY=450,MAX_DELAY=850;
  ...
  async function typing(delay=naturalDelay()){
    const node=renderTyping();
    await sleep(delay);
    node?.remove();
  }
  ...
  async function ask({text:message='',options=[],className='',delay=naturalDelay(),host=null}={}){
    clearPrompt();
  ...
  async function showOffers(){
    ...
    let products=[];
    try{
      const data=await productApi('page',{offers:true,offset:0,limit:24});
      products=(data.products||[]).filter(validOffer).slice(0,6);
    }catch(error){toast(error.message||'Não consegui carregar as ofertas.')}finally{waiting?.remove()}

    if(!products.length){
      app.assistantMessage('As melhores ofertas de hoje já acabaram, mas posso te mostrar outros produtos.');
      quickReplies([
    ...
    app.assistantMessage(`Separei ${products.length} ${products.length===1?'oferta boa':'ofertas boas'} de hoje para você 👇`);
    const section=stage(2,'Ofertas de hoje','','conversation-offers-stage');if(!section)return null;
    const grid=document.createElement('div');grid.className='conversation-offers-grid';section.appendChild(grid);
  ...
  function renderPostBasketPrompt(){
  ...
  function maskPhone(value=''){
    ...
    if(digits.length<4)return 'meu número';
    const tail=digits.slice(-4),ddd=digits.length>=10?digits.slice(-11,-9):'';
    return ddd?`(${ddd}) *****-${tail}`:`*****-${tail}`;
  }

  function resetCheckoutFlow(){
    Object.assign(checkoutFlow,{stage:null,phone:'',profile:null,selectedAddressId:null,form:null,payment:state.payment||'',locator:null,opening:false,orderSaved:false,whatsappUrl:'',handoffWindow:null});
    ui.checkoutStep='idle';
  }
  ...
  function setupCheckoutStage(){
    ...
    document.querySelectorAll('.checkout-intro-message').forEach(node=>node.remove());
    clearPrompt();
    ...
    return checkoutFlow.stage;
  }

  async function openCheckoutConversation(button){
    if(checkoutFlow.opening||button?.dataset.busy==='1')return;
    checkoutFlow.opening=true;
    try{
      await state.modules.products?.waitForPending?.();
      const data=await api('checkout_preview');state.checkout=data.checkout||{};state.payment=data.payment_method||state.payment||null;
      ...
      if(checkoutFlow.profile?.customer_id)await renderAddressStep();else renderIdentificationStep();
  ...
  function renderIdentificationStep(){
    ...
    host.innerHTML=`<section class="checkout-turn-card"><label><span>WhatsApp com DDD</span><input id="conversationCheckoutPhone" inputmode="tel" autocomplete="tel" placeholder="(65) 99999-9999" value="${escapeHtml(checkoutFlow.phone)}"></label><button id="conversationCheckoutLookup" class="primary" type="button">Continuar</button><small data-checkout-status class="muted"></small></section>`;
    app.assistantMessage('Qual é seu WhatsApp com DDD? Vou procurar seu cadastro.');
    ...
    const submit=async()=>{
      ...
      try{
        const data=await customerApi('lookup_customer',{phone});const profile=data.profile||{customer_id:null,name:'',phone,addresses:[]};
  ...
  async function renderAddressStep(forceNew=false){
    ...
    if(saved&&!forceNew){
      host.innerHTML=`<section class="checkout-address-preview"><small>Endereço salvo</small><strong>${escapeHtml(addressLine(saved))}</strong>${saved.complement?`<span>${escapeHtml(saved.complement)}</span>`:''}</section>`;
      await ask({text:'Posso entregar neste endereço?',delay:0,options:[
    ...
    renderAddressForm(forceNew?{city:'Cuiabá',state:'MT'}:(saved||{city:'Cuiabá',state:'MT'}));
  }
  ...
  function readAddressForm(){
    return {name:text(document.getElementById('conversationCheckoutName')?.value),phone:text(document.getElementById('conversationCheckoutWhatsapp')?.value),address:{street:text(document.getElementById('conversationCheckoutStreet')?.value),number:text(document.getElementById('conversationCheckoutNumber')?.value),neighborhood:text(document.getElementById('conversationCheckoutNeighborhood')?.value),complement:text(document.getElementById('conversationCheckoutComplement')?.value),reference:text(document.getElementById('conversationCheckoutReference')?.value),city:text(document.getElementById('conversationCheckoutCity')?.value),state:text(document.getElementById('conversationCheckoutState')?.value).toUpperCase()||'MT',postal_code:text(document.getElementById('conversationCheckoutPostal')?.value)}};
  }
  ...
  function renderAddressSummary(){
    const host=checkoutBody();if(!host||!checkoutFlow.form)return;
    host.innerHTML='';
    host.appendChild(app.compactToolSummary({className:'checkout-address-compact',title:'Entrega',meta:addressLine(checkoutFlow.form.address),actions:[{label:'Alterar',onClick:()=>renderAddressStep(true)}]}));
  }

  async function requestLocation(status){
    if(!navigator.geolocation){status.textContent='Localização indisponível neste aparelho.';return}
    ...
    navigator.geolocation.getCurrentPosition(async position=>{
      ...
      try{
        ...
        fillAddressInputs(data.address||{});status.textContent='Endereço localizado. Confira antes de continuar.';
  ...
  function fillAddressInputs(address={}){
    const map=[['conversationCheckoutStreet','street',''],['conversationCheckoutNumber','number',''],['conversationCheckoutNeighborhood','neighborhood',''],['conversationCheckoutComplement','complement',''],['conversationCheckoutReference','reference',''],['conversationCheckoutCity','city','Cuiabá'],['conversationCheckoutState','state','MT'],['conversationCheckoutPostal','postal_code','']];
    for(const [id,key,fallback] of map){const input=document.getElementById(id);if(input)input.value=address[key]||fallback}
  }

  function renderPaymentStep(){
    ui.checkoutStep='payment';
    return ask({text:'Como você prefere pagar na entrega?',delay:0,options:Object.entries(paymentLabels).map(([key,label])=>({label,onChoose:()=>{checkoutFlow.payment=key;state.payment=key;return renderConfirmationStep()}}))});
  }
  ...
  function renderConfirmationStep(){
    ...
    app.assistantMessage('Perfeito. Confira tudo antes de confirmar:');
    host.innerHTML=`<section class="checkout-confirm-card"><div><span>Entrega</span><strong>${escapeHtml(addressLine(checkoutFlow.form.address))}</strong></div><div><span>Pagamento</span><strong>${escapeHtml(paymentLabels[checkoutFlow.payment]||'')}</strong></div><div class="checkout-confirm-total"><span>Total</span><strong>${money(total)}</strong></div><button type="button" class="confirm" data-confirm-conversation-order>Confirmar pedido</button><small data-confirm-status class="muted"></small></section>`;
    host.querySelector('[data-confirm-conversation-order]').onclick=event=>confirmAndSend(event.currentTarget);app.scrollTo(host,{block:'center'});
  }

  function reserveWhatsAppWindow(){
    if(isAdminTest())return null;
    try{const target=window.open('about:blank','da_whatsapp_order');if(target){try{target.opener=null;target.document.title='Abrindo WhatsApp';target.document.body.textContent='Salvando seu pedido…'}catch{}}return target}catch{return null}
  }
  ...
  function openSavedWhatsApp(){
    if(isAdminTest()||!checkoutFlow.whatsappUrl)return;
    if(checkoutFlow.handoffWindow&&!checkoutFlow.handoffWindow.closed){try{checkoutFlow.handoffWindow.location.replace(checkoutFlow.whatsappUrl);checkoutFlow.handoffWindow=null;return}catch{}}
    location.replace(checkoutFlow.whatsappUrl);
  }

  async function confirmAndSend(button){
    if(checkoutFlow.orderSaved){openSavedWhatsApp();return}
    if(button?.dataset.busy==='1'||!checkoutFlow.form||!checkoutFlow.payment)return;
    ...
    try{
      ...
      const payload={payment_method:state.payment,delivery_address:form.address,save_address:true,...(checkoutFlow.locator?{delivery_locator:checkoutFlow.locator}:{})};
      const data=await app.confirmOrder(payload);const simulated=data.admin_test===true||isAdminTest();if(!simulated&&!data?.order?.order_id&&!data?.order?.order_number)throw new Error('Não consegui confirmar o número do pedido.');
  ...
  function renderSuccess(data={}){
    ui.checkoutStep='success';clearPrompt();if(!checkoutFlow.stage)return;const order=data.order||{},simulated=data.admin_test===true||isAdminTest();
    checkoutFlow.stage.innerHTML=`<div class="checkout-card success ${simulated?'admin-test-success':''}"><div class="check">${simulated?'🧪':'✓'}</div><h2>${simulated?'Teste concluído':'Pedido confirmado!'}</h2><p>${order.order_number?`Pedido <strong>#${escapeHtml(order.order_number)}</strong> salvo com sucesso.`:'Seu pedido foi salvo com sucesso.'}</p>${simulated?'<small>Modo de teste: nenhum pedido real foi criado e o WhatsApp não será aberto.</small>':'<small>Vamos abrir o WhatsApp para você enviar a confirmação à equipe.</small>'}</div>`;
    state.modules.help?.setCheckoutMode?.(false);app.scrollTo(checkoutFlow.stage,{block:'center'});
  }
  ...
  function interceptSemanticClicks(event){
    ...
    if(button.closest('.order-review-actions')){
      ...
      else choose('Quero alterar meu pedido',{onChoose:()=>originalProductsRenderEntry?.({auto:false})});
      return;
    }
    if(button.matches('[data-review-products]')){
      event.preventDefault();event.stopImmediatePropagation();choose('Quero alterar os produtos',{onChoose:()=>originalProductsRenderEntry?.({auto:false})});
    }
  }

  app.registerModule('conversation',{ask,choose,say,quickReplies,clearPrompt,typing,isBusy:()=>ui.busy,showOffers,renderPostBasketPrompt,openCheckoutConversation});

  if(upsellModule){
    state.modules.upsell.renderAfterBasket = () => false;
    state.modules.upsell.renderBeforeCheckout = () => false;
    state.modules.upsell.showOffers=showOffers;
  }
  if(productsModule&&originalProductsRenderEntry){
    state.modules.products.renderEntry=options=>{
      if(options?.auto===true && state.selectedBasket)return renderPostBasketPrompt();
      return originalProductsRenderEntry(options||{});
    };
  }
