(()=>{
  ...
  function renderPicker(){
    ...
    document.querySelectorAll('.basket-picker-message,.basket-preview-message,.products-browser-message').forEach(el=>el.remove());
  ...
  async function chooseBasket(selection,button){
    ...
    try{
      ...
      state.selectedBasket.selected_total=selectedBasketTotal();
      ...
      try{state.modules.upsell?.renderAfterBasket?.(summary)}catch{}
  ...
  function selectedBasketTotal(){const basket=state.selectedBasket;if(!basket)return 0;return previewBasketTotal({...basket,items:state.basketItems||[]})}
  function refreshSelectedBasketViews(){
    const basket=state.selectedBasket;if(!basket)return;
    const count=basketItemCount(),value=selectedBasketTotal();basket.selected_total=value;
    document.querySelectorAll('.basket-compact-summary .conversation-tool-summary-copy small').forEach(node=>{node.textContent=`${count} ${count===1?'item':'itens'} · ${money(value)}`});
    document.querySelectorAll('[data-basket-total]').forEach(node=>{node.textContent=money(value)});
  }
  ...
  function expandSelectedBasket(){
    ...
    const copy=document.createElement('div');copy.innerHTML=`<h2>${escapeHtml(basket.name||'Sua cesta')}</h2><div class="value" data-basket-total>${money(selectedBasketTotal())}</div>`;hero.appendChild(copy);inner.appendChild(hero);
    const list=document.createElement('div');list.className='basket-list';inner.appendChild(list);
