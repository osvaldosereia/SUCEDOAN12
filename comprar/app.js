(()=>{
  ...
  const errors={
    ...
    room_not_found:'Esta compra não foi encontrada.',
    room_expired:'Este link expirou.',
    room_closed:'Este pedido já foi concluído.',
    payment_method_required:'Escolha como vai pagar na entrega.',
    invalid_payment_method:'Escolha uma forma de pagamento válida.',
    customer_name_required:'Informe seu nome.',
    valid_whatsapp_required:'Informe seu WhatsApp com DDD.',
    customer_document_required:'Informe o CPF para concluir.',
    customer_identification_required:'Preencha seus dados para concluir.',
    delivery_address_required:'Informe o endereço de entrega.',
    empty_cart:'Escolha uma cesta ou produto antes de finalizar.',
    product_not_available:'Este produto não está disponível no momento.',
    quantity_exceeds_stock:'A quantidade escolhida é maior que o estoque disponível.',
    quantity_exceeds_customer_limit:'Você atingiu o limite permitido para este produto.',
    invalid_response:'Não consegui concluir esta ação agora.'
  };

  function errorText(value){
    const key=String(value||'invalid_response');
  ...
  function toast(message){
    const element=$('toast');
    if(!element)return;
    element.textContent=errorText(message);
    element.classList.add('show');
    clearTimeout(toast.timer);
    toast.timer=setTimeout(()=>element.classList.remove('show'),2800);
  }

  function scrollTo(element,{block='start',behavior='smooth'}={}){
    if(!element)return;
    requestAnimationFrame(()=>element.scrollIntoView({block,behavior}));
  }

  function cartCount(cart=state.cart){
  ...
  function bodyFor(action,payload={}){
    return {action,...(token?{token}:{}),...payload};
  }
  ...
  async function post(url,action,payload={},options={}){
    ...
    const response=await fetch(url,{
      ...
      body:JSON.stringify(bodyFor(action,payload)),
      cache:'no-store',
  ...
  async function uploadMedia(kind,file,durationMs=0){
    ...
    form.append('file',file,file.name||`${kind}-${Date.now()}`);
    const response=await fetch(config.api,{method:'POST',body:form,cache:'no-store'});
    const data=await response.json().catch(()=>({ok:false,error:'invalid_response'}));
    if(!response.ok||data?.ok===false)throw new Error(errorText(data?.detail||data?.error||`Erro ${response.status}`));
    return data;
  }
  ...
  function isAdminTest(){
    return params.get('admin_test')==='1'&&window.parent!==window;
  }
  ...
  function registerModule(name,module){
    if(!name||!module)throw new Error('Módulo inválido.');
  ...
  function registerPendingProductSync(productId,promise){
    ...
    state.pendingProductSyncs.set(key,promise);
    Promise.resolve(promise).finally(()=>{
      if(state.pendingProductSyncs.get(key)===promise)state.pendingProductSyncs.delete(key);
    });
    return promise;
  }

  async function waitForPendingProductSyncs(){
    while(state.pendingProductSyncs.size){
      const pending=[...state.pendingProductSyncs.values()];
  ...
  function image(url,alt=''){
    ...
    element.src=url||fallbackImage;
    element.alt=alt;
    element.loading='lazy';
    element.decoding='async';
    element.onerror=()=>{if(element.src!==fallbackImage)element.src=fallbackImage};
    return element;
  }

  function bubble(message,who='assistant'){
    const timeline=$('timeline');
    if(!timeline||!message)return null;
    const element=document.createElement('div');
    element.className=`bubble ${who}`;
    element.textContent=String(message);
    timeline.appendChild(element);
    return element;
  }

  function stage(number,title,subtitle='',className=''){
    const timeline=$('timeline');
    if(!timeline)return null;
    const section=document.createElement('section');
    section.className=`stage ${className}`.trim();
    section.innerHTML=`<div class="stage-head"><span class="stage-no">${escapeHtml(number)}</span><div><strong>${escapeHtml(title)}</strong>${subtitle?`<small>${escapeHtml(subtitle)}</small>`:''}</div></div>`;
    timeline.appendChild(section);
    return section;
  }

  function clearStages(selector='.stage'){
    document.querySelectorAll(selector).forEach(element=>element.remove());
  }

  function renderStart(){
    clearStages('.stage.start-stage');
    const section=stage(1,'Como posso ajudar?','Escolha uma opção ou escreva normalmente.','start-stage');
    if(!section)return;
    const chips=document.createElement('div');
    chips.className='chips start-chips';
    const choices=[
      ['Cestas Básicas',()=>state.modules.baskets?.renderPicker?.()],
      ['Ofertas',()=>state.modules.products?.renderEntry?.({section:'Ofertas'})],
      ['Para Você',()=>state.modules.products?.renderEntry?.({section:'Para Você'})],
      ['Para Casa',()=>state.modules.products?.renderEntry?.({section:'Para Casa'})]
    ];
  ...
  async function ensureToken(){
    ...
    history.replaceState({},'',`${location.pathname}?s=${encodeURIComponent(token)}`);
    return token;
  }
  ...
  async function start(){
    ...
    try{
      ...
      $('loadingCard')?.remove();
      if(state.cart?.basket_id&&state.modules.baskets?.restoreFromOpen){
        await state.modules.baskets.restoreFromOpen(data);
      }else{
        renderStart();
      }
      return state;
      ...
      started=false;
      $('loadingCard')?.remove();
      bubble(error.message||'Não consegui abrir sua compra.');
      throw error;
    }
  }
  ...
  window.DA_COMPRAR_APP={
    ...
    $,text,money,escapeHtml,fallbackImage,errorText,toast,scrollTo,image,bubble,stage,clearStages,
    post,api,productApi,customerApi,checkoutApi,basketStorefrontApi,uploadMedia,confirmOrder,
    setCart,cartCount,registerModule,registerPendingProductSync,waitForPendingProductSyncs,renderStart,start
  };
})();
