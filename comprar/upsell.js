(()=>{
  ...
  const MAX_AFTER_BASKET=4,MAX_BEFORE_CHECKOUT=3,MIN_RELEVANT=2,RELEVANCE_THRESHOLD=10;
  ...
  function chooseSection(){const blob=JSON.stringify(state.cart?.items||[]).toLowerCase();return /limpeza|lavanderia|casa|pet/.test(blob)?'Para Você':'Para Casa'}
  function score(product){
    ...
    const categories=cartCategories();const pcat=String(product.customer_subcategory||product.customer_category||product.category||'').toLowerCase();if(pcat&&!categories.has(pcat))value+=20;
  ...
  function renderCards(products,limit,message,host){
    if(!host||products.length<MIN_RELEVANT)return false;
  ...
  async function renderAfterBasket(host){
    if(stopped||afterBasketUsed||!host)return false;afterBasketUsed=true;
    const products=await fetchCandidates();return renderCards(products,MAX_AFTER_BASKET,'Separei algumas coisas que podem completar sua compra.',host);
  }
  async function renderBeforeCheckout(host){
    if(stopped||beforeCheckoutUsed||!host)return false;beforeCheckoutUsed=true;
    const products=await fetchCandidates();return renderCards(products,MAX_BEFORE_CHECKOUT,'Antes de finalizar, tem algo que costuma faltar em casa?',host);
  }
