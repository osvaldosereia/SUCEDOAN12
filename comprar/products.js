(()=>{
  ...
  function renderEntry({auto=false,section=''}={}){
    document.querySelectorAll('.stage.products-entry-stage,.stage.products-stage,.stage.checkout-stage').forEach(el=>el.remove());
    const host=stage(2,'Adicionar mais produtos','Se quiser, escolha uma seção.','products-entry-stage');
    if(!host)return null;
  ...
  async function openSection(label='Para Você'){
    ...
    document.querySelectorAll('.stage.products-entry-stage,.stage.products-stage,.stage.checkout-stage').forEach(el=>el.remove());
    const host=stage(2,'Adicionar mais produtos',config.label,'products-stage');
    activeStage=host;
    if(!host)return;

    const selector=document.createElement('div');selector.className='chips products-section-chips';
    for(const choice of ['Para Você','Para Casa','Ofertas']){
      const button=document.createElement('button');button.type='button';button.className=`chip ${choice===label?'active':''}`;button.textContent=choice;button.onclick=()=>openSection(choice);selector.appendChild(button);
    }
    ...
    try{
      const [filters]=await Promise.all([
        productApi('filters',{customer_category:config.customerCategory,offers:config.offers}),
      ...
      state.productFilters.availableCategories=list;
      state.productFilters.availableSubcategories=filters.subsubcategories||{};
      renderFilterChips(list,filters.subsubcategories||{});
    }catch(error){
      if(requestGeneration!==generation)return;
      toast(error.message);
    }
  ...
  function renderFilterChips(categories=[],subMap={}){
    ...
    const categoryHost=activeStage.querySelector('[data-categories]');
    const subHost=activeStage.querySelector('[data-subcategories]');
    if(!categoryHost||!subHost)return;
    categoryHost.innerHTML='';
    const all=document.createElement('button');all.type='button';all.className=`chip ${!state.productFilters.subcategory?'active':''}`;all.textContent='Todos';all.onclick=()=>{state.productFilters.subcategory='';state.productFilters.subsubcategory='';resetProducts()};categoryHost.appendChild(all);
    for(const category of categories){
      const button=document.createElement('button');button.type='button';button.className=`chip ${state.productFilters.subcategory===category.key?'active':''}`;button.textContent=category.label;button.onclick=()=>{state.productFilters.subcategory=category.key;state.productFilters.subsubcategory='';resetProducts()};categoryHost.appendChild(button);
    }
    subHost.innerHTML='';
    const leaves=state.productFilters.subcategory?(subMap[state.productFilters.subcategory]||[]):[];
    subHost.hidden=!leaves.length;
    ...
    for(const sub of leaves){
      const button=document.createElement('button');button.type='button';button.className=`chip ${state.productFilters.subsubcategory===sub.key?'active':''}`;button.textContent=sub.label;button.onclick=()=>{state.productFilters.subsubcategory=sub.key;resetProducts()};subHost.appendChild(button);
    }
  }
  ...
  function resetProducts(){
    const requestGeneration=++generation;
    resetPagination();
    renderFilterChips(state.productFilters.availableCategories||[],state.productFilters.availableSubcategories||{});
    loadMore(requestGeneration);
  }
  ...
  async function loadMore(requestGeneration=generation){
    ...
    loading=true;
    const loadingNode=activeStage?.querySelector('[data-products-loading]');if(loadingNode)loadingNode.textContent='Carregando…';
    try{
      const data=await productApi('page',{
        customer_category:state.productFilters.customerCategory,
        offers:state.productFilters.offers,
  ...
  function productState(product){
    ...
    let sync=syncState.get(key);
    if(!sync){
      const quantity=Math.max(0,Number(product.quantity||0));
      sync={desiredQuantity:quantity,confirmedQuantity:quantity,syncing:false,product};
      syncState.set(key,sync);
    }else sync.product=product;
    return sync;
  }

  function productCard(product){
    const sync=productState(product);
    const card=document.createElement('article');card.className='product';card.dataset.productId=String(product.id);
  ...
  function refreshProductViews(productId){
    const sync=syncState.get(String(productId));if(!sync)return;
    document.querySelectorAll(`[data-product-id="${CSS.escape(String(productId))}"] [data-qty]`).forEach(node=>{node.textContent=String(sync.desiredQuantity)});
    if(detailLayer?.dataset.productId===String(productId)){const node=detailLayer.querySelector('[data-detail-qty]');if(node)node.textContent=String(sync.desiredQuantity)}
  }

  function applyOptimisticDelta(product,before,after){
    const cart=state.cart?structuredClone(state.cart):{items:[],total:0};
  ...
  function changeQuantity(product,delta){
    const sync=productState(product),before=sync.desiredQuantity,next=Math.max(0,before+delta);
    ...
    if(!sync.syncing){
      const promise=syncProduct(product,sync);
      registerPendingProductSync(product.id,promise);
    }
  }

  async function syncProduct(product,sync){
    sync.syncing=true;
    const cardSelector=`[data-product-id="${CSS.escape(String(product.id))}"]`;
    document.querySelectorAll(cardSelector).forEach(card=>card.classList.add('saving'));
    try{
      while(sync.desiredQuantity!==sync.confirmedQuantity){
        const target=sync.desiredQuantity;
        try{
          const data=await api('set_quantity',{product_id:product.id,quantity:target});
          const confirmed=Math.max(0,Number(data.quantity??target));
          sync.confirmedQuantity=confirmed;
          if(sync.desiredQuantity===target&&confirmed!==target)sync.desiredQuantity=confirmed;
          product.quantity=sync.desiredQuantity;
          if(data.cart)setCart(data.cart);
          refreshProductViews(product.id);
        }catch(error){
          const current=sync.desiredQuantity;
          sync.desiredQuantity=sync.confirmedQuantity;
      ...
      sync.syncing=false;
      document.querySelectorAll(cardSelector).forEach(card=>card.classList.remove('saving'));
    }
  }

  async function waitForPending(){
    while(pendingProductSyncs.size)await app.waitForPendingProductSyncs();
  }

  function openDetail(product){
    closeDetail();
    const sync=productState(product);
    detailLayer=document.createElement('div');detailLayer.className='product-detail-layer open';detailLayer.dataset.productId=String(product.id);
    ...
    detailLayer.querySelector('.product-detail-image').appendChild(image(product.image_url,product.name));
    detailLayer.querySelector('.product-detail-backdrop').onclick=closeDetail;detailLayer.querySelector('.product-detail-close').onclick=closeDetail;
    detailLayer.querySelector('[data-detail-minus]').onclick=()=>changeQuantity(product,-1);detailLayer.querySelector('[data-detail-plus]').onclick=()=>changeQuantity(product,1);
    document.body.appendChild(detailLayer);document.body.classList.add('product-detail-open');
  }
