...
const sampleJob = {
  ...
  duration_seconds: 20,
  product_snapshot: {
    name: 'Di Pomodoro Passata Tomate Nor Foods 680ml',
    image_url: 'https://example.com/product.webp'
  },
  resolved_assets: {
    packshot: { kind: 'packshot', url: 'https://example.com/product.webp' }
  },
  creative_plan: {
    concept: 'O vermelho transforma o prato.',
    hook: 'Um fio de macarrão segue um brilho vermelho.',
    payoff: 'O prato ganha cor e celebra o molho.',
    scenes: [{ summary: 'Abertura' }, { summary: 'Transformação' }, { summary: 'Fechamento' }]
  }
};
...
test('worker exporta helpers e preserva parâmetros oficiais do vídeo', async () => {
  ...
  const args = mod.buildFfmpegArgs(sampleJob, '/tmp/product.webp', '/tmp/output.mp4');
  const joined = args.join(' ');
  assert.match(joined, /1080x1920|1080:1920|1080.*1920/);
  assert.match(joined, /30/);
  assert.match(joined, /20/);
  assert.doesNotMatch(joined, /voice|narrat/i);
});
...
test('worker usa exatamente as RPCs transacionais existentes', async () => {
  ...
  assert.deepEqual(mod.completePayload(sampleJob, 'worker-1', 'x/final.mp4', { codec: 'h264' }), {
    p_job_id: sampleJob.id,
    p_worker_id: 'worker-1',
    p_output_path: 'x/final.mp4',
    p_output_metadata: { codec: 'h264' },
    p_actual_cost_brl: 0
  });
  assert.deepEqual(mod.failPayload(sampleJob, 'worker-1', 'boom'), {
    p_job_id: sampleJob.id,
    p_worker_id: 'worker-1',
    p_error: 'boom'
  });
...
test('workflow do worker é manual e agendado, sem expor secrets', () => {
  ...
  assert.match(yaml, /ffmpeg\s+-version/);
  ...
  assert.match(yaml, /node scripts\/creative-studio-render-worker\.mjs/);
});
