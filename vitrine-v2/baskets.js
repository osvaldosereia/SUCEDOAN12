...
export function renderBasketList(baskets=[]){
  ...
  return `<section class="section"><div class="section-head"><div><span class="eyebrow">Escolha rápida</span><h2>Cestas básicas</h2><p>Abra uma cesta, confira os produtos e personalize se quiser.</p></div></div><div class="basket-grid">${baskets.map(b=>`<article class="basket-card">
    <button type="button" data-open-basket="${esc(b.id)}" aria-label="Abrir ${esc(b.name)}">
      <div class="basket-photo">${b.image_url?`<img src="${esc(b.image_url)}" alt="" loading="lazy" decoding="async">`:'<span>🧺</span>'}</div>
      <div class="basket-info"><div class="flag-row">${b.is_featured?'<span class="flag">Destaque</span>':''}</div><h3>${esc(b.name)}</h3><p>${esc(b.description||'')}</p><div class="basket-bottom"><strong>${money(b.base_price)}</strong><span>${esc(b.item_count||0)} itens</span></div></div>
    </button>
  </article>`).join('')}</div></section>`;
}

export function renderBasketDetail(basket,items=[]){
  const notice=basket?.ready===false?'<div class="stock-notice">A composição está disponível para consulta enquanto o estoque termina de ser conferido. A disponibilidade final é validada ao fechar o pedido.</div>':'';
  return `<section class="detail-shell">
    ...
    ${notice}<div class="detail-actions"><button type="button" class="button primary" data-open-cart>Ver minha compra</button><button type="button" class="button secondary" data-browse-products>Adicionar outros produtos</button></div>
...
function renderBasketItem(item){
  ...
  return `<article class="line-item" data-basket-product="${esc(item.product_id)}">
    ...
    <div class="line-copy"><strong>${esc(p.name||'Produto')}</strong><small>${esc([p.brand,p.packaging].filter(Boolean).join(' · '))}</small>${item.removable&&canEdit?'<em>Pode retirar</em>':''}${!canEdit?'<em class="checking">Estoque em conferência</em>':''}</div>
