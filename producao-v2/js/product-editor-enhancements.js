...
async function enhance() {
  ...
  if (content && !content.querySelector('[data-admin-advanced-content]')) {
    ...
    contentPanel.innerHTML = `<div class="panel-header"><div><h3>Publicação no catálogo</h3><p>Campos realmente usados para organizar os produtos na loja.</p></div></div><div class="form-grid">${field('ordem','Ordem no catálogo',pendingValue(product,'ordem'),'number','step="1"')}<label>Destaque<select data-field="destaque" data-admin-extra="1"><option value="0" ${!highlighted ? 'selected' : ''}>Não</option><option value="1" ${highlighted ? 'selected' : ''}>Sim</option></select></label></div><div class="panel-header" style="margin-top:18px"><div><h3>Histórico de imagens</h3><p>Escolha uma imagem anterior e depois clique em Salvar produto.</p></div></div>${imageHistoryMarkup(product)}`;
