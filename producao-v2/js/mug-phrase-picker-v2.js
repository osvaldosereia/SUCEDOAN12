...
function openLibrary(panel){const d=dialog();d._s.panel=panel;if(!d.open)d.showModal();load(d);}
function install(panel){
  if(!panel?.classList.contains('mugv7')||panel.dataset.phraseLazyBuild===BUILD)return false;const box=panel.querySelector('.mugv7-instruction');if(!box)return false;styles();panel.dataset.phraseLazyBuild=BUILD;const b=document.createElement('button');b.type='button';b.className='button secondary compact mug-phrase-open';b.textContent='Frases para a arte · 5.200';b.title='Abrir 26 catálogos de 200 frases';b.addEventListener('click',()=>openLibrary(panel));box.appendChild(b);return true;
}
