const BUILD = '20260826-mug-make-client-guard-v15-accepted-poll';
...
async function waitFinalProduct(payload, fetcher){
  ...
  while (Date.now() < deadline) {
    try {
      ...
      console.warn('[Canecas V15] Aguardando Firebase após Accepted:', error);
    }
    await sleep(POLL_MS);
  }
  return null;
...
function install(){
  ...
  window.fetch = async function guardedMugMakeFetch(input, init){
    ...
    if (response.ok && /^accepted\.?$/i.test(text(raw)) && payload?.action === 'finalize_mug_product') {
      console.info('[Canecas V15] Make respondeu Accepted; aguardando produto final no Firebase.', payload.request_id);
      const recovered = await waitFinalProduct(payload, previousFetch);
      if (recovered) return jsonResponse(recovered, 200);
      return jsonResponse({ ok:false, error:'A automação aceitou a finalização, mas o produto final não apareceu no Firebase dentro do tempo esperado. Atualize o histórico em alguns instantes.', upstream_status:response.status, accepted:true, request_id:payload.request_id, client_contract:BUILD }, 504);
    }
