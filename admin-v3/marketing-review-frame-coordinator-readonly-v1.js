...
function createLeaseToken(session,epoch){
  return `${TOKEN_PREFIX}${encodeURIComponent(text(session.asset_id))}:${Number(session.revision)}:${epoch}:${encodeURIComponent(text(session.session_token))}`;
}
...
function makeRecord(session,summary,epoch){
  ...
  return Object.freeze({
    ...
    session,
    review_status:text(session.review_status||'blocked'),
...
function rememberLease(record){
  ...
  while(leaseOrder.length>MAX_LEASES){
    const evicted=leaseOrder.shift();
    if(evicted)leasesByToken.delete(evicted);
  }
}
...
function rememberCurrent(record){
  ...
  while(assetOrder.length>MAX_ASSETS){
    const evictedAsset=assetOrder.shift();
    if(evictedAsset)currentByAsset.delete(evictedAsset);
  }
}
...
function validateFrame(frame){
  ...
  const assetId=text(frame.asset_id);
  const revision=Number(frame.revision);
  const epoch=Number(frame.epoch);
  const leaseToken=text(frame.lease_token);
...
function sameFrameIdentity(identity,record){
  return !!identity&&!!record
    && identity.asset_id===record.asset_id
    && identity.revision===record.revision
    && identity.epoch===record.epoch
    && identity.lease_token===record.lease_token;
}

function safeStatus(frame,status,reason=''){
  return Object.freeze({
    schema_version:SCHEMA,
...
function getFrameStatus(frame){
  ...
  const lifecycle=getSessionApi().getSessionStatus(record.session)||{};
  const sessionStatus=text(lifecycle.status);
...
function resolveRoot(rootOrId){
  ...
  if(!root)throw new Error('review_frame_mount_required');
  return root;
}

function mountCurrent(rootOrId,frame){
  const root=resolveRoot(rootOrId);
  const initial=getFrameStatus(frame);
  if(initial.status!=='current')throw new Error(`review_frame_not_current:${initial.status}`);

  const identity=validateFrame(frame);
  const record=identity?leasesByToken.get(identity.lease_token):null;
  ...
  getSurfaceApi().mountCurrent(root,record.session);
  return confirmed;
}

function getRegistryStats(){
  return Object.freeze({
    asset_count:currentByAsset.size,
    lease_count:leasesByToken.size,
    max_assets:MAX_ASSETS,
    max_leases:MAX_LEASES,
  });
}

window.DAMarketingReviewFrameCoordinatorReadonlyV1=Object.freeze({
  schema_version:SCHEMA,
  openFrame,
  getFrameStatus,
  mountCurrent,
