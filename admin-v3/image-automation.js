...
function renderIssues(){
  const key=$('issueFilter').value||'bad_images',rows=state.triage[key]||[],totals={bad_images:state.stats.bad_images||0,unprocessed:state.stats.not_processed||0,problems:state.stats.problems||0,pending:state.stats.pending||0,ignored:state.stats.ignored||0};
  $('issueCountLabel').textContent=`${Number(totals[key]||0)} produto(s)`;
  $('issueRows').innerHTML=rows.map(renderIssue).join('')||'<div class="empty">Nenhum produto neste grupo.</div>';
}
...
function openRepair(product){
  ...
  const d=diagnosis(product);
  const source=safeUrl(product.image_source_url||product.image_original_url||product.image_url);
  const generated=safeUrl(product.image_ai_url||product.image_url);
  ...
  $('repairDiagnosis').textContent=`${d.title}: ${d.text}`;
  $('repairRawError').textContent=product.image_ai_error||'';
  setCompareImage('repairSourceImage',source);
  setCompareImage('repairGeneratedImage',generated);
  $('repairSourceUrl').value=product.image_source_url||'';
  $('repairNote').value=product.image_ai_admin_note||'';
  $('repairManualPrompt').value=product.image_ai_manual_prompt||'';
  ...
  $('generateIndividualManual').classList.toggle('hidden',!!product.image_ai_ignored||!product.image_ai_manual_review_required);
  $('approveGeneratedManual').classList.toggle('hidden',!!product.image_ai_ignored||!product.image_ai_manual_review_required||!safeUrl(product.image_ai_url));
  $('useCurrentSource').classList.toggle('hidden',!!product.image_ai_ignored||!safeUrl(product.image_url));
...
async function load(){
  setBusy(true);
  try{
    const d=await call('status'),c=d.control||{},s=d.stats||{};
    state.stats=s;state.triage={...state.triage,...(d.triage||{})};
    ...
    $('recentResults').innerHTML=(d.results||[]).map(renderResult).join('')||'<div class="empty">Nenhum resultado ainda.</div>';
    $('recentBatches').innerHTML=(d.batches||[]).map(renderBatch).join('')||'<div class="empty">Nenhum lote ainda.</div>';
    renderIssues();
  }catch(e){
    if(e.code==='admin_session_required'||e.code==='admin_session_invalid'){$('automationArea').classList.add('hidden');$('unlockPanel').classList.remove('hidden')}
...
$('repairDialog').addEventListener('click',e=>{if(e.target===$('repairDialog'))closeRepair()});
...
$('imageZoomDialog').addEventListener('click',e=>{if(e.target===$('imageZoomDialog'))closeZoom()});

$('approveGeneratedManual').addEventListener('click',async()=>{
  const p=state.current;if(!p)return;
  if(!confirm('Aprovar esta imagem gerada como está e retirar o produto da lista de imagens ruins?'))return;
  try{
    await call('approve_manual',{product_id:p.id,note:$('repairNote').value});
    toast('Imagem aprovada manualmente.','success');
    closeRepair();
    await load();
  }catch(e){toast(e.message,'error')}
});
$('generateIndividualManual').addEventListener('click',async()=>{
  const p=state.current;if(!p)return;
  const manual_prompt=$('repairManualPrompt').value.trim();
  if(!manual_prompt){toast('Escreva a orientação para a geração individual.','error');return}
  if(!confirm('Gerar outra imagem individualmente em qualidade média usando sua orientação? Esta opção custa mais que o lote de 18.'))return;
  try{await call('generate_manual',{product_id:p.id,manual_prompt});toast('Nova geração individual solicitada.','success');closeRepair();await load()}catch(e){toast(e.message,'error')}
});
