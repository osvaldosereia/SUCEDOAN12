...
const BUILD = '20260831-admin-canecas-li-dual-sync-v2';
...
async function fbPatch(path, value) {
  const r = await fetch(`${FIREBASE_BASE}/${path}.json`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify(value) });
  if (!r.ok) throw new Error(`Firebase ${r.status}`);
  return r.json().catch(() => null);
}
...
async function enqueueProduct(product, source = 'admin_github') {
  ...
  const item = { product_key: key, sku, nome: text(product.nome), acao: 'sincronizar', status: 'pendente', solicitado_em: at, atualizado_em: at, solicitado_por: source, tentativas: 0 };
  await Promise.all([
...
function setProgress(value) { const el = $('#cfDualSyncProgress'); if (el && el.textContent !== (value || '')) el.textContent = value || ''; }
async function withBusy(fn) {
  if (busy) return;
  busy = true; applyDisabledState();
  try { await fn(); } catch (error) { toast(error?.message || error, true); }
...
async function saveDrawerThenQueue() {
  ...
  toast('Cadastro salvo e enviado para o GitHub Actions.');
}
...
async function refreshQueuePanel(force = false) {
  const panel = $('#cfDualSyncPanel');
  if (!panel || refreshBusy) return;
  if (!force && Date.now() - lastPanelRefresh < 12000) return;
  refreshBusy = true;
  try {
    const s = statsOf(await readQueue());
    const html = [metric(s.pendente, 'aguardando'), metric(s.processando, 'processando'), metric(s.waiting, 'aguarda imagens'), metric(s.error, 'erro'), metric(s.done, 'concluídas')].join('');
...
function ensurePanel() {
  ...
  panel.innerHTML = `<div class="cf-dual-head"><div><h3>CanecaFácil · sincronização</h3><p><b>GitHub é o principal.</b> Processa automaticamente e procura o SKU antes de criar. <b>Make é a reserva imediata</b>.</p></div><div class="cf-dual-actions"><button type="button" class="primary cf-github-primary" id="cfGithubSelected">GitHub · selecionadas</button><button type="button" class="secondary" id="cfGithubAllActive">GitHub · todas ativas</button><button type="button" class="secondary" id="cfGithubRetry">GitHub · reenviar erros</button><button type="button" class="secondary" id="cfGithubRefresh">Atualizar status</button></div></div><div id="cfDualSyncStats"></div><div class="cf-dual-note"><b>Make:</b> use os botões “via Make” nas ações em lote ou dentro da caneca.</div><div id="cfDualSyncProgress" class="cf-dual-note"></div>`;
  root.prepend(panel);
  ...
  $('#cfGithubRefresh', panel).onclick = () => withBusy(() => refreshQueuePanel(true));
  void refreshQueuePanel(true);
  return true;
}
...
function renameMakeButtons() {
  setButtonText($('#cfBulkActivateCf'), 'Ativar CanecaFácil + Make', 'Contingência: sincronização imediata pelo Make.');
  setButtonText($('#cfBulkActivateBoth'), 'Ativar nos dois + Make', 'Contingência: sincronização imediata pelo Make.');
  setButtonText($('#cfBulkSync'), 'Sincronizar selecionadas via Make', 'Contingência: sincronização imediata pelo Make.');
  setButtonText($('#cfSaveSync'), 'Salvar + sincronizar via Make', 'Reserva imediata: salva e sincroniza pelo Make.');
  setButtonText($('#cfSyncNow'), 'Sincronizar agora via Make', 'Reserva imediata: sincroniza pelo Make.');
}
function enhanceDrawer() {
  ...
  if (!github) {
    ...
    github.textContent = 'Salvar + enviar via GitHub';
  ...
  if (key && github.dataset.labelKey !== key) {
    ...
    void getMug(key).then(p => {
      ...
      setButtonText(github, text(liOf(p).produto_id) ? 'Salvar + atualizar via GitHub' : 'Salvar + publicar via GitHub');
    }).catch(() => {});
  }
  if (!$('#cfDualDrawerInfo', content)) {
    ...
    info.innerHTML = '<strong>Escolha o canal</strong>GitHub = principal e automático. Make = reserva imediata.';
...
function updateUi() {
  ...
  applyDisabledState();
  void refreshQueuePanel(false);
}
function scheduleUi(delays = [0, 180, 650, 1400]) {
  for (const delay of delays) setTimeout(updateUi, delay);
}

// IMPORTANTE: sem MutationObserver global. Ele causava ciclo de DOM e a lista piscava.
...
window.addEventListener('admin-canecas:drawer', event => { if (event.detail?.kind === 'mug') scheduleUi([30, 250]); });
document.addEventListener('DOMContentLoaded', () => scheduleUi());
setTimeout(updateUi, 350);
setInterval(() => { if (location.hash.includes('mugs')) updateUi(); }, 4000);
