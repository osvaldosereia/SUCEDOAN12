(() => {
  ...
  const BUILD = '20260902-admin-creations-manager-v1';
  ...
  function toast(message, error=false) {
    ...
    toast.t = setTimeout(() => { el.hidden = true; }, error ? 4300 : 2200);
  }
  ...
  function metrics() {
    const out = { ready:0, cart:0, awaiting:0, released:0, total:state.creations.length };
  ...
  function filtered() {
    ...
    let list=state.creations.filter(c=>{
      ...
      if (!q) return true;
      return norm([c.id,c.__key,modelName(c),clientName(c),clientEmail(c),clientPhone(c),orderId(c),order?.id,si.label].join(' ')).includes(q);
    });
  ...
  function row(c) {
    ...
    const pay=payment(c,order), qty=quantity(c,order), phone=clientPhone(c), email=clientEmail(c);
    return `<div class="cfm-row" data-cfm-id="${esc(id)}">
      ...
      <div class="cfm-code"><strong>${esc(id)}</strong><small>${esc(formatDate(createdAt(c)))}</small></div>
      <div class="cfm-model"><strong title="${esc(modelName(c))}">${esc(modelName(c))}</strong><small>${qty} unidade${qty===1?'':'s'}</small></div>
      <div class="cfm-client"><strong title="${esc(clientName(c))}">${esc(clientName(c))}</strong><span title="${esc(email || phone)}">${esc(email || phone || 'Sem contato')}</span></div>
  ...
  function render() {
    ...
    if (subtitle) subtitle.textContent='Gerencie criações, carrinhos, pedidos e liberação para produção.';
    host.innerHTML=`<div class="cfm-shell">
      ...
      <div class="cfm-toolbar">
        <input id="cfmSearch" type="search" placeholder="Buscar CF-ID, cliente, e-mail, modelo ou pedido…" value="${esc(state.query)}">
        ...
        <div class="cfm-toolbar-actions"><button class="cfm-btn" id="cfmExport">CSV</button><button class="cfm-btn primary" id="cfmReload">Atualizar</button></div>
      </div>
  ...
  function openDetail(id) {
    ...
    const si=statusInfo(c), order=si.order, image=artUrl(c), orderCode=text(order?.id || orderId(c)), pay=payment(c,order), qty=quantity(c,order);
    ...
    content.innerHTML=`<h2>Arte ${esc(id)}</h2><div class="subtitle">${esc(formatDate(createdAt(c)))} · ${esc(si.label)}</div>
      ...
      <div class="cfm-detail-grid">
        ...
        <div class="cfm-detail-box"><span>Produção</span><strong>${esc(si.key==='released'?'LIBERADA':'Não liberada')}</strong></div>
      </div>
      <div class="notice" style="margin-top:12px">${si.key==='cart'?'Cliente aprovou e colocou no carrinho. Ainda não existe pedido concluído.':si.key==='awaiting_payment'?'Pedido criado na Loja Integrada. A produção permanece bloqueada até confirmação do pagamento.':si.key==='released'?'Pagamento confirmado. Esta criação está liberada para produção.':si.key==='ready'?'Arte criada e salva em Minhas Artes. Ainda não foi encomendada.':'Status operacional: '+esc(si.label)}</div>
      <div class="cfm-detail-actions">
        ${image?`<a class="cfm-btn primary" href="${esc(image)}" target="_blank">Abrir arte</a>`:''}
  ...
  function exportCsv() {
    const rows=filtered();
    const cols=['CF-ID','Data','Modelo','Cliente','Email','WhatsApp','Status','Pedido','Pagamento','Quantidade'];
    const csv=[cols, ...rows.map(c=>{const si=statusInfo(c),o=si.order;return [text(c.id||c.__key),formatDate(createdAt(c)),modelName(c),clientName(c),clientEmail(c),clientPhone(c),si.label,text(o?.id||orderId(c)),payment(c,o)==='pago'?'Pago':'Pendente',quantity(c,o)]})]
  ...
  window.CFArtesGeradas={reload:()=>load(true),open:openDetail};
