...
const BUILD = '20260830-admin-canecas-storefront-github-v1';
...
async function enrichLiPayload(payload) {
  ...
  if(!images.every(isHttp)) throw new Error('Vitrine incompleta: precisam existir 2 mockups + esquerda + centro + direita.');
...
async function renderDrawerCrops(){
  ...
  const crops=cropUrlsOf(p), ready=cropSetReady(p), status=text(p.vitrine_recortes_status||p.vitrine_recortes?.status||'pendente'), anchor=$('.drawer-actions',content); if(!anchor)return;
  const section=document.createElement('div'); section.id='cfStorefrontCropsPreview'; section.className='form-section';
  section.innerHTML=`<h3>Vitrine CanecaFácil · GitHub Actions</h3><div class="notice ${ready?'':'warn'}" style="margin-bottom:10px"><b>${ready?'3 recortes prontos':`Recortes: ${status}`}</b><br>Processamento feito no GitHub com Sharp, sem IA e sem Base64 no Make. A horizontal inteira continua somente no Admin/Impressão. <a href="${ACTIONS_URL}" target="_blank" rel="noopener">Ver automação no GitHub</a>.</div>${ready?`<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px"><img src="${crops.left}" alt="Vista esquerda" style="width:100%;object-fit:contain"><img src="${crops.center}" alt="Vista centro" style="width:100%;object-fit:contain"><img src="${crops.right}" alt="Vista direita" style="width:100%;object-fit:contain"></div>`:'<button type="button" class="secondary" id="cfRefreshGitHubCrops">Verificar novamente</button>'}`;
...
export { BUILD, ensureCrops, cropUrlsOf, cropSetReady };
