...
const BUILD='20260830-banner-manager-v6';
...
const PROFILE={
  ...
  tarja:{label:'Banner Tarja',desktop:[1270,70],mobile:[361,70],desktopAi:'1536x1024',mobileAi:'1536x1024',hint:'Avisos curtos e benefícios rápidos.'},
  vitrine:{label:'Banner Vitrine',desktop:[850,200],mobile:[722,170],desktopAi:'1536x1024',mobileAi:'1536x1024',hint:'Campanhas entre vitrines.'},
...
function commandsHtml(){
  return `<div class="banner-command-box">
    <div class="banner-command-head"><div><b>Instruções reutilizáveis = PROMPT</b><small>O Make enviará ao OpenAI somente o texto das instruções marcadas, sem acrescentar nenhum prompt próprio.</small></div></div>
    ...
    <div class="banner-command-form"><input id="bnCommandName" placeholder="Nome da instrução"><textarea id="bnCommandText" placeholder="Escreva aqui o prompt completo ou uma parte dele."></textarea><button type="button" class="banner-btn alt" id="bnSaveCommand">Salvar instrução</button></div>
  </div>`
}
...
function historyHtml(){
  ...
  return `<div class="banner-library">${state.history.map(h=>`<article class="banner-saved-card">
    ...
    <div class="banner-saved-info"><b>${esc(h.nome||'Banner')}</b><small>${esc(PROFILE[h.tipo]?.label||h.tipo||'')} · ${esc(new Date(h.criado_em||0).toLocaleString('pt-BR'))}</small><small>${Number(h.produtos_count||0)} imagem(ns) · ${Number(h.instrucoes_count||0)} instrução(ões)</small></div>
...
function render(){
  ...
  root.innerHTML=`<div class="banner-shell">
    <div class="banner-note"><strong>Fluxo simples:</strong> escolha o formato, marque as imagens e marque as instruções. <b>Somente as instruções selecionadas formam o prompt.</b> O formato é enviado separadamente como dado técnico.</div>
    <section class="banner-panel"><div class="bp-head"><div><h2>Make + OpenAI</h2><p>Sem prompt fixo na automação.</p></div><span class="banner-status ${cfg.webhook?'ok':''}">${cfg.webhook?'Configurado':'Pendente'}</span></div><div class="bp-body"><div class="banner-config"><input id="bannerWebhook" placeholder="https://hook.eu1.make.com/..." value="${esc(cfg.webhook||window.__CANECAS_ADMIN_CONFIG__?.bannerWebhook||'')}"><button class="banner-btn alt" id="saveBannerWebhook">Salvar webhook</button></div></div></section>
    <div class="banner-grid"><section class="banner-panel"><div class="bp-head"><div><h2>Novo banner</h2><p>Formato + imagens + instruções selecionadas. Nada mais é enviado como briefing.</p></div></div><div class="bp-body">
      ...
      <div class="banner-note" style="margin-top:12px"><strong>${p.label}</strong> · Desktop ${fmtDims(p.desktop)} · Celular ${fmtDims(p.mobile)}.</div>
      <h3 class="banner-subtitle">Imagens que o OpenAI deve receber</h3>${productPickerHtml()}
      <h3 class="banner-subtitle">Instruções reutilizáveis</h3>${commandsHtml()}
      <div class="banner-loading ${state.busy?'show':''}">${state.busy?'Enviando todas as imagens + prompt selecionado e gerando Desktop/Celular…':''}</div>
      <div class="banner-toolbar" style="margin-top:14px"><button class="banner-btn teal" id="generateBanner" ${state.busy?'disabled':''}>Gerar Desktop + Celular</button><button class="banner-btn alt" id="clearBanner">Limpar seleção</button></div>
    </div></section>
    <section class="banner-panel"><div class="bp-head"><div><h2>Prévia e arquivos</h2><p>Resultado direto do OpenAI, ajustado ao tamanho final e salvo no Admin.</p></div></div><div class="bp-body">${resultHtml()}</div></section></div>
...
function bind(){
  ...
  $$('[data-banner-product]').forEach(b=>b.onclick=()=>{const k=b.dataset.bannerProduct;if(state.selectedProducts.has(k))state.selectedProducts.delete(k);else if(state.selectedProducts.size>=MAX_PRODUCTS)return toast(`O GPT Image aceita até ${MAX_PRODUCTS} imagens por edição.`,true);else state.selectedProducts.add(k);render()});
...
function payload(){
  ...
  const images=products.map(x=>({image_url:pImage(x)})).filter(x=>/^https?:\/\//i.test(x.image_url));
  const prompt=commands.map(c=>text(c.texto)).filter(Boolean).join('\n\n');
  return {
    action:'generate_banner_from_selected_instructions',
    request_id:bannerId(),
    build:BUILD,
    banner:{
...
async function generate(){
  ...
  const req=payload();
  if(!req.images.length)return toast('Selecione pelo menos uma caneca com imagem.',true);
  if(!req.prompt)return toast('Selecione pelo menos uma Instrução reutilizável. Ela será o prompt.',true);
  if(req.images.length!==state.selectedProducts.size)return toast('Uma ou mais canecas selecionadas não possuem URL de imagem pública.',true);
  ...
  try{
    ...
    if(!r.ok)throw new Error(`Make ${r.status}: ${raw.slice(0,260)}`);
    ...
    if(!desktop||!mobile)throw new Error('O cenário precisa devolver as imagens Desktop e Celular.');
    state.result={id:req.request_id,payload:req,images:{desktop,mobile}};
    const saved=await persistFinalAssets(state.result);
    const meta={nome:`${pLabel(req.banner.type)} · ${new Date().toLocaleString('pt-BR')}`,tipo:req.banner.type,status:'salvo',payload:req,produtos_count:req.images.length,instrucoes_count:req.instruction_ids.length,thumb:saved.thumb,tem_desktop:true,tem_mobile:true,criado_em:nowIso(),atualizado_em:nowIso()};
    await fbWrite(`${BANNER_NODE}/${safeKey(req.request_id)}`,meta,'PUT');
    await audit('banner_ia_salvo',{banner_id:req.request_id,tipo:req.banner.type,imagens:req.images.length,instrucoes:req.instruction_ids.length});
    state.history=[{id:req.request_id,...meta},...state.history.filter(h=>h.id!==req.request_id)].slice(0,60);
    toast('Banner Desktop + Celular salvo no Admin.');
  }catch(e){toast(e.message||e,true)}finally{state.busy=false;render()}
}
function pLabel(type){return PROFILE[type]?.label||type||'Banner'}

function loadImage(url){return new Promise((resolve,reject)=>{const img=new Image();if(!/^data:/i.test(url))img.crossOrigin='anonymous';img.onload=()=>resolve(img);img.onerror=()=>reject(new Error('Não foi possível carregar a imagem gerada.'));img.src=url})}
...
async function reuseSaved(id){
  ...
  state.result=null;render();toast('Formato, imagens e instruções carregados.');
}
