...
function manualPrompt(product){
  ...
  return `Crie uma fotografia quadrada de catálogo do MESMO produto da referência. IDENTIDADE FIXA: ${identity}. Use a referência SOMENTE para identificar o produto: marca, variante, formato, tampa, cores, proporções e rótulo. Não use nem preserve o ambiente, fundo, mesa, parede, piso, prateleira, iluminação ambiente ou cenário da referência. A instrução administrativa abaixo serve apenas para corrigir composição/limpeza e NUNCA autoriza trocar produto, versão, peso, sabor, fragrância, marca ou embalagem.\n\nINSTRUÇÃO DO ADMIN: ${instruction}\n\nREGRAS OBRIGATÓRIAS: mostrar uma única unidade do produto, inteiro da base ao topo e de lateral a lateral, centralizado e com margens confortáveis; remover mãos, dedos, frutas, alimentos decorativos, copos, pratos, mesa, tecidos, cenário, preço, selo solto, logo flutuante, outro produto e qualquer elemento que não faça parte física do produto; corrigir recorte sem amputar nenhuma parte da embalagem. FUNDO É REGRA DE HARD FAIL: não preserve o fundo da imagem original. Remova completamente todo o fundo original e qualquer vestígio dele. Substitua tudo ao redor do produto por fundo sólido, chapado e uniforme #ECECEC, que deve ocupar toda a imagem de ponta a ponta e chegar aos quatro cantos. Não usar fundo branco, quase branco diferente de #ECECEC, degradê, vinheta, transparência, painel branco, moldura branca, faixa branca, borda branca, parede, piso, mesa, prateleira, textura, cenário ou sombra de ambiente. Iluminação profissional de estúdio; textura fotográfica realista; única sombra adicional permitida é uma sombra de contato macia e fisicamente plausível na base; não parecer render 3D artificial.`;
}
...
const validationSchema={type:'object',additionalProperties:false,properties:{
  ...
  background_uniform:{type:'boolean'},background_is_ececec:{type:'boolean'},white_background:{type:'boolean'},original_background_visible:{type:'boolean'},white_border:{type:'boolean'},
...
},required:['pass','same_product','packaging_color_match','shape_match','label_match','product_complete','bad_crop','bad_cutout','extra_elements','professional_photo','natural_contact_shadow','background_uniform','background_is_ececec','white_background','original_background_visible','white_border','fidelity_score','composition_score','cutout_score','background_score','critical_issue']};
async function validate(key,source,candidate){
  const body={model:VALIDATOR_MODEL,store:false,max_output_tokens:480,reasoning:{effort:'low'},input:[{role:'user',content:[
    {type:'input_text',text:'Compare a primeira imagem (referência) com a segunda (candidata). A referência é verdade SOMENTE para identidade do produto. Aprove somente se for o mesmo produto/variante e a candidata mostrar o produto inteiro, sem recorte amputado e sem qualquer elemento extra. HARD FAIL para mãos, frutas, alimentos, copos, pratos, cenário, preços, outro produto, parte faltando ou produto encostado/cortado. HARD FAIL DE FUNDO: fundo branco; fundo original total ou parcialmente preservado; parede, mesa, prateleira, piso, textura, gradiente, cenário ou sombra de ambiente herdados da referência; moldura branca, faixa branca ou borda branca; transparência ou painel branco. Exija fundo sólido e uniforme #ECECEC ocupando toda a imagem até os quatro cantos. Marque background_uniform=true e background_is_ececec=true somente se isso estiver claramente atendido; white_background, original_background_visible e white_border devem ser false. Exija aparência fotográfica profissional, sombra de contato natural somente na base e formato/cor/rótulo fiéis. Não use conhecimento externo.'},
  ...
  ]}],text:{format:{type:'json_schema',name:'manual_product_image_validation_v2',strict:true,schema:validationSchema}}};
  ...
  const v={pass:p?.pass===true,same_product:p?.same_product===true,packaging_color_match:p?.packaging_color_match===true,shape_match:p?.shape_match===true,label_match:p?.label_match===true,product_complete:p?.product_complete===true,bad_crop:p?.bad_crop===true,bad_cutout:p?.bad_cutout===true,extra_elements:p?.extra_elements===true,professional_photo:p?.professional_photo===true,natural_contact_shadow:p?.natural_contact_shadow===true,background_uniform:p?.background_uniform===true,background_is_ececec:p?.background_is_ececec===true,white_background:p?.white_background===true,original_background_visible:p?.original_background_visible===true,white_border:p?.white_border===true,fidelity_score:score(p?.fidelity_score),composition_score:score(p?.composition_score),cutout_score:score(p?.cutout_score),background_score:score(p?.background_score),critical_issue:clean(p?.critical_issue,260)};
  const accepted=v.pass&&v.same_product&&v.packaging_color_match&&v.shape_match&&v.label_match&&v.product_complete&&!v.bad_crop&&!v.bad_cutout&&!v.extra_elements&&v.professional_photo&&v.natural_contact_shadow&&v.background_uniform&&v.background_is_ececec&&!v.white_background&&!v.original_background_visible&&!v.white_border&&v.fidelity_score>=0.90&&v.composition_score>=0.90&&v.cutout_score>=0.90&&v.background_score>=0.90;
