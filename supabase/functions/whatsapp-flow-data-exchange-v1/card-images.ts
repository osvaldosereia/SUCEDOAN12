...
function hydrateFastBasketOptions(items:unknown[]):unknown[]{
  return items.slice(0,9).map((item)=>{
    if(!item||typeof item!=="object"||Array.isArray(item))return item;
    const option={...(item as Record<string,unknown>)};
    delete option.image_url;
    option.image=FALLBACK_IMAGE_BASE64;
    return option;
  });
}
...
async function hydrateNavigationProductItems(items:unknown[],supabaseUrl:string):Promise<unknown[]>{
  ...
  return await mapLimited(items.slice(0,20),2,async(item)=>{
    ...
    const existing=String(start.src||"").trim();
    delete start.image_url;
    delete start.image;

    if(existing&&existing.length<=COMPACT_LIST_MAX_BASE64_CHARS&&existing.length<=budget){
      start.src=existing;
      budget-=existing.length;
    }else{
      delete start.src;
      if(imageUrl&&budget>0){
        ...
        if(image&&image.length<=COMPACT_LIST_MAX_BASE64_CHARS&&image.length<=budget){
          start.src=image;
          budget-=image.length;
        }
      }
...
export async function hydrateExperienceImagesWithCards(
  response:unknown,
  supabaseUrl:string,
  definitionSlug:string|null=null,
):Promise<unknown>{
  if(!response||typeof response!=="object"||Array.isArray(response))return response;
  const raw=response as Record<string,unknown>;
  const rawData=raw.data;
  const screen=String(raw.screen||"");
  const isFastCandidate=definitionSlug==="flow-cestas-comercial-v5";

  let hydrated=response;
  if(isFastCandidate&&screen==="CESTAS"&&rawData&&typeof rawData==="object"&&!Array.isArray(rawData)){
    const data=rawData as Record<string,unknown>;
    if(Array.isArray(data.baskets))data.baskets=hydrateFastBasketOptions(data.baskets as unknown[]);
  }else{
    hydrated=await hydrateExperienceImages(response,supabaseUrl);
  }
