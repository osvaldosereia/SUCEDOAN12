...
const reply=(body:unknown,status=200,origin:string|null=null,cache=CACHE)=>new Response(JSON.stringify(body),{
  status,
...
Deno.serve(async(req:Request)=>{
  ...
  if(resource==="home"){
    const basketResult=await sb.from("basket_templates")
      .select("id,name,description,image_url,base_price,sort_order,is_featured,basket_template_items(quantity,product:products(is_active,physically_verified,stock))")
      .eq("is_active",true)
    ...
    if(!configured.error&&configured.data?.length){
      categoryRows=configured.data;
    }else{
    ...
    let featured:any[]=[];
    const featuredResult=await sb.from("products")
      .select("id,name,price,image_url,brand,category,packaging,is_offer,sort_order")
      .eq("is_active",true).eq("physically_verified",true).gt("stock",0).eq("storefront_featured",true)
      .order("sort_order",{ascending:true}).order("name",{ascending:true}).limit(8);
    if(!featuredResult.error)featured=(featuredResult.data||[]).map(publicProduct);
    if(!featured.length){
      const offers=await sb.from("products")
        .select("id,name,price,image_url,brand,category,packaging,is_offer,sort_order")
        .eq("is_active",true).eq("physically_verified",true).gt("stock",0).eq("is_offer",true)
        .order("sort_order",{ascending:true}).order("name",{ascending:true}).limit(8);
      if(!offers.error)featured=(offers.data||[]).map(publicProduct);
    }

    const baskets=(basketResult.data||[]).map((b:any)=>{
  ...
  if(resource==="category"||resource==="search"){
    const page=integer(requestUrl.searchParams.get("page"),1,100000,1);
    const limit=integer(requestUrl.searchParams.get("limit"),1,24,12);
    const from=(page-1)*limit;
    const to=from+limit;
    const category=clean(requestUrl.searchParams.get("name"),120);
    const q=clean(requestUrl.searchParams.get("q"),100);
    ...
    let query=sb.from("products")
      .select("id,name,price,image_url,brand,category,packaging,is_offer,sort_order")
      .eq("is_active",true).eq("physically_verified",true).gt("stock",0)
      .range(from,to)
    ...
    if(error)return reply({ok:false,error:"products_failed"},500,origin,NO_STORE);
    const rows=data||[];
  ...
  if(resource==="basket"){
    const id=clean(requestUrl.searchParams.get("id"),80);
    if(!/^[0-9a-f-]{36}$/i.test(id))return reply({ok:false,error:"invalid_id"},400,origin,NO_STORE);
    const {data:b,error}=await sb.from("basket_templates")
      .select("id,name,description,image_url,base_price,is_featured,basket_template_items(id,product_id,quantity,removable,quantity_editable,min_quantity,max_quantity,sort_order,product:products(id,name,price,stock,image_url,brand,category,packaging,is_active,physically_verified))")
    ...
    return reply({
      ok:true,
      basket:{id:(b as any).id,name:(b as any).name,description:(b as any).description||null,image_url:(b as any).image_url||null,base_price:Number((b as any).base_price||0),is_featured:(b as any).is_featured===true,ready:basketReady(items)},
      items:items.sort((a:any,c:any)=>Number(a.sort_order||0)-Number(c.sort_order||0)).map((i:any)=>({product_id:i.product_id,quantity:Number(i.quantity||0),removable:i.removable===true,quantity_editable:i.quantity_editable===true,min_quantity:Number(i.min_quantity||0),max_quantity:i.max_quantity==null?null:Number(i.max_quantity),product:publicProduct(i.product)}))
    },200,origin);
  }
