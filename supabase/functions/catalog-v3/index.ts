...
const publicProduct=(p:any)=>({id:p.id,name:p.name,price:Number(p.price||0),stock:Math.max(0,Math.floor(Number(p.stock||0))),image_url:p.image_url||null,brand:p.brand||null,category:p.category||null,sales_category:p.sales_category||null,packaging:p.packaging||null,is_offer:p.is_offer===true,sort_order:Number(p.sort_order||0)});
const productFields="id,name,price,stock,image_url,brand,category,sales_category,packaging,is_offer,sort_order";
...
Deno.serve(async(req:Request)=>{
  ...
  if(resource==="home"){
    ...
    if(!configured.error&&configured.data?.length){categoryRows=configured.data}else{
      const fallback=await sb.from("products").select("sales_category").eq("is_active",true).gt("stock",0).not("sales_category","is",null).limit(5000);
      if(fallback.error)return reply({ok:false,error:"categories_failed"},500,origin,NO_STORE);
      const names=[...new Set((fallback.data||[]).map((row:any)=>clean(row.sales_category,120)).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"pt-BR"));
  ...
  if(resource==="category"||resource==="search"){
    ...
    if(resource==="category"){query=query.eq("sales_category",category);if(sub)query=query.eq("category",sub)}
    ...
    if(resource==="category"){
      const subResult=await sb.from("products").select("category").eq("is_active",true).gt("stock",0).eq("sales_category",category).not("category","is",null).limit(5000);
