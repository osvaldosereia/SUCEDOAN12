...
Deno.serve(async(req:Request)=>{
  ...
  if(action==="overview"){
    const [
      ...
      sb.from("marketing_assets").select("id,campaign_id,title,media_kind,generation_mode,status,version,template_id,source_refs,edit_spec,render_spec,output_spec,estimated_cost_cents,actual_cost_cents,updated_at").neq("status","archived").order("updated_at",{ascending:false}).limit(100),
