...
Deno.serve(async(req:Request)=>{
  ...
  if(action==='relationship_overview'){
    const [
    ...
    if(errors.length)return json(origin,{ok:false,error:'relationship_overview_failed',detail:text(errors[0]?.message,500)},500);
    return json(origin,{
      ok:true,
  ...
  if(action==='relationship_audit'){
    const limit=int(body?.limit,10,100);
    const [actionsR,eventsR,providerR,metaR]=await Promise.all([
      sb.from('ai_action_executions')
        .select('id,action_key,status,channel,customer_id,decision,side_effect_performed,estimated_cost_brl,actual_cost_brl,created_at,finished_at')
        .order('created_at',{ascending:false}).limit(limit),
      sb.from('marketing_events')
        .select('id,entity_type,entity_id,event_type,actor_id,data,external_side_effect,created_at')
        .order('created_at',{ascending:false}).limit(limit),
      sb.from('channel_provider_event_receipts')
        .select('id,provider_key,processing_status,conversation_id,customer_id,context,occurred_at,received_at')
        .order('received_at',{ascending:false}).limit(limit),
      sb.from('meta_control_plane_errors')
        .select('id,operation,provider_error_code,message,severity,retryable,occurred_at,resolved_at')
        .order('occurred_at',{ascending:false}).limit(limit)
    ]);
    const err=actionsR.error||eventsR.error||providerR.error||metaR.error;
    if(err)return json(origin,{ok:false,error:'relationship_audit_failed',detail:text(err.message,500)},500);
    return json(origin,{
      ok:true,
      actions:actionsR.data||[],
      marketing_events:eventsR.data||[],
      provider_events:providerR.data||[],
      meta_errors:metaR.data||[],
      read_only:true,
      external_side_effect:false,
      version:'cm1.15-v1'
    });
