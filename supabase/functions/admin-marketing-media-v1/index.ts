...
function xml(v:unknown){return String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&apos;"}[c]||c))}
function wrapText(input:string,maxChars:number,maxLines:number){
  const words=clean(input,500).split(" ").filter(Boolean),lines:string[]=[];let line="";
...
function tspans(lines:string[],x:number,y:number,lineHeight:number){return lines.map((l,i)=>`<tspan x="${x}" y="${y+i*lineHeight}">${xml(l)}</tspan>`).join("")}
function brl(v:unknown){return num(v).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}
...
function textSlideSvg(width:number,height:number,headline:string,cta:string){
