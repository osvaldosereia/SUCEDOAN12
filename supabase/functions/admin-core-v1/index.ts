...
Deno.serve(async(req:Request)=>{
  ...
  if(action==="customer_history"){
    ...
    if(segmentsError)return respond({ok:false,error:"customer_segments_failed",detail:segmentsError.message},400);
    ...
    return respond({ok:true,intelligence:intelligence||{},segments:segments||{},orders:history||[],total,page,limit});
  }
  ...
  if(action==="save_customer"){
    ...
    const customerId=customer.id;if(phone){await sb.from("customer_phones").delete().eq("customer_id",customerId);const {error}=await sb.from("customer_phones").insert({customer_id:customerId,phone_e164:phone,source:"admin",is_primary:true});if(error)return respond({ok:false,error:"phone_save_failed",detail:error.message},400)}
    if(email){await sb.from("customer_emails").delete().eq("customer_id",customerId);const {error}=await sb.from("customer_emails").insert({customer_id:customerId,email,email_normalized:email,verification_status:"unverified",is_primary:true,source:"admin",evidence:{}});if(error)return respond({ok:false,error:"email_save_failed",detail:error.message},400)}
