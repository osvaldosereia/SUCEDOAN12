...
async function authorize(req:Request){const url=Deno.env.get("SUPABASE_URL"),key=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");if(!url||!key)return{error:json({ok:false,error:"server_config"},500)};const token=(req.headers.get("Authorization")||"").replace(/^Bearer\s+/i,"").trim();if(!token)return{error:json({ok:false,error:"missing_token"},401)};const sb=createClient(url,key,{auth:{persistSession:false,autoRefreshToken:false}});const {data:ud,error:ue}=await sb.auth.getUser(token);if(ue||!ud?.user?.id)return{error:json({ok:false,error:"invalid_user"},401)};const {data:admin,error:ae}=await sb.from("admin_users").select("role,is_active").eq("user_id",ud.user.id).maybeSingle();if(ae)return{error:json({ok:false,error:"admin_lookup_failed"},500)};if(!admin?.is_active)return{error:json({ok:false,error:"admin_not_authorized"},403)};return{sb,user:ud.user,admin}}
...
Deno.serve(async(req:Request)=>{
  ...
  if(action==="health")return json({ok:true,mode:"balance_only",window_minutes:30,capture_queue:true,max_batch:100});
  if(action==="bootstrap"){
    let adopted:any={adopted_products:0,closed_checkpoints:0};
    if(admin.role!=="viewer"){const {data,error}=await sb.rpc("adopt_open_inventory_fast_balance_checkpoints_v2",{p_user_id:user.id});if(!error&&data)adopted=data}
  ...
  const {data:applied,error:applyError}=await sb.rpc("apply_inventory_fast_scan_events_v3",{p_user_id:user.id,p_device_label:deviceLabel,p_events:rpcEvents});
