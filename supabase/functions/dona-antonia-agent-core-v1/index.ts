...
Se o cliente disser apenas que quer alterar seus dados, sem informar qual campo nem fornecer o novo valor, não escreva nada e não encaminhe para humano apenas por isso: faça uma pergunta curta para identificar qual dado deseja alterar.
...
function allowedForTopic(topic:string,allNames:string[]){
  ...
  const basketCommerce=["wa_list_baskets","wa_get_basket_contents","wa_find_baskets_by_items",...basketState,"wa_get_recommendations","wa_add_product","wa_set_quantity","wa_replace_product",...basketWrites,"wa_request_address_flow","wa_confirm_order",...core];
  ...
  const map:Record<string,string[]>={
    ...
    basket_customization:[...basketState,"wa_get_basket_contents","wa_find_baskets_by_items","wa_create_basket_replacement","wa_open_basket_storefront","wa_add_more_products","wa_set_quantity","wa_replace_product","wa_get_recommendations","wa_start_basket_checkout",...core],
    product_search:["wa_search_products","wa_get_product","wa_get_cart","wa_get_basket_contents","wa_find_baskets_by_items","wa_add_product","wa_set_quantity","wa_create_search_showcase","wa_create_basket_replacement","wa_open_basket_storefront","wa_add_more_products","wa_start_order_checkout",...core],
    ...
    general:["wa_get_policy","wa_search_products","wa_list_baskets","wa_get_basket_contents","wa_find_baskets_by_items","wa_get_cart","wa_get_basket_state","wa_get_checkout_contact","wa_request_address_flow","wa_handoff_human"]
  };
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const {data:packet,error:packetError}=await sb.rpc("build_whatsapp_agent_core_packet_v4",{p_conversation_id:job.conversation_id,p_message_id:job.message_id});if(packetError||!packet?.enabled)throw new Error("agent_packet_failed");
    const toolset=arr(packet.toolset),allNames=toolset.map((t:any)=>clean(t.name,64)).filter(Boolean);if(!toolset.length)throw new Error("toolset_empty");const plannerPacket=minimizePacket(packet),postSaleContext=isConfirmedOrderPostSaleContext(plannerPacket),topic=postSaleContext?"post_sale":clean(packet.topic||plannerPacket.topic||"general",60);plannerPacket.topic=topic;const packetJson=JSON.stringify(plannerPacket),base=baseline(job.result);
    ...
    const executeReadTool=async(name:string,args:any)=>{const a=obj(args);if(name==="wa_search_products")return await sb.rpc("search_whatsapp_sellable_products_agent_v1",{p_query:clean(a.query,120),p_limit:Math.floor(clampNumber(a.limit,1,10,6))});if(name==="wa_get_product")return await sb.rpc("get_whatsapp_sellable_product_v1",{p_product_id:clean(a.product_id,80)});if(name==="wa_get_cart")return await sb.rpc("get_whatsapp_sales_cart_v1",{p_conversation_id:job.conversation_id});if(name==="wa_list_baskets")return await sb.rpc("get_whatsapp_simple_baskets_v1");if(name==="wa_get_basket_contents")return await sb.rpc("get_whatsapp_basket_contents_v1",{p_basket_query:clean(a.basket_query,120)});if(name==="wa_find_baskets_by_items")return await sb.rpc("find_whatsapp_baskets_by_items_v1",{p_items_query:clean(a.items_query,240)});if(name==="wa_get_policy")return await sb.rpc("get_whatsapp_basic_policy_reply_v1",{p_message:clean(a.message,1000)});if(name==="wa_get_recommendations")return await sb.rpc("get_cart_aware_recommendations",{p_conversation_id:job.conversation_id,p_limit:Math.floor(clampNumber(a.limit,1,6,4)),p_kind:clean(a.kind,30)});if(name==="wa_get_basket_state")return await sb.rpc("get_agent_core_basket_state_compact_v1",{p_conversation_id:job.conversation_id});if(name==="wa_get_checkout_contact")return await sb.rpc("get_agent_core_checkout_contact_compact_v1",{p_conversation_id:job.conversation_id});if(name==="wa_get_basket_customer_status")return await sb.rpc("get_agent_core_basket_customer_status_compact_v1",{p_conversation_id:job.conversation_id});return {data:null,error:{message:"not_read_executor"}} as any};
