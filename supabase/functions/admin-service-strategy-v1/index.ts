...
const maskPhone=(v:unknown)=>{const d=String(v??"").replace(/\D/g,"");return d.length<4?"—":`••••••${d.slice(-4)}`};

function strategyAnalysis(metrics:any){
  const findings:any[]=[],recommendations:any[]=[];
  const automation=metrics?.runtime?.automation||{},rules=metrics?.rules||{};
  const uncovered=Number(metrics?.uncovered_intents||0),handoffs=Number(metrics?.human_handoffs||0),automated=Number(metrics?.automated_replies||0),inbound=Number(metrics?.messages_inbound||0);
  ...
  if(handoffs>0)findings.push({severity:"info",key:"handoffs",title:"Transferências para humano",detail:`Foram ${handoffs} transferência(s) operacional(is) no período, excluindo controles de canário/homologação.`});
...
Deno.serve(async(req:Request)=>{
  ...
  const action=clean(body?.action||"dashboard",80).toLowerCase(),since=new Date(Date.now()-7*24*60*60*1000).toISOString();

  if(action==="dashboard"){
    const [{data:metrics,error:metricError},{data:latest},{count:snapshotCount},{data:playbook}]=await Promise.all([
      ...
      sb.from("service_strategy_playbook").select("id,version,title,strategy_text,analysis_method,success_metrics,constraints,updated_at").eq("id",1).maybeSingle()
    ]);
    if(metricError)return json(origin,{ok:false,error:"metrics_failed",detail:metricError.message},500);
    return json(origin,{ok:true,user:{role:admin.role,display_name:admin.display_name||null},metrics,latest_snapshot:latest||null,snapshot_count:snapshotCount||0,playbook:playbook||null});
  }

  if(action==="playbook"){
    const {data,error}=await sb.from("service_strategy_playbook").select("id,version,title,strategy_text,analysis_method,success_metrics,constraints,updated_at").eq("id",1).maybeSingle();
    if(error)return json(origin,{ok:false,error:"playbook_failed",detail:error.message},500);
    return json(origin,{ok:true,playbook:data||null});
  }
  ...
  if(action==="conversations_7d"){
    ...
    const {data:raw,error}=await sb.from("conversations").select("id,customer_id,wa_contact_e164,source,channel,status,stage,mode,human_required,context_summary,opened_at,last_inbound_at,last_outbound_at,updated_at,created_at").or(`last_inbound_at.gte.${since},opened_at.gte.${since},created_at.gte.${since}`).order("updated_at",{ascending:false}).limit(500);
    if(error)return json(origin,{ok:false,error:"conversations_failed",detail:error.message},500);
    const conversations=arr(raw),customerIds=[...new Set(conversations.map((x:any)=>x.customer_id).filter(Boolean))],convIds=conversations.map((x:any)=>x.id).filter(Boolean);
    ...
    const byCustomer=new Map(arr(customers).map((x:any)=>[x.id,x])),msgMap=new Map<string,{inbound:number,outbound:number,last:any|null}>();
    for(const m of arr(messages)){const state=msgMap.get(m.conversation_id)||{inbound:0,outbound:0,last:null};if(m.direction==="inbound")state.inbound++;else if(m.direction==="outbound")state.outbound++;if(!state.last)state.last=m;msgMap.set(m.conversation_id,state)}
    let items=conversations.map((c:any)=>{const customer=byCustomer.get(c.customer_id)||null,state=msgMap.get(c.id)||{inbound:0,outbound:0,last:null};return {...c,customer_name:clean(customer?.name,180)||null,phone_masked:maskPhone(c.wa_contact_e164||customer?.primary_whatsapp_e164),last_message:clean(state.last?.body_text||state.last?.transcript||state.last?.message_type||"",220),inbound_count:state.inbound,outbound_count:state.outbound}});
    ...
    const total=items.length,from=(page-1)*limit;return json(origin,{ok:true,items:items.slice(from,from+limit),total,page,limit,window_started_at:since});
  }

  if(action==="conversation_detail"){
    const id=uuid(body?.id);if(!id)return json(origin,{ok:false,error:"invalid_conversation_id"},400);
    const {data:conversation,error}=await sb.from("conversations").select("*").eq("id",id).maybeSingle();if(error||!conversation)return json(origin,{ok:false,error:"conversation_not_found"},404);
  ...
  if(action==="generate_snapshot"){
    const {data:metrics,error}=await sb.rpc("get_service_strategy_7d_metrics_v1");if(error)return json(origin,{ok:false,error:"metrics_failed",detail:error.message},500);
    const analysis=strategyAnalysis(metrics||{}),note=clean(body?.note,4000)||"Snapshot de 7 dias gerado pelo Admin V3 para acompanhar a evolução do atendimento.";
    ...
    const {data:snapshot,error:insertError}=await sb.from("service_strategy_analysis_snapshots").insert(row).select("*").single();if(insertError)return json(origin,{ok:false,error:"snapshot_save_failed",detail:insertError.message},500);
  ...
  if(action==="snapshots"){
    const limit=clamp(body?.limit,1,100,30),{data,error}=await sb.from("service_strategy_analysis_snapshots").select("id,window_started_at,window_ended_at,window_days,metrics,findings,recommendations,analysis_note,source,created_at").order("created_at",{ascending:false}).limit(limit);if(error)return json(origin,{ok:false,error:"snapshots_failed",detail:error.message},500);return json(origin,{ok:true,items:data||[]});
  }

  if(action==="change_log"){
    const limit=clamp(body?.limit,1,200,100),{data,error}=await sb.from("service_strategy_change_log").select("id,entity_type,entity_id,operation,before_data,after_data,source,reason,expected_result,observed_result,review_status,analysis_snapshot_id,changed_by,created_at,reviewed_at").order("created_at",{ascending:false}).limit(limit);if(error)return json(origin,{ok:false,error:"change_log_failed",detail:error.message},500);return json(origin,{ok:true,items:data||[]});
  }

  if(action==="annotate_change"){
    const id=uuid(body?.id);if(!id)return json(origin,{ok:false,error:"invalid_change_id"},400);const status=clean(body?.review_status,30)||"pending";if(!["pending","kept","adjusted","reverted","baseline"].includes(status))return json(origin,{ok:false,error:"invalid_review_status"},400);
    const patch:any={reason:clean(body?.reason,2000)||null,expected_result:clean(body?.expected_result,2000)||null,observed_result:clean(body?.observed_result,2000)||null,review_status:status};if(status!=="pending")patch.reviewed_at=new Date().toISOString();
    const {data,error}=await sb.from("service_strategy_change_log").update(patch).eq("id",id).select("*").single();if(error)return json(origin,{ok:false,error:"change_annotation_failed",detail:error.message},500);return json(origin,{ok:true,item:data});
  }
