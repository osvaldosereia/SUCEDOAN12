...
async function listProducts(url: URL) {
  ...
  if(ids.length){
    const {data:offerRows,error:oErr}=await db.from("offers")
      .select("id,product_id,title,sale_price_cents,starts_at,ends_at,active,metadata,created_at")
      .eq("organization_id",ORG_ID)
      .in("product_id",ids);
    if(oErr)throw oErr;
    for(const offer of offerRows??[])offerMap.set(offer.product_id,offer);
  }
  return {
...
async function saveProduct(payload: any) {
  ...
  const hasExpiration=Object.prototype.hasOwnProperty.call(payload??{},"expiration_date");
  ...
  const row:any = {
    ...
    auto_expiry_offer_enabled: hasAutoExpiry ? payload?.auto_expiry_offer_enabled===true : Boolean(previous?.auto_expiry_offer_enabled),
...
async function saveExpiration(payload:any) {
  ...
  const expirationRaw=text(payload?.expiration_date,10);
  if(expirationRaw&&!validIsoDate(expirationRaw))return {error:"invalid_expiration_date",status:400};
  ...
  const {error}=await db.from("products")
    .update({
      ...
      auto_expiry_offer_enabled:autoEnabled,
...
async function saveManualOffer(payload:any) {
  ...
  if(!active){
    const {error}=await db.from("offers")
      .update({
        active:false,
        metadata:{source:"manual",deactivated_reason:"admin",deactivated_at:new Date().toISOString()}
      })
      .eq("organization_id",ORG_ID)
  ...
  let discount=Number(payload?.discount_percent);
  if((!Number.isFinite(sale)||sale<=0)&&Number.isFinite(discount)&&discount>0&&discount<100){
    sale=Math.max(1,Math.round(regular*(100-discount)/100));
  }
  if(!Number.isFinite(sale)||sale<=0||sale>=regular)return {error:"invalid_offer_price",status:400};
  if(!Number.isFinite(discount)||discount<=0||discount>=100){
    discount=Math.round((1-sale/regular)*1000)/10;
  }
  const duration=text(payload?.duration,20)||"stock_zero";
  if(!["stock_zero","5","10","15"].includes(duration))return {error:"invalid_offer_duration",status:400};
  const endsAt=duration==="stock_zero"?null:new Date(Date.now()+Number(duration)*86400000).toISOString();
  const now=new Date().toISOString();
  const row={
    organization_id:ORG_ID,
    product_id:productId,
    title:`Oferta · ${discount}%`,
    sale_price_cents:sale,
    starts_at:now,
    ends_at:endsAt,
    active:Number(product.stock_quantity||0)>0,
    metadata:{
      source:"manual",
  ...
  const {data,error}=await db.from("offers")
    .upsert(row,{onConflict:"organization_id,product_id"})
    .select("id,product_id,title,sale_price_cents,starts_at,ends_at,active,metadata,created_at")
    .single();
  if(error)throw error;

  const {error:uErr}=await db.from("products")
    .update({auto_expiry_offer_enabled:false,updated_at:now})
    .eq("organization_id",ORG_ID)
    .eq("id",productId);
  if(uErr)throw uErr;
...
async function buildBlingOrderSnapshot(orderId:string){
  ...
  if(productIds.length){
    const p=await db.from("products")
      .select("id,sku,gtin,name,sale_price_cents")
  ...
  for(const item of itemRows){
    ...
    if(item.item_kind==="basket"){
      ...
      for(const c of rows){
        const p=pMap.get(c.product_id);
        const storedPrice=c.metadata?.unit_price_cents;
        const catalogPrice=p?.sale_price_cents;
        const rawPrice=(storedPrice!==undefined&&storedPrice!==null&&Number.isFinite(Number(storedPrice)))
          ? Number(storedPrice)
          : Number(catalogPrice);
        ...
        lines.push({
          ...
          price_source:(storedPrice!==undefined&&storedPrice!==null&&Number.isFinite(Number(storedPrice)))?"snapshot":"catalog_at_separation"
        });
      }
    }
  }
  ...
  const history=await db.from("vitrine_history_sync_outbox")
    .select("remote_customer_id")
    .eq("order_id",orderId)
    .maybeSingle();
  if(history.error)throw history.error;
  const canonicalCustomerId=delivery.source_customer_id||history.data?.remote_customer_id||null;
  ...
  return {
    ...
    customer:{
      source_customer_id:canonicalCustomerId,
...
async function updateOrder(payload:any) {
  ...
  if(historySync.ok && (patch.status==="delivered"||patch.status==="cancelled")){
    try{
      const fiscal=await blingHubControl("fiscal_status",{source_order_id:data.id});
      fiscalSynced=!(fiscal as any).error;
    }catch(e){
      console.error("vitrine_fiscal_status_sync_failed",String((e as Error)?.message||e));
    }
  }
  return {
    order_id:data.id,
  ...
  try {
    ...
    if (req.method==="POST") {
      ...
      if (action==="expiration_save") {
        const result=await saveExpiration(payload);
        if ((result as any).error) return json(req,{ok:false,error:(result as any).error},(result as any).status);
        return json(req,{ok:true,...result});
      }
      if (action==="offer_save") {
        const result=await saveManualOffer(payload);
        if ((result as any).error) return json(req,{ok:false,error:(result as any).error},(result as any).status);
        return json(req,{ok:true,...result});
      }
      ...
      if (action==="bling_reconcile_customers_readonly") {
        const result=await blingHubControl("reconcile_customers_readonly",{limit:payload?.limit??650});
        if ((result as any).error) return json(req,{ok:false,error:(result as any).error,detail:(result as any).detail},(result as any).status);
        return json(req,{ok:true,...((result as any).data||{})});
      }
      if (action==="bling_preview_customer_sync") {
        const id=uuid(payload?.customer_id);
      ...
      if (action==="order_fiscal_dispatch_canary_execute") {
        const id=uuid(payload?.id);
        if(!id)return json(req,{ok:false,error:"invalid_order"},400);
        if(text(payload?.confirmation,40)!=="EMITIR_NFE")return json(req,{ok:false,error:"fiscal_human_confirmation_required"},409);
        const result=await blingHubControl("fiscal_dispatch_canary_human_execute",{
          source_order_id:id,
      ...
      if (action==="order_fiscal_confirm_payment") {
        const id=uuid(payload?.id);
        if(!id)return json(req,{ok:false,error:"invalid_order"},400);
        const result=await blingHubControl("fiscal_confirm_payment",{
          source_order_id:id,
          payment_method:text(payload?.payment_method,80)
        });
        if ((result as any).error) return json(req,{ok:false,error:(result as any).error,detail:(result as any).detail},(result as any).status);
        return json(req,{ok:true,...((result as any).data||{})});
      }
