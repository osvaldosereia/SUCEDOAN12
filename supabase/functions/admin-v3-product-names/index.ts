...
Deno.serve(async(req:Request)=>{
  ...
  if(action==="health")return respond({ok:true,version:1,mode:"admin_v3_product_names"});
  ...
  if(action==="product_name_normalization_review"){
    ...
    if(decision==="keep"){
      const note=`[Admin V3 ${now}] Nome atual mantido manualmente.`;
    ...
    if(product.name!==finalName){
      ...
      const nextMetadata={...metadata,product_name_normalization:{...existing,version:job.normalization_version,original_name:job.original_name,normalized_name:finalName,confidence:job.confidence,admin_reviewed_at:now,admin_review_action:decision,source:"admin_v3"}};
    ...
    const note=`[Admin V3 ${now}] ${decision==="approve"?"Sugestão aprovada":"Nome editado e aplicado"} manualmente.`;
