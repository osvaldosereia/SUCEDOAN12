...
function stagesOf(v:any){const input=Array.isArray(v)?v:[];return input.slice(0,20).map((s:any)=>({question:clean(s?.question,600),variations:vars(s?.variations),answer:clean(s?.answer,6000),response_mode:clean(s?.response_mode,40)||"text",tool_config:toolCfg(s?.tool_config)})).filter((s:any)=>s.question)}
Deno.serve(async(req:Request)=>{
  ...
  if(action==="set_status"){if(!isOwner)return json({ok:false,error:"owner_required"},403);const id=uuid(body?.id),status=clean(body?.status,30);if(!id||!["draft","published","archived"].includes(status))return json({ok:false,error:"invalid_status"},400);if(status==="published"){const {data:current,error:readError}=await sb.from("service_simple_rules").select("stages,response_mode").eq("id",id).maybeSingle();if(readError||!current)return json({ok:false,error:"entity_not_found"},404);const stages=stagesOf(current.stages);const blocked=stages.filter((s:any)=>!LIVE_MODES.has(s.response_mode)).map((s:any)=>s.response_mode);if(blocked.length)return json({ok:false,error:"resource_not_ready",detail:`Configure à vontade, mas ainda não publique: ${[...new Set(blocked)].join(', ')} ainda depende de transporte/template/Flow habilitado.`},409)}const {data:item,error}=await sb.from("service_simple_rules").update({status,updated_by:userData.user.id,updated_at:new Date().toISOString()}).eq("id",id).select("*").single();if(error)return json({ok:false,error:"status_update_failed",detail:error.message},400);return json({ok:true,item})}
