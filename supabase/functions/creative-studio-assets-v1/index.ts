...
Deno.serve(async(req:Request)=>{
  ...
  if(action==='search')return json({ok:true,assets:await localSearch(sb,need,keywords,Math.min(30,Math.max(1,Number(body.limit)||12))),source:'library'});
  if(action==='signed_url'){
    const path=clean(body.path,500);if(!path)return json({ok:false,error:'path_required'},400);
    const {data,error}=await sb.storage.from(BUCKET).createSignedUrl(path,600);if(error)return json({ok:false,error:'signed_url_failed'},400);return json({ok:true,signed_url:data.signedUrl});
  }
  if(action!=='resolve')return json({ok:false,error:'unknown_action'},400);
  if(!need)return json({ok:false,error:'need_required'},400);
  const local=await localSearch(sb,need,keywords,12);if(local.length)return json({ok:true,status:'resolved_local',asset:local[0],external_acquisition:false});
  if(body.allow_external!==true)return json({ok:true,status:'missing',reason:'external_disabled'});
  ...
  const bytes=await download.arrayBuffer();if(bytes.byteLength>25*1024*1024)return json({ok:true,status:'blocked',reason:'file_too_large'});
  ...
  return json({ok:true,status:'acquired',asset,external_acquisition:true,external_acquisitions:used+1,provider_credit:'Powered by Poly Haven'});
});
