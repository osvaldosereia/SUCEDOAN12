...
const json=(v:any,s=200,h:Record<string,string>={})=>new Response(JSON.stringify(v),{status:s,headers:{...cors,"Content-Type":"application/json; charset=utf-8",...h}});
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==="health")return json({ok:true,service:"storefront-v2",mode:"canonical-vitrine",version:8},200,{"Cache-Control":"no-store"});
    if(req.method==="GET"&&action==="home")return json(await home(),200,{"Cache-Control":"public, max-age=120, stale-while-revalidate=600"});
    if(req.method==="GET"&&action==="offers")return json(await offerList(),200,{"Cache-Control":"no-store"});
    if(req.method==="GET"&&action==="subcategories")return json(await subcats(u),200,{"Cache-Control":"public, max-age=120, stale-while-revalidate=600"});
    if(req.method==="GET"&&action==="products")return json(await productList(u),200,{"Cache-Control":"no-store"});
    if(req.method==="GET"&&action==="product"){const id=uid(u.searchParams.get("product_id"));if(!id)return json({ok:false,error:"invalid_product"},400);const p=await oneProduct(id);return p?json({ok:true,product:p},200,{"Cache-Control":"no-store"}):json({ok:false,error:"product_not_found"},404)}
    if(req.method==="GET"&&action==="basket"){const id=uid(u.searchParams.get("basket_id"));if(!id)return json({ok:false,error:"invalid_basket"},400);const b=await basket(id);return b?json({ok:true,...b},200,{"Cache-Control":"no-store"}):json({ok:false,error:"basket_not_found"},404)}
    if(req.method==="GET"&&action==="resolve_identity_code"){const r=await resolveCode(req,u.searchParams.get("code"));return r.error?json({ok:false,...r},r.status||400,{"Cache-Control":"no-store"}):json({ok:true,...r},200,{"Cache-Control":"no-store"})}
    if(req.method==="GET"&&action==="resolve_identity_token"){const r=await resolveToken(u.searchParams.get("token"));return r.error?json({ok:false,...r},r.status||400,{"Cache-Control":"no-store"}):json({ok:true,...r},200,{"Cache-Control":"no-store"})}

    const body=req.method==="POST"?await req.json().catch(()=>({})):{};
    if(req.method==="POST"&&action==="basket_quote"){const r=await quote(body);return r.error?json({ok:false,...r},r.status||400,{"Cache-Control":"no-store"}):json(r,200,{"Cache-Control":"no-store"})}
    if(req.method==="POST"&&action==="submit_order"){const r=await submit(req,body);return r.error?json({ok:false,...r},r.status||400,{"Cache-Control":"no-store"}):json({ok:true,...r},200,{"Cache-Control":"no-store"})}
    if(req.method==="POST"&&action==="issue_identity_link"){if(!(await bridgeOk(req)))return json({ok:false,error:"unauthorized"},401);const r=await issueLink(body);return r.error?json({ok:false,...r},r.status||400):json({ok:true,...r})}
    if(req.method==="POST"&&action==="reconcile_customer"){if(!(await bridgeOk(req)))return json({ok:false,error:"unauthorized"},401);const r=await reconcile(body);return r.error?json({ok:false,...r},r.status||400):json({ok:true,...r})}

    // Compatibility with the previous canonical storefront-v2 contract.
    if(req.method==="POST"&&action==="list_baskets"){const h=await home();return json({ok:true,baskets:h.baskets.map((b:any)=>({id:b.id,name:b.name,image_url:b.image_url,base_price:Number(b.display_price_cents||0)/100,ready:true}))})}
    if(req.method==="POST"&&action==="get_basket"){const b=await basket(uid(body?.id));return b?json({ok:true,basket:{id:b.basket.id,name:b.basket.name,image_url:b.basket.image_url,base_price:Number(b.basket.display_price_cents||0)/100,ready:true},items:b.items.map((i:any)=>({...i,product:{id:i.product_id,name:i.name,image_url:i.image_url,stock:i.stock_quantity,packaging:i.packaging,is_active:i.stock_quantity>0}}))}):json({ok:false,error:"basket_not_found"},404)}
    if(req.method==="POST"&&action==="list_sections"){return json({ok:true,sections:CATEGORIES.map(c=>({name:c.key,label:c.label}))})}
    if(req.method==="POST"&&action==="list_products"){const fake=new URL(req.url);if(body?.section)fake.searchParams.set("category",String(body.section));if(body?.q)fake.searchParams.set("q",String(body.q));fake.searchParams.set("limit",String(body?.limit||20));fake.searchParams.set("offset",String(Math.max(0,(Number(body?.page||1)-1)*Number(body?.limit||20))));const p=await productList(fake);return json({ok:true,products:p.products.map((x:any)=>({id:x.id,name:x.name,price:Number(x.price_cents||0)/100,stock:x.stock_quantity,image_url:x.image_url,brand:x.brand,category:x.category,packaging:x.packaging,is_offer:x.regular_price_cents!=null})),page:Number(body?.page||1),limit:Number(body?.limit||20),total:null,has_more:p.next_offset!=null})}
    if(req.method==="POST"&&action==="lookup_customer_by_phone")return json(await lookupCustomer(body?.phone));
    if(req.method==="POST"&&action==="create_order"){let ph=phone(body?.phone);if(!ph)return json({ok:false,error:"invalid_phone"},400);const r=await db.rpc("create_storefront_order_v2",{p_phone:ph,p_items:Array.isArray(body?.items)?body.items:[],p_basket:body?.basket&&typeof body.basket==="object"?body.basket:null});if(r.error)return json({ok:false,error:txt(r.error.message,120).split("\n")[0]||"order_failed"},400);return json({ok:true,order:{id:r.data?.order_id,number:r.data?.order_number,total:Number(r.data?.total||0),message:r.data?.message||"",customer_found:Boolean(r.data?.customer_id),status:"storefront_received"}})}

    return json({ok:false,error:"unknown_action"},400);
  }catch(e){console.error("storefront-v2 canonical-vitrine",e);return json({ok:false,error:"service_unavailable",detail:txt((e as any)?.message,180)},500,{"Cache-Control":"no-store"})}
});