...
Deno.serve(async(req:Request)=>{
  ...
  const length=Number(req.headers.get("content-length")||0);if(length>131072)return json({ok:false,error:"payload_too_large"},413);
  ...
  if(action==="health")return json({ok:true,version:2});
  ...
  if(action==="list_baskets"){
    ...
    if(error)return json({ok:false,error:"baskets_failed"},500);
  ...
  if(action==="get_basket"){
    ...
    if(error||!b)return json({ok:false,error:"basket_not_found"},404);
    const items=Array.isArray((b as any).basket_template_items)?(b as any).basket_template_items:[];
    const ready=items.length>0&&items.every((i:any)=>i.product?.is_active===true&&i.product?.physically_verified===true&&Number(i.product?.stock||0)>=Number(i.quantity||0));
    if(!ready)return json({ok:false,error:"basket_not_ready"},409);
    return json({ok:true,basket:{id:(b as any).id,name:(b as any).name,description:(b as any).description||null,image_url:(b as any).image_url||null,base_price:Number((b as any).base_price||0),is_featured:(b as any).is_featured===true},items:items.sort((a:any,c:any)=>Number(a.sort_order||0)-Number(c.sort_order||0)).map((i:any)=>({product_id:i.product_id,quantity:Number(i.quantity||0),removable:i.removable===true,quantity_editable:i.quantity_editable===true,min_quantity:Number(i.min_quantity||0),max_quantity:i.max_quantity==null?null:Number(i.max_quantity),product:publicProduct(i.product)}))});
  }
  ...
  if(action==="list_sections"){
    ...
    if(error)return json({ok:false,error:"sections_failed"},500);
  ...
  if(action==="list_products"){
    ...
    const {data,error,count}=await query;if(error)return json({ok:false,error:"products_failed"},500);
    return json({ok:true,products:(data||[]).map(publicProduct),page,limit,total:count||0,has_more:page*limit<(count||0)});
  }

  if(action==="lookup_customer_by_phone"){
    let phone:string;try{phone=normalizePhone(body?.phone)}catch{return json({ok:false,error:"invalid_phone"},400)}
    ...
    return json({ok:true,found:Boolean(customer),first_name:firstName});
  }

  if(action==="create_order"){
    let phone:string;try{phone=normalizePhone(body?.phone)}catch{return json({ok:false,error:"invalid_phone"},400)}
    const items=Array.isArray(body?.items)?body.items:[];if(items.length>120)return json({ok:false,error:"too_many_items"},400);
    ...
    if(error){const code=clean(error.message,120).split("\n")[0];return json({ok:false,error:code||"order_failed"},400)}
    const result:any=data||{};
    return json({ok:true,order:{id:result.order_id,number:result.order_number,total:Number(result.total||0),message:result.message||"",customer_found:Boolean(result.customer_id),status:"storefront_received"}});
  }
