drop function if exists public.marketing_product_shortlist_v1(integer,integer);

create function public.marketing_product_shortlist_v1(
...
base as (
  select p.*,
    coalesce(nullif(p.customer_subcategory,''),nullif(p.category,''),nullif(p.storefront_category,''),nullif(p.customer_category,'')) as campaign_category,
    coalesce(nullif(p.customer_subsubcategory,''),nullif(p.subcategory,'')) as campaign_subcategory,
...
eligible as (
  select * from base
  where best_image is not null
    and (coalesce(cost,0)<=0 or effective_price>cost)
),
scored as (
  select e.*,coalesce(r.recent_campaign_count,0) as recent_campaign_count,
  ...
  from eligible e left join recent r on r.product_id=e.id
),
ranked as (
  select s.*,
    round((
      case when s.is_offer and coalesce(s.offer_price,0)>0 and s.offer_price<s.price then 16 else 0 end
      + least(12::numeric,coalesce(s.offer_discount_percent,0)/35*12)
      + case when s.margin_percent is not null then least(24::numeric,s.margin_percent/45*24) else -3 end
      + least(16::numeric,coalesce(s.stock,0)/30*16)
      + case when s.storefront_featured then 8 else 0 end
      + case when nullif(trim(coalesce(s.brand,'')),'') is not null then 3 else 0 end
      + case when nullif(trim(coalesce(s.campaign_category,'')),'') is not null then 3 else 0 end
      + case when nullif(trim(coalesce(s.description_short,'')),'') is not null then 2 else 0 end
      - case when coalesce(s.min_stock,0)>0 and s.stock<=s.min_stock then 25
             when s.stock<3 then 12 when s.stock<6 then 5 else 0 end
      - least(60::numeric,coalesce(s.recent_campaign_count,0)::numeric*30)
    )::numeric,2) as score
  from scored s
)
select r.id,r.name,r.brand,r.campaign_category,r.campaign_subcategory,
       r.price,r.effective_price,r.cost,r.stock,r.is_offer,r.offer_price,r.best_image,
       ...
       array_remove(array[
         ...
         case when nullif(trim(coalesce(r.brand,'')),'') is not null and nullif(trim(coalesce(r.campaign_category,'')),'') is not null then 'good_product_data' end
       ],null)::text[]
...
is 'Deterministic server-only shortlist for Marketing Brain. Excludes known loss-making items, penalizes very low stock and recent campaign repetition.';
