...
begin
  ...
  if v_basket_id is not null then
    for v_bi in select * from public.basket_template_items where basket_id=v_basket_id order by sort_order,created_at loop
      ...
      v_delta:=0;
      if v_qty<v_bi.quantity then if v_bi.remove_unit_delta is null and coalesce(v_product.price,0)<=0 then raise exception 'remove_pricing_not_configured'; end if; v_delta:=abs(v_qty-v_bi.quantity)*coalesce(v_bi.remove_unit_delta,-v_product.price);
      elsif v_qty>v_bi.quantity then if v_bi.add_unit_delta is null and coalesce(v_product.price,0)<=0 then raise exception 'add_pricing_not_configured'; end if; v_delta:=(v_qty-v_bi.quantity)*coalesce(v_bi.add_unit_delta,v_product.price); end if;
      v_total:=v_total+v_delta;
      if v_qty>0 then v_line:=round(v_qty*coalesce(v_product.price,0),2);v_fiscal:=v_fiscal+v_line; insert into public.order_items(order_id,product_id,sku_snapshot,name_snapshot,quantity,unit_price,line_total,metadata) values(v_order_id,v_product.id,v_product.sku,v_product.name,v_qty,coalesce(v_product.price,0),v_line,jsonb_build_object('source','basket','basket_id',v_basket_id,'base_quantity',v_bi.quantity,'commercial_delta',v_delta,'removable',v_bi.removable,'quantity_editable',v_bi.quantity_editable)); end if;
    end loop;
  end if;
  for v_item in select value from jsonb_array_elements(v_extra_items) loop
    ...
    insert into public.order_items(order_id,product_id,sku_snapshot,name_snapshot,quantity,unit_price,line_total,metadata) values(v_order_id,v_product.id,v_product.sku,v_product.name,v_qty,v_product.price,v_line,jsonb_build_object('source','extra'));
  end loop;
