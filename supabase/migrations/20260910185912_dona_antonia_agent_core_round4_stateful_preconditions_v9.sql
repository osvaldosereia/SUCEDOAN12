...
begin
  ...
  if not found then
    return jsonb_build_object('ready',false,'action_key',p_action_key,'missing',jsonb_build_array('unknown_action'),'unsupported','[]'::jsonb,'checks','{}'::jsonb);
  end if;
  ...
  limit 1;
  v_basket_session_active:=base_s.id is not null
    and base_s.status='open'
  ...
  v_cart_valid:=cart_row.id is not null
    and cart_row.status='draft'
  ...
  begin contact:=public.get_agent_core_checkout_contact_compact_v1(p_conversation_id); exception when others then contact:='{}'::jsonb; end;
  ...
  if coalesce(p_input->>'product_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then v_product_id:=(p_input->>'product_id')::uuid; end if;
  if coalesce(p_input->>'source_product_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then v_source_product_id:=(p_input->>'source_product_id')::uuid; end if;
  if coalesce(p_input->>'original_product_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then v_original_product_id:=(p_input->>'original_product_id')::uuid; end if;
  if coalesce(p_input->>'replacement_product_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then v_replacement_product_id:=(p_input->>'replacement_product_id')::uuid; end if;
  if coalesce(p_input->>'basket_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then v_basket_id:=(p_input->>'basket_id')::uuid; end if;
  ...
  for pre in select jsonb_array_elements_text(coalesce(a.preconditions,'[]'::jsonb)) loop
    ...
    case pre
      ...
      when 'address_flow_pending' then
        ok:=v_address_flow_pending;
      ...
      when 'basket_session_active' then
        ok:=v_basket_session_active;
      ...
      when 'basket_valid' then
        ok:=v_basket_session_active and v_cart_valid
          and exists(select 1 from public.basket_templates b where b.id=nullif(base_s.metadata->>'basket_id','')::uuid and b.is_active=true and b.is_whatsapp_active=true);
      ...
      when 'cart_valid' then
        ok:=v_cart_valid;
      when 'checkout_customer_data_requested' then
        ok:=coalesce(st.awaiting,'') in ('basket_customer_base_data','order_customer_base_data');
      when 'current_message_present' then
        ok:=v_current_message;
      when 'customer_valid' then
        ok:=v_customer_valid;
      when 'explicit_customer_confirmation' then
        ok:=v_current_message and (
          iid in ('da_basket_confirm_order','da_confirm_order')
          or norm in ('confirmar pedido','confirmo o pedido','pode confirmar o pedido')
        );
      when 'locator_requested' then
        ok:=coalesce(st.awaiting,'') in ('basket_locator_confirmation','order_locator_confirmation');
      when 'payment_method_valid' then
        ok:=v_payment is not null;
      when 'product_validated' then
        ok:=v_product_id is not null
          and coalesce((p_input->>'quantity')::numeric,0)>0
      ...
      when 'replacement_validated' then
        ok:=v_original_product_id is not null and v_replacement_product_id is not null
          and coalesce((p_input->>'customer_confirmed')::boolean,false)=true
...
begin
  ...
  if coalesce((base->>'allowed')::boolean,false) is not true then
    return base||jsonb_build_object('precondition_version',2,'state_preconditions',null);
  end if;

  pre:=public.evaluate_whatsapp_agent_action_preconditions_v1(p_conversation_id,p_message_id,p_action_key,coalesce(p_input,'{}'::jsonb));
...
create or replace function public.get_agent_core_round4_stateful_precondition_readiness_v1()
returns jsonb
language sql
stable
security definer
...
with supported(name) as (values
 ...
 ('product_validated'),('replacement_validated'),('source_product_in_basket'),('valid_address')
  ...
  select distinct jsonb_array_elements_text(coalesce(a.preconditions,'[]'::jsonb)) name
  from public.ai_action_registry a
  where a.enabled=true and 'whatsapp'=any(a.allowed_channels)
), missing_support as (
  select d.name from declared d left join supported s using(name) where s.name is null
), stateful as (
  select count(*)::int total,
    count(*) filter(where risk_class='reversible_write')::int reversible,
    count(*) filter(where risk_class='commitment')::int commitments,
    count(*) filter(where execution_mode='observe')::int observe_count
  from public.ai_action_registry
  where enabled=true and 'whatsapp'=any(allowed_channels) and risk_class in ('reversible_write','commitment')
)
select jsonb_build_object(
  'version',1,
  'declared_precondition_count',(select count(*) from declared),
  'supported_precondition_count',(select count(*) from supported),
  'unsupported_preconditions',coalesce((select jsonb_agg(name order by name) from missing_support),'[]'::jsonb),
  'all_declared_preconditions_supported',not exists(select 1 from missing_support),
  'stateful_action_count',(select total from stateful),
  'reversible_write_count',(select reversible from stateful),
  'commitment_count',(select commitments from stateful),
  'all_stateful_still_observe',(select total=observe_count from stateful),
  'preview_function','preview_whatsapp_agent_action_v2',
  'pii_returned',false,
  'ready',not exists(select 1 from missing_support) and (select total=observe_count from stateful)
); $$;

revoke all on function public.get_agent_core_round4_stateful_precondition_readiness_v1() from public,anon,authenticated;
grant execute on function public.get_agent_core_round4_stateful_precondition_readiness_v1() to service_role;
