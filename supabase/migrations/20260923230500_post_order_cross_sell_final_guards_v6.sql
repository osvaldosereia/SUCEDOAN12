-- Final guards for post-order cross-sell.
-- Prevents duplicate additions and blocks/rejects sessions after separation starts.
...
declare
  ...
  v_separation_started boolean := false;
...
begin
  ...
  select (
    coalesce((v_order.payment_method_snapshot->>'stock_consumed')::boolean,false)
    or v_order.status in ('ready','out_for_delivery')
    or exists(
      select 1
      from public.order_stock_reservations r
      where r.organization_id=v_order.organization_id
        and r.order_id=p_order_id
        and r.status='consumed'
    )
  ) into v_separation_started;
  ...
  elsif v_separation_started then v_reason:='separation_started';
