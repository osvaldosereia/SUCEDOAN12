-- Reconcile Make/Meta delivery receipts for all production outbound lock owners.
-- Flow jobs use pgnet-make-flow-v1, while the recovery cron calls v4.
-- Keep delivery fail-closed: missing/uncertain provider evidence is never retried blindly.
...
begin
  ...
  loop
    if v_job.last_dispatch_request_id is null then
      if v_job.locked_at is not null and v_job.locked_at<now()-interval '2 minutes' then
        update public.outbound_jobs
           set status='error',
               last_error='delivery_uncertain_review_required',
               not_before=now()+interval '100 years',
               locked_at=null,
               locked_by=null,
               dispatch_response_checked_at=now(),
               updated_at=now()
         where id=v_job.id;
        v_review:=v_review+1;
      else
    ...
    if v_mode not in ('text','audio','image','interactive') then
      v_mode:='text';
    end if;
    v_expected_interactive_type:=case
      when v_mode='interactive' then coalesce(v_job.payload#>>'{interactive,type}','')
      else ''
    end;
