...
create or replace function public.get_agent_core_round4_consolidated_readiness_v17()
returns jsonb
language plpgsql
stable
security definer
...
as $$
declare
  base jsonb:=public.get_agent_core_round4_consolidated_readiness_v16();
  evidence jsonb:=public.get_agent_core_round4_stateful_evidence_report_v1(168);
begin
  return base || jsonb_build_object(
    'version',17,
    'stateful_evidence',evidence,
    'stateful_evidence_ready',coalesce((evidence->>'evidence_ready')::boolean,false),
    'stateful_evidence_schema_version',coalesce((evidence->>'version')::integer,0),
    ...
    'reason',case
      ...
      when not coalesce((base->>'passive_collection_ready')::boolean,false) then base->>'reason'
      when not coalesce((evidence->>'evidence_ready')::boolean,false) then 'collect_real_homologation_stateful_evidence'
      else base->>'reason' end
  );
end
$$;

revoke all on function public.get_agent_core_round4_consolidated_readiness_v17() from public,anon,authenticated;
grant execute on function public.get_agent_core_round4_consolidated_readiness_v17() to service_role;
