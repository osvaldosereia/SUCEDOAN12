create or replace function public.get_whatsapp_flow_v31_homologation_preflight_v1(p_session_id uuid default null)
returns jsonb
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  select count(*) into v_allowlist_count from public.whatsapp_test_allowlist w
  ...
  from (values
    ...
    (20,'owner_allowlist_present',v_allowlist_count>0,'active_entries='||v_allowlist_count::text),
  ...
  return jsonb_build_object('ok',coalesce(v_all_ok,false),'checked_at',now(),'candidate_slug',d.slug,'provider_id',d.provider_id,'handler_version',d.config->>'handler_version','flow_json_version',d.config->>'flow_json_version','checks',coalesce(v_checks,'[]'::jsonb));
end;
$function$;

revoke all on function public.get_whatsapp_flow_v31_homologation_preflight_v1(uuid) from public,anon,authenticated;
grant execute on function public.get_whatsapp_flow_v31_homologation_preflight_v1(uuid) to service_role;
