...
-- Marketing Render Metrics V13 — read-only operational observability.
-- No provider calls, publication, rendering or paid AI side effects.
...
comment on function public.marketing_render_metrics_read_model_v1(timestamptz,timestamptz) is 'Read-only Marketing renderer metrics. Deterministic counts/latency only; no provider or publication side effect.';

insert into public.marketing_events(entity_type,entity_id,event_type,data,external_side_effect)
values('runtime','1','render_metrics_v13_ready',jsonb_build_object('read_only',true,'external_side_effect',false),false);
