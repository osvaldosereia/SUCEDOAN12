...
-- Correção Etapa 9 — evitar classificar importações Bling sem basket_id como compras avulsas.
  ...
  select
    ...
    count(*) filter(
      where public.is_customer_purchase_valid_v1(o.status,o.cancelled_at,o.returned_at)
        ...
        and o.source is distinct from 'bling_import'
    )::int as attributable_nonbasket_orders
...
base as (
  select
    ...
    coalesce(bs.attributable_nonbasket_orders,0)::int as attributable_nonbasket_orders,
    rv.p80_lifetime,
    case
...
flags as (
  select
    ...
    (b.attributable_nonbasket_orders>0 and b.basket_orders=0) as is_product_only_buyer,
    (
...
select
  ...
  jsonb_strip_nulls(jsonb_build_object(
    ...
    'produtos_avulsos',case when f.is_product_only_buyer then 'Possui pedido(s) local(is) de produtos avulsos e nenhum pedido identificável com cesta' end,
