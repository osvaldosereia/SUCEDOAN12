...
begin
  ...
  if exists(select 1 from public.human_handoffs h where h.conversation_id=c.id and h.status in ('open','claimed')) then
    raise exception 'homologation_human_handoff_active';
  end if;
  ...
  if not exists(
    ...
    where w.phone_e164=j.recipient_e164
      and w.enabled
  ...
  if coalesce((s.context->>'homologation_test')::boolean,false) is not true
     ...
     or s.context->>'test_recipient'<>j.recipient_e164 then
  ...
  update public.outbound_jobs
     set status='processing',attempts=attempts+1,locked_at=now(),locked_by='pgnet-make-flow-owner-homologation-v1',
         dispatch_attempts=dispatch_attempts+1,last_dispatch_at=now(),last_error=null,updated_at=now()
  ...
  v_payload:=jsonb_build_object(
    ...
    'job',jsonb_build_object(
      'id',j.id::text,
      'conversation_id',j.conversation_id::text,
      'recipient_e164',j.recipient_e164,
      'attempt',j.attempts,
      'delivery_mode','interactive',
      'body_text',left(coalesce(j.payload->>'body_text',''),4096),
      'interactive',j.payload->'interactive',
      'homologation_session_id',v_session_id::text,
      'reply_message_id',j.payload->>'reply_message_id'
    )
  );
