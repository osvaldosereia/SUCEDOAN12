...
declare
  ...
  v_pending uuid;
  v_conflict uuid;
  v_phone_owned_by_customer boolean:=false;
begin
  ...
  if v_customer_id is not null then
    ...
    ) into v_phone_owned_by_customer;
  end if;
  ...
  if v_conflict is not null
     and (v_customer_id is null or not v_phone_owned_by_customer) then
  ...
  if v_customer_id is null then
    ...
    -- O lookup da própria sessão já autorizou este cliente para este telefone.
    -- Se existir um cadastro legado duplicado para o mesmo telefone normalizado,
    -- não reatribuímos a identidade nem o telefone durante o checkout.
    update public.customers
       set name=v_name,
           updated_at=now()
     where id=v_customer_id;
