-- Database-level delivery gate for post-order cross-sell.
-- Even internal calls cannot mark a delivery or mutate an order until the PapoAI contract is ready.
...
begin
  ...
  if coalesce(v_cfg.delivery_contract_ready,false) is not true then
    return jsonb_build_object('ok',false,'error','delivery_contract_not_ready');
  end if;
