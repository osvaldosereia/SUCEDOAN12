  ...
  v_items:=jsonb_build_array(
    ...
    jsonb_build_object(
      ...
      'status',case when coalesce((v_segments->>'customer_count')::int,0)>0
                          and jsonb_object_length(coalesce(v_segments->'segments','{}'::jsonb))>0
                    then 'verified' else 'implemented' end,
    ...
    jsonb_build_object(
      ...
      'status',case when coalesce((v_opportunities->>'active_total')::int,0)>0
                          and to_regprocedure('public.evaluate_customer_opportunities_v1(uuid)') is not null
                    then 'implemented' else 'blocked' end,
      'evidence',jsonb_build_object(
        ...
        'note','criacao observada; lifecycle de remocao coberto pelo engine e testes, sem fixture persistente'
      )
    ),
    jsonb_build_object(
