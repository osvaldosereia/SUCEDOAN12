...
begin
  ...
  where n.nspname='public' and p.prokind='f'
    and p.proname<>'get_agent_core_round4_worker_v2_retirement_readiness_v1'
    and lower(pg_get_functiondef(p.oid)) ~ 'url[[:space:]]*:?=[[:space:]]*''https://[^'']+/functions/v1/conversation-worker-v2''';
...
create or replace function public.get_agent_core_round4_consolidated_readiness_v12()
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function public.get_agent_core_round4_consolidated_readiness_v12() from public,anon,authenticated;
grant execute on function public.get_agent_core_round4_consolidated_readiness_v12() to service_role;
