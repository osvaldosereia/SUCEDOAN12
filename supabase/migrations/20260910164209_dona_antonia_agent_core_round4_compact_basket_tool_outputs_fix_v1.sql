...
begin
  begin
    v:=public.get_whatsapp_basket_flow_state_v1(p_conversation_id);
  exception when others then
    return jsonb_build_object('available',false,'active',false,'reason','basket_state_unavailable');
  end;
...
begin
  begin
    v:=public.get_whatsapp_checkout_contact_v1(p_conversation_id);
  exception when others then
    return jsonb_build_object('available',false,'known_customer',false,'base_complete',false,'address_known',false,'locator_known',false,'city_known',false);
  end;
  a:=coalesce(v->'address','{}'::jsonb);
  return jsonb_build_object(
    'available',true,
...
begin
  begin
    v:=public.get_whatsapp_basket_customer_status_v1(p_conversation_id);
  exception when others then
    return jsonb_build_object('available',false,'registered',false,'has_customer',false,'has_address',false);
  end;
  return jsonb_build_object(
    'available',true,
...
as $$
  select jsonb_build_object(
    ...
    'raw_basket_token_not_exposed',position('basket_session' in lower(pg_catalog.pg_get_functiondef('public.get_agent_core_basket_state_compact_v1(uuid)'::regprocedure)))=0
      and position('token' in lower(pg_catalog.pg_get_functiondef('public.get_agent_core_basket_state_compact_v1(uuid)'::regprocedure)))=0,
    ...
    'missing_conversation_fail_safe',coalesce((public.get_agent_core_basket_state_compact_v1('00000000-0000-0000-0000-000000000000'::uuid)->>'available')::boolean,false)=false
  );
$$;
