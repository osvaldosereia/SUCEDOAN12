...
set
  ...
  metadata=(coalesce(metadata,'{}'::jsonb)
    - 'origin_candidate_source'
    - 'origin_candidate_evidence_count'
    - 'origin_candidate_at')
    || jsonb_build_object(
      'origin_candidate_reverted_at',now(),
      'origin_candidate_reverted_reason','bling_only_or_mixed_evidence_not_accepted_r0_9'
    ),
  updated_at=now()
where metadata->>'origin_candidate_source'='evidence_consensus_r0_6';

update public.product_fiscal_review_items
set severity='blocker',updated_at=now()
where issue_code='origin_evidence_conflict'
