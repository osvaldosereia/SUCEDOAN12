...
alter table public.inventory_fast_scan_events_v3
  add column if not exists requested_quantity integer;

comment on column public.inventory_fast_scan_events_v3.requested_quantity is
  'Quantidade total informada no modo Ler + quantidade. NULL mantém o comportamento direto de +1 por bip.';
...
begin
  ...
  loop
    ...
    v_requested_quantity:=null;
    if v_raw ? 'quantity' and nullif(v_raw->>'quantity','') is not null then
      begin
        v_requested_quantity:=greatest(0,least(1000000,trunc((v_raw->>'quantity')::numeric)::integer));
      exception when others then
        v_requested_quantity:=null;
      end;
    end if;

    insert into public.inventory_fast_scan_events_v3(event_id,ean,product_id,user_id,device_label,source,scanned_at,status,requested_quantity)
    values(v_event_id,v_ean,v_product_id,p_user_id,nullif(left(coalesce(p_device_label,''),120),''),v_source,v_scan_time,'received',v_requested_quantity)
    ...
    if v_product_id is null then
      ...
      values(
        v_ean,'pending',1,v_scan_time,v_scan_time,p_user_id,p_user_id,'inventory_fast_balance_v3',
        jsonb_strip_nulls(jsonb_build_object('device_label',p_device_label,'requested_quantity',v_requested_quantity)),clock_timestamp()
      )
      ...
      v_results:=v_results||jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
        'event_id',v_event_id,'ean',v_ean,'kind','unknown','requested_quantity',v_requested_quantity,'scanned_at',v_scan_time
      )));
      continue;
    end if;
    ...
    if not found then
      v_quantity:=coalesce(v_requested_quantity,1);
      v_new_window:=true;
      ...
      values(v_product_id,v_ean,v_quantity,v_scan_time,v_scan_time,v_scan_time+interval '30 minutes',p_user_id,p_user_id,nullif(left(coalesce(p_device_label,''),120),''),clock_timestamp());
      ...
      continue;
    elsif v_requested_quantity is not null then
      v_quantity:=v_requested_quantity;
      if v_scan_time > v_state.last_scanned_at + interval '30 minutes' then
        v_new_window:=true;
        update public.inventory_fast_balance_state_v2 set
          ean=v_ean,quantity=v_quantity,window_started_at=v_scan_time,last_scanned_at=v_scan_time,expires_at=v_scan_time+interval '30 minutes',
    ...
    update public.products set
      ...
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object(
        'last_inventory_mode',case when v_requested_quantity is null then 'fast_balance_direct_v3' else 'fast_balance_quantity_v1' end,
        'fast_balance_quantity',v_quantity,
        'fast_balance_last_scan_at',v_scan_time
      )
    ...
    values(
      v_product_id,v_ean,p_user_id,nullif(left(coalesce(p_device_label,''),120),''),'scan',v_new_window,v_previous_stock,v_quantity,v_scan_time,
      jsonb_strip_nulls(jsonb_build_object('client_event_id',v_event_id,'source','capture_queue_v3','requested_quantity',v_requested_quantity,'mode',case when v_requested_quantity is null then 'direct' else 'quantity' end))
    );
    ...
    v_results:=v_results||jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
      'event_id',v_event_id,'ean',v_ean,'kind','counted','product_id',v_product_id,'quantity',v_quantity,
      'requested_quantity',v_requested_quantity,'mode',case when v_requested_quantity is null then 'direct' else 'quantity' end,
      'new_window',v_new_window,'scanned_at',v_scan_time
    )));
  end loop;
