...
create or replace function public.room_confirm_web_order_v1(
  p_public_token text,
...
begin
  ...
  if not exists(select 1 from public.order_items oi where oi.order_id=(v_result->>'order_id')::uuid) then
...
revoke all on function public.room_confirm_web_order_v1(text,jsonb) from public,anon,authenticated;
grant execute on function public.room_confirm_web_order_v1(text,jsonb) to service_role;
