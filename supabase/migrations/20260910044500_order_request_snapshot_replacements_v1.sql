-- Finalized WhatsApp order snapshot must show the actual item selected after a basket swap.
...
begin
  ...
  if bs.cart_id is not null then select * into cart_row from public.carts where id=bs.cart_id; end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'substituted',coalesce(i.metadata->'substitution','null'::jsonb)<>'null'::jsonb,
    'replacement_product_id',nullif(i.metadata->'substitution'->>'replacement_product_id',''),
    'replacement_name',nullif(i.metadata->'substitution'->>'replacement_name',''),
    'actual_name',coalesce(nullif(i.metadata->'substitution'->>'replacement_name',''),p.name)
