...
update public.bling_history_status_policy
set status_name=coalesce(status_name,'Em aberto'),
    canonical_status='ignored',
    approved=true,
    notes='Situação Em aberto não entra no histórico de compras concluídas; permanece apenas no staging.',
    updated_at=now()
where bling_status_id=6;
