(() => {
  ...
  const BUILD = '20260826-ceneca10-personalizadas-v2-whatsapp-gate';
  ...
  function modelImages(model, product) {
    return uniqueUrls([
      model.mockup_1, model.mockup_2, model.mockup_3, model.imagem,
  ...
  async function fetchProduct(key) {
    const response = await fetch(`${FIREBASE_URL}/${PRODUCTS_NODE}/${encodeURIComponent(key)}.json`, { cache: 'no-store' });
  ...
  async function fetchModels() {
    ...
    return Promise.all(base.map(async model => {
      const product = await fetchProduct(model.product_key).catch(() => null);
      return { ...model, product, images: modelImages(model, product), art: modelArt(model, product) };
    }));
  }
  ...
  function renderModels() {
    ...
    if (!state.models.length) {
      track.innerHTML = '<div class="empty-models">Nenhum modelo está disponível ainda. Marque modelos no Criador de Canecas do Produção.</div>';
      return;
    }
    track.innerHTML = state.models.map((model, index) => `
      <button type="button" class="model-card ${state.selectedModel?.product_key === model.product_key ? 'is-selected' : ''}" data-model="${escapeHtml(model.product_key)}">
        <div class="model-image"><img src="${escapeHtml(model.images[0] || model.imagem || '../site/img/logoantonia5.png')}" alt="Modelo ${index + 1}"></div>
        <div class="model-foot"><strong>${escapeHtml(model.nome)}</strong><small>Escolher este modelo</small></div>
      </button>`).join('');
  }
  ...
  function resultWhatsappUrl(id, publicUrl, customerName) {
    const message = [
  ...
  async function saveCreationRecords(id, data, urls) {
    ...
    const publicData = {
      id,
      nome_publico: `Caneca personalizada de ${data.customerName.split(/\s+/)[0] || 'cliente'}`,
      modelo_nome: data.model.nome,
      modelo_key: data.model.product_key,
      frase: data.phrase,
  ...
  function openWhatsappGate() {
    ...
    const url = contactWhatsappUrl(state.requestId, data);
  ...
  async function generate() {
    ...
    const productName = safeName(baseData.customerName, baseData.phrase);
    ...
    try {
      ...
      const recipe = await modelRecipe(data.model);

      progress(22, 'Criando a arte personalizada…', 'Aplicando sua foto e frase ao estilo escolhido.');
      const artResult = await callMake({
        action: 'generate_mug_art', request_id: id, image_base64: composite,
        instruction: recipe, prompt_art: artPrompt(recipe, data.phrase), quality: state.quality,
      });
      ...
      const [left, right, center] = await Promise.all([
        sideReference(master, 1), sideReference(master, 2), sideReference(master, 3),
      ]);
      const template = firebaseTemplate(id, data);

      progress(58, 'Criando as prévias…', 'Gerando as três vistas da caneca.');
      const finalResult = await callMake({
        ...
        instruction: recipe, product_name: productName,
        prompt_mockup_1: mockupPrompt(1), prompt_mockup_2: mockupPrompt(2), prompt_mockup_3: mockupPrompt(3),
        quality: 'high', firebase_url: FIREBASE_URL, products_node: PRODUCTS_NODE,
      ...
      progress(82, 'Salvando sua criação…', 'Primeiro estamos criando o link com as quatro imagens.');
      if (finalResult.product_saved !== true) await directSaveProduct(id, template, urls);
      await saveCreationRecords(id, data, urls);

      progress(100, 'Tudo pronto!', 'O link foi criado. Agora você pode enviá-lo para nossa equipe no WhatsApp.');
      $('donePreview').src = urls.m1;
      $('viewResultButton').href = publicUrl;
      $('sendResultWhatsappButton').href = resultWhatsappUrl(id, publicUrl, data.customerName);
      $('doneMessage').textContent = 'Seu link com as 4 imagens foi criado. Confira e envie a criação para a Dona Antônia pelo WhatsApp.';
  ...
  function bind() {
    $('modelsTrack').addEventListener('click', event => {
      ...
      const next = state.models.find(model => model.product_key === button.dataset.model) || null;
      if (next?.product_key !== state.selectedModel?.product_key) invalidateWhatsappGate();
    ...
    $('customerPhotoInput').addEventListener('change', () => {
      ...
      if (state.photoFile) invalidateWhatsappGate();
    ...
    $('phraseInput').addEventListener('input', () => {
      ...
      if (state.whatsappOpened) invalidateWhatsappGate();
    });
    ...
    $('whatsappSentConfirm').addEventListener('change', () => {
      if (!state.whatsappOpened) {
        $('whatsappSentConfirm').checked = false;
        return;
      }
      state.whatsappConfirmed = $('whatsappSentConfirm').checked;
      updateGateUi();
    });
  ...
  async function init() {
    ...
    if (!state.webhook && new URLSearchParams(location.search).get('admin') === '1') {
      toast('Configure o webhook desta página de teste na engrenagem.');
    }
