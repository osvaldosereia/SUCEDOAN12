(() => {
  'use strict';
  const BUILD = '20260826-ceneca10-make-client-guard-v5-accepted-poll';
  ...
  async function waitFinalProduct(payload, fetcher){
    ...
    while (Date.now() < deadline) {
      ...
      } catch (error) { console.warn('[Caneca10] aguardando Firebase após Accepted:', error); }
      await sleep(POLL_MS);
    }
    return null;
  }
  ...
  window.fetch = async function ceneca10MakeFetch(input, init){
    ...
    if (response.ok && /^accepted\.?$/i.test(text(raw)) && payload?.action === 'finalize_mug_product') {
      console.info('[Caneca10] Make respondeu Accepted; aguardando arte + 3 mockups no Firebase.', payload.request_id);
