...
const wa=buildMarketingChannelRequestBundle({...base,channel:'whatsapp_status',publication_path:'manual_confirm',official_api_family:'whatsapp_business_platform',request_preview:null});
assert.equal(wa.operation,'manual_confirmation_only');
assert.equal(wa.dispatch_allowed,false);

console.log('PASS: channel request bundle stays credential-free, endpoint-free and non-dispatching.');
