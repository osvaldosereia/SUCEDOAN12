...
export function readyEmailHtml({ creationCode = '', artUrl = '', customerName = '' } = {}) {
  const code = encodeURIComponent(text(creationCode));
  const safeName = text(customerName);
  const greeting = safeName ? `<p>Olá, ${escapeHtml(safeName)}.</p>` : '';
  const image = artUrl
...
export function buildReadyEmail({ from, to, creationCode, artUrl, customerName } = {}) {
  ...
  return {
    ...
    html: readyEmailHtml({ creationCode, artUrl, customerName }),
  };
}
...
export function catalogMaps(categories = [], brands = []) {
  ...
  return {
    categorias,
    categorias_lista,
    total_categorias: Object.keys(categorias_lista).length,
    marcas,
    marcas_lista,
    total_marcas: Object.keys(marcas_lista).length,
  };
}
