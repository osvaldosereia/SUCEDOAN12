...
assert.match(conversation, /checkoutApi\('finalize_order',payload\)/, 'confirmação final deve usar uma única chamada consolidada');
assert.match(conversation, /customer_id:checkoutFlow\.profile\?\.customer_id/, 'payload final deve preservar o vínculo do cliente');
assert.match(conversation, /delivery_address:form\.address/, 'payload final deve enviar o endereço conferido');
assert.match(conversation, /payment_method:checkoutFlow\.payment/, 'payload final deve enviar a forma de pagamento');
assert.match(conversation, /app\.markOrderCompleted\(data\)/, 'estado local só deve encerrar depois do pedido persistido');
