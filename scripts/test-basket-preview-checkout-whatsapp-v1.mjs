...
assert.match(preview,/Escolher esta cesta/);assert.match(preview,/Voltar às cestas/);
...
assert.match(choose,/renderSelectedBasket\(\)/);assert.match(choose,/state\.modules\.products\?\.renderEntry\?\.\(\{auto:true\}\)/);
assert.match(choose,/scrollTo\(finish,\{block:'start'\}\)/);
...
assert.match(app,/function openCheckout\(button\)/);assert.match(app,/checkoutButton\.onclick=\(\)=>openCheckout\(checkoutButton\)/);assert.match(app,/cartButton\.onclick=\(\)=>openCheckout\(cartButton\)/);
...
assert.match(app,/window\.DA_ADMIN_TEST_TRANSPORT/);assert.doesNotMatch(app,/window\.fetch\s*=/);assert.doesNotMatch(checkout,/window\.fetch\s*=/);
