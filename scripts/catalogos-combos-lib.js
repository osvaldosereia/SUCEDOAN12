...
function resolveComboItem(item, productIndex) {
  const codes = [item?.codigo, ...(Array.isArray(item?.substitutos) ? item.substitutos : [])]
    .map(cleanText)
    .filter(Boolean);
  const candidates = codes.map(code => ({ code, product: findProduct(productIndex, code) })).filter(row => row.product);
  const selected = candidates.find(row => isActiveProduct(row.product) && productPrice(row.product) > 0 && productStock(row.product) > 0)
...
function dateAtBrazil(value, endOfDay = false) {
  ...
  if (match) return new Date(`${match[1]}-${match[2]}-${match[3]}T${endOfDay ? '23:59:59' : '00:00:00'}-04:00`);
  const brazil = raw.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (brazil) return new Date(`${brazil[3]}-${String(brazil[2]).padStart(2, '0')}-${String(brazil[1]).padStart(2, '0')}T${endOfDay ? '23:59:59' : '00:00:00'}-04:00`);
  ...
  try {
    ...
    if (url.hostname === 'raw.githubusercontent.com') {
      ...
      if (parts[0] === 'osvaldosereia' && parts[1] === 'SUCEDOAN12' && branchIndex >= 0) {
        const repositoryPath = parts.slice(branchIndex + 1).join('/');
        return `${SITE_URL}/${repositoryPath}`;
      }
    }
    if (url.hostname === 'github.com') {
      ...
      if (parts[0] === 'osvaldosereia' && parts[1] === 'SUCEDOAN12' && blobIndex >= 0) {
        const repositoryPath = parts.slice(blobIndex + 2).join('/');
        return `${SITE_URL}/${repositoryPath}`;
      }
    }
...
function comboRecord(combo, type, productIndex, now = new Date()) {
