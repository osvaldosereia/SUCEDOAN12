...
const controls=fs.readFileSync('admin/products-inline-controls-v4.js','utf8');
const css=fs.readFileSync('admin/products-inline-controls-v4.css','utf8');
const html=fs.readFileSync('admin/index.html','utf8');
const api=fs.readFileSync('supabase/functions/admin-simple-v2/index.ts','utf8');

for(const heading of ['Produto','Estoque','Preço normal','Ativo','Verificado','Validade','Oferta','Preço da oferta','Ações']){
  assert.match(controls,new RegExp(`<th>${heading.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}<\\/th>`,'i'),`Admin precisa exibir a coluna ${heading}`);
}
...
assert.ok(rowStart>=0,'Admin precisa ter renderização própria da linha');
...
assert.match(rowSource,/data-inline-verified/i,'verificação física precisa ser controlada diretamente na linha');
...
assert.match(controls,/data-product-inline/i,'controles inline precisam continuar editáveis na própria lista');
assert.match(controls,/data-inline-save-all/i,'lista precisa ter um único botão para salvar alterações pendentes');
assert.match(controls,/pendingInlineChanges/i,'alterações inline precisam ser acumuladas antes do envio');
assert.match(controls,/product_in_use/i,'UI precisa tratar exclusão protegida');
assert.match(controls,/admin-simple-v2/i,'Admin deve usar o backend de produto já endurecido');

const patchStart=controls.indexOf('function readInlinePatch');
const patchEnd=controls.indexOf('function queueInlineChange',patchStart+1);
const patchSource=controls.slice(patchStart,patchEnd>patchStart?patchEnd:patchStart+5000);
assert.ok(patchStart>=0,'Admin precisa montar patch das alterações inline');
assert.match(patchSource,/patch\.is_offer=offer/i,'ativação de oferta deve entrar no mesmo patch pendente');
assert.match(patchSource,/patch\.offer_price=offerPrice/i,'preço da oferta deve entrar no mesmo patch pendente');
assert.match(patchSource,/if\(offer&&offerPrice===null\)/i,'oferta ativa deve exigir preço antes do salvamento');
assert.match(controls,/productApi\('update_product',\{id:item\.id,patch:item\.patch\}\)/i,'salvamento em lote precisa enviar o patch consolidado por produto');
assert.match(controls,/item\.row\.outerHTML=productRow\(result\.value\.data\.product\)/i,'linha deve refletir o estado devolvido pelo banco após salvar');
...
assert.match(html,/app\.js\?v=20260918-customer-workspace-5/i,'Admin deve usar cache-bust atual do app base');
