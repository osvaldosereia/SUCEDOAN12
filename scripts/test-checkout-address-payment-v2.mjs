...
assert.match(customerEdge,/WA_NUMBER='5565998150975'/,'identity proof source must use the official WhatsApp');
...
assert.match(router,/rewriteVerificationResponse/,'router must normalize the verification response');
assert.match(router,/C\.whatsappFallback/,'verification link must be rewritten to the configured official WhatsApp');
