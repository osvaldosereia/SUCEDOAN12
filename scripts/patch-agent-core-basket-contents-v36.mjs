...
for(const path of files){
  ...
  const productFrom='product_search:["wa_search_products","wa_get_product","wa_get_cart","wa_add_product","wa_set_quantity","wa_create_search_showcase",...core]';
  const productTo='product_search:["wa_search_products","wa_get_product","wa_get_cart","wa_add_product","wa_set_quantity","wa_create_search_showcase","wa_create_basket_replacement","wa_open_basket_storefront","wa_add_more_products","wa_start_order_checkout",...core]';
  if(!src.includes(productTo)) src=once(src,productFrom,productTo,`${path} productSearchRecovery`);

  const generalFrom='general:["wa_get_policy","wa_search_products","wa_list_baskets","wa_get_cart","wa_handoff_human"]';
  const generalTo='general:["wa_get_policy","wa_search_products","wa_list_baskets","wa_get_cart","wa_get_basket_state","wa_get_checkout_contact","wa_request_address_flow","wa_handoff_human"]';
  if(!src.includes(generalTo)) src=once(src,generalFrom,generalTo,`${path} generalSemanticRecovery`);
...
console.log('Patched Agent Core and Eval V36 basket contents + behavior recovery support');
