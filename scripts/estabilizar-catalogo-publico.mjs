...
function previousFile(pathname) {
  try {
    return execFileSync('git', ['show', `HEAD:${pathname}`], { encoding: 'utf8' });
  } catch {
    return '';
  }
}
...
function compactProduct(key, product = {}) {
  const result = {
    ...
    estoque: integer(product.estoque),
    situacao: text(product.situacao || 'A'),
    ...
    imagens: Array.isArray(product.imagens) ? product.imagens.map(publicImageValue).filter(Boolean).slice(0, 3) : [],
...
const compactPublic = Object.fromEntries(
  Object.entries(generatedPublic || {}).map(([key, product]) => [key, compactProduct(key, product)])
);
...
const version = {
  version: publicChanged
    ? `catalog-${contentHash(compactPublic)}`
    : text(previousVersion.version || `catalog-${contentHash(compactPublic)}`),
  adminVersion: adminChanged
    ? `admin-${createHash('sha256').update(generatedAdminText).digest('hex').slice(0, 16)}`
    : text(previousVersion.adminVersion || ''),
  updatedAt: (publicChanged || adminChanged)
    ? new Date().toISOString()
    : text(previousVersion.updatedAt || new Date().toISOString()),
  products: PRODUCTS_HOME_PATH,
  adminProducts: PRODUCTS_ADMIN_PATH,
  changed: [
    ...(publicChanged ? ['products'] : []),
    ...(adminChanged ? ['admin-products'] : [])
  ],
  ...
  instructions: 'Catálogo público compacto; versão pública alterada somente quando os dados públicos mudam.'
};
...
console.log(publicChanged
  ? `Catálogo público alterado: versão ${version.version}.`
  : `Catálogo público sem alteração: versão ${version.version} preservada.`);
