...
import {storedAssetDownloads,storageObjectUrl,renderAssetExtension} from '../marketing/creative-studio/render-worker-assets.js';
...
async function main(){const job=await claim();if(!job){console.log('creative-studio: no queued jobs');return}const dir=join(tmpdir(),`creative-studio-${job.id}`);mkdirSync(dir,{recursive:true});try{await event(job.id,'render_started',{worker:'github_actions_ffmpeg_v2'});const productInput=await downloadProduct(job,dir);const assetInputs=await downloadStoredAssets(job,dir);await event(job.id,'assets_prepared',{stored_assets:assetInputs.length,product_image_used:Boolean(productInput)});const output=join(dir,'reel.mp4');const args=buildFfmpegArgs(job,{productInput,assetInputs,output});const render=spawnSync('ffmpeg',args,{stdio:'inherit'});if(render.status!==0||!existsSync(output))throw new Error(`ffmpeg_failed:${render.status}`);const path=await uploadMp4(job.id,output);await patchJob(job.id,{status:'completed',output_bucket:'creative-studio-renders',output_path:path,output_metadata:{renderer:'github_actions_ffmpeg_v2',product_image_used:Boolean(productInput),stored_assets_used:assetInputs.length},actual_cost_brl:0,completed_at:new Date().toISOString()});await event(job.id,'completed',{output_path:path,product_image_used:Boolean(productInput),stored_assets_used:assetInputs.length});console.log(JSON.stringify({ok:true,job_id:job.id,output_path:path,stored_assets_used:assetInputs.length}))}catch(error){const detail=String(error?.message||error).slice(0,1500);await patchJob(job.id,{status:'failed',error_code:'render_failed',error_detail:detail}).catch(()=>{});await event(job.id,'failed',{detail}).catch(()=>{});throw error}}
