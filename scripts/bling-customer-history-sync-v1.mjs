...
import { mapBlingContact, mapBlingSale, digits } from './bling-customer-history-sync-core.mjs';
...
const MAX_YEARS=Math.max(1,Math.min(15,Number.parseInt(process.env.HISTORY_MAX_YEARS||'10',10)||10));
...
const summary={mode:APPLY?'apply':'dry-run',local_customers_before:0,linked_bling_before:0,contacts_synced:0,contacts_created:0,contacts_updated:0,addresses_saved:0,emails_saved:0,phones_saved:0,contact_conflicts:0,contact_errors:0,years_scanned:0,sales_listed:0,history_orders_saved:0,history_items_saved:0,history_errors:0};
...
async function bling(path,{allow404=false,label=path}={}){for(let attempt=1;attempt<=5;attempt++){await pace();let r;try{r=await fetch(`${API_BASE}${path}`,{headers:{Authorization:`Bearer ${accessToken}`,Accept:'application/json','enable-jwt':'1'}})}catch(e){if(attempt===5)throw new Error(`${label}: falha de rede (${e.message})`);await sleep(attempt*900);continue}if(r.ok||(allow404&&r.status===404))return r;const body=(await r.text()).slice(0,1000);if(!(r.status===429||r.status>=500)||attempt===5)throw new Error(`${label}: HTTP ${r.status} ${body}`);const ra=Number(r.headers.get('retry-after'));await sleep(Number.isFinite(ra)&&ra>0?ra*1000:attempt*attempt*900)}throw new Error(`${label}: falha inesperada`)}
async function oauth(){const body=new URLSearchParams({grant_type:'refresh_token',refresh_token:required('BLING_REFRESH_TOKEN')});const basic=Buffer.from(`${required('BLING_CLIENT_ID')}:${required('BLING_CLIENT_SECRET')}`).toString('base64');await pace();const r=await fetch(`${API_BASE}/oauth/token`,{method:'POST',headers:{Authorization:`Basic ${basic}`,'Content-Type':'application/x-www-form-urlencoded',Accept:'application/json','enable-jwt':'1'},body});if(!r.ok)throw new Error(`OAuth Bling HTTP ${r.status}: ${(await r.text()).slice(0,700)}`);const d=await r.json();if(!text(d.access_token))throw new Error('OAuth não retornou access_token.');accessToken=text(d.access_token);const f=text(process.env.BLING_REFRESH_TOKEN_FILE);if(f&&text(d.refresh_token))writeFileSync(f,text(d.refresh_token),{encoding:'utf8',mode:0o600})}
...
function chooseCustomer(c){
  const byBling=c.bling_contact_id?state.byBling.get(c.bling_contact_id):null;
  if(byBling)return{customer:byBling,match:'bling'};
  const byDoc=c.cpf_cnpj?state.byDoc.get(c.cpf_cnpj):null;
  const byPhone=c.phone_e164?state.byPhone.get(c.phone_e164):null;
  if(byDoc&&byPhone&&byDoc.id!==byPhone.id)return{conflict:true};
  return{customer:byDoc||byPhone||null,match:byDoc?'document':byPhone?'phone':'new'};
}
...
async function syncContact(raw){
  ...
  let customer=chosen.customer;
  const phoneOwner=c.phone_e164?state.phoneOwners.get(c.phone_e164):null;
  const safePhone=!c.phone_e164||!phoneOwner||phoneOwner.customer_id===customer?.id?c.phone_e164:null;
...
async function saveSale(raw){const s=mapBlingSale(raw);if(!s.bling_order_id)return;let customer=null;if(s.bling_contact_id)customer=await ensureContact(s.bling_contact_id);if(!APPLY){summary.history_orders_saved++;summary.history_items_saved+=s.items.length;return}
...
async function syncHistory(){const currentYear=new Date().getUTCFullYear();let foundAny=false,emptyOlder=0;for(let offset=0;offset<MAX_YEARS;offset++){const year=currentYear-offset;const start=`${year}-01-01`;const end=offset===0?new Date().toISOString().slice(0,10):`${year}-12-31`;let rows;try{rows=await listSales(start,end)}catch(e){summary.history_errors++;console.error(`Ano ${year}: ${e.message}`);continue}summary.years_scanned++;summary.sales_listed+=rows.length;if(rows.length){foundAny=true;emptyOlder=0}else if(foundAny){emptyOlder++}
