...
const required=[
  'admin-v3/index.html',
  'admin-v3/nomes-produtos-v3.html',
...
const page=fs.readFileSync('admin-v3/nomes-produtos-v3.html','utf8');
...
assert.match(index,/nomes-produtos-v3\.html/,'menu do Admin V3 deve apontar para o runtime novo de Nomes dos produtos');
...
assert.match(page,/window\.__DA_NAMES_V3_READY__\s*=\s*true/,'runtime deve expor marcador simples de diagnóstico');
assert.match(page,/response\.text\(\)/,'runtime deve aceitar resposta antes de converter JSON para diagnóstico mais claro');
assert.match(page,/admin-v3-api/,'editor de produto deve reutilizar a API atual do Admin V3');
assert.match(page,/admin-v3-product-names/,'gestão deve usar a API isolada de normalização');
