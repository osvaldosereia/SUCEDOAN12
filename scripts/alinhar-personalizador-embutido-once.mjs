...
const contentManagerReplacement = 'const personalizerUrl = `${base}?model=${encodeURIComponent(key)}&embed=1&return=${encodeURIComponent(returnUrl)}`;\n  return {';
replaceRegex(
  'admin-canecas/product-content-manager-v1.js',
  /const personalizerUrl = "javascript:[\s\S]*?";\n  return \{/,
  contentManagerReplacement,
  'content manager remove javascript legado'
);

const contentSyncReplacement = 'const url = `${base}?model=${encodeURIComponent(key)}&embed=1&return=${encodeURIComponent(ret)}`;\n  return {';
replaceRegex(
  'scripts/sincronizar-conteudo-canecafacil.mjs',
  /const url = "javascript:[\s\S]*?";\n  return \{/,
  contentSyncReplacement,
