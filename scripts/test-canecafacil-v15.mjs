...
// Admin: configuração individual continua sendo a fonte de verdade.
for(const id of ['nome','foto','logo','endereco','telefone','site']){
  assert.match(personalization,new RegExp(`\\['${id}'`),`campo ${id} deve existir no cadastro`);
}
...
// Modelo normal: horizontal interna + 2 mockups + 2 recortes na vitrine.
...
// Cliente: arte, preview e recuperação; nenhum produto temporário.
...
assert.match(cartBridge,/PERSONALIZADA <span>· \$\{entry\.code\}<\/span>/,'carrinho deve identificar item personalizado com CF-ID');
assert.match(cartBridge,/location\.replace\(CART_URL\)/,'após adicionar deve abrir o carrinho nativo');

// Minhas Artes: salva cedo e recupera depois.
...
// Homologação antiga permanece segura e isolada.
...
// Pedido/produção: CF-ID liga pedido real à arte aprovada.
...
assert.match(orderWorker,/personalizada:\s*true/,'item do Admin deve ser marcado como personalizado');
...
console.log('OK CanecaFácil V15: Admin completo, cliente sem produto temporário, Minhas Artes, produto original no carrinho e CF-ID ligado à produção.');
