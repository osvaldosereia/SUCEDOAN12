...
const adminPage=readFileSync('admin-v3/atendimento.html','utf8');
const adminController=readFileSync('admin-v3/service-strategy.js','utf8');
...
assert.match(adminPage,/data-strategy-tab=["']flow["']/,'Admin V3 deve expor o fluxo do Chat Comprar');
assert.match(adminController,/admin-chat-menu-v1/,'Admin V3 deve salvar o menu pelo backend próprio');
assert.match(adminController,/response_text/,'respostas simples do menu devem continuar editáveis');
assert.match(adminController,/menuAddText/,'deve ser possível adicionar uma resposta textual ao fluxo');
assert.doesNotMatch(adminController,/chatMenuEdgeFunction/,'não deve depender da configuração legada do Admin antigo');
