...
export async function processJob(job,{workspace=process.cwd(),outputDir=path.join(process.cwd(),'tmp','marketing-renders'),persistence=null}={}){
  ...
  if(persistence){
    ...
    persisted=await persistFn({...options,assetId:job.asset_id,version:job.asset_version,jobId:job.id,role:'output',filePath:output.path,mimeType:output.mime_type,width:output.width,height:output.height,durationMs:output.duration_ms,metadata:{render_kind:job.render_kind,ai_used:false}});
  }
  return {
    ok:true,
    job_id:job.id||null,
...
async function cli(){
  ...
  if(idx<0||!args[idx+1]) fail('usage: node scripts/marketing-render-worker-v1.mjs --job-file job.json [--workspace dir] [--out-dir dir] [--persist]');
  ...
  const persistence=persist?{supabaseUrl:process.env.SUPABASE_URL,serviceRoleKey:process.env.SUPABASE_SERVICE_ROLE_KEY,actor:process.env.MARKETING_ACTOR_ID||null}:null;
  const result=await processJob(job,{workspace:workspaceIdx>=0&&args[workspaceIdx+1]?path.resolve(args[workspaceIdx+1]):path.dirname(file),outputDir:outIdx>=0&&args[outIdx+1]?path.resolve(args[outIdx+1]):path.join(path.dirname(file),'renders'),persistence});
