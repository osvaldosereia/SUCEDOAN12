...
export async function processJob(job,{workspace=process.cwd(),outputDir=path.join(process.cwd(),'tmp','marketing-renders'),persistence=null,sourceResolution=null}={}){
...
async function cli(){
  ...
  if(idx<0||!args[idx+1]) fail('usage: node scripts/marketing-render-worker-v1.mjs --job-file job.json [--workspace dir] [--out-dir dir] [--persist] [--resolve-private]');
  ...
  const persist=args.includes('--persist'),resolvePrivate=args.includes('--resolve-private');
  ...
  const result=await processJob(job,{workspace:workspaceIdx>=0&&args[workspaceIdx+1]?path.resolve(args[workspaceIdx+1]):path.dirname(file),outputDir:outIdx>=0&&args[outIdx+1]?path.resolve(args[outIdx+1]):path.join(path.dirname(file),'renders'),persistence,sourceResolution});
