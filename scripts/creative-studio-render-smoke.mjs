...
const proceduralVisuals=[{kind:'gradient',start:0,end:15,role:'background'},{kind:'confetti',start:2,end:6,role:'foreground'},{kind:'star',start:4,end:8,role:'support'}];
const args=buildFfmpegArgs(job,{proceduralVisuals,output});
const graph=args[args.indexOf('-filter_complex')+1];assert.match(graph,/drawbox/);assert.match(graph,/★/);assert.match(graph,/between\(t,2\.000,6\.000\)/);
