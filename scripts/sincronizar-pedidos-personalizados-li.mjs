...
async function li(path) {
  ...
  const response = await fetch(url, {
    headers: { Authorization: AUTH, Accept: 'application/json', 'User-Agent': 'CanecaFacil-Personalized-Orders/1.0' },
...
function itemProductId(item = {}) {
  return text(item.produto_id || item.id_produto || item.product_id || item?.produto?.id || resourceId(item.produto));
}
...
function orderComment(order = {}) { return text(order.cliente_obs || order.comentario || order.observacao || order.observacoes || order.obs || order?.cliente?.obs); }
...
async function resolveSituation(order = {}) {
  ...
  if (/\/situacao\//i.test(uri)) {
    try {
      ...
      console.warn(`Situação não resolvida para pedido ${orderId(order)}: ${error.message}`);
    }
  }
...
function customer(order = {}) {
  return {
    nome: text(order.cliente_nome || order?.cliente?.nome),
    ...
    cpf: text(order.cliente_cpf || order?.cliente?.cpf),
    cnpj: text(order.cliente_cnpj || order?.cliente?.cnpj)
  };
}
...
function shipping(order = {}) {
  ...
  return {
    ...
    valor: Number(envio.valor || order.valor_envio || order.frete_valor || 0) || 0,
    prazo: Number(envio.prazo || order.prazo_envio || 0) || 0,
    endereco: text(order.endereco_entrega || order.entrega_endereco || order?.endereco?.endereco),
    numero: text(order.numero_entrega || order.entrega_numero || order?.endereco?.numero),
    complemento: text(order.complemento_entrega || order.entrega_complemento || order?.endereco?.complemento),
    bairro: text(order.bairro_entrega || order.entrega_bairro || order?.endereco?.bairro),
    cidade: text(order.cidade_entrega || order.entrega_cidade || order?.endereco?.cidade),
    uf: text(order.estado_entrega || order.entrega_estado || order?.endereco?.estado),
    cep: text(order.cep_entrega || order.entrega_cep || order?.endereco?.cep)
  };
}

function creationArt(creation = {}) {
  return text(creation?.arte_aprovada?.url || creation.arte_aprovada_url || creation.arte_horizontal || creation.arte_personalizacao || creation.arte_impressao?.url);
}

function pendingRows(raw = {}) {
  return Object.entries(raw || {}).map(([key, value]) => ({ key, ...(value || {}) })).filter(row => text(row.criacao_id || row.id || row.key));
}
...
function matchByHint(order, rows) {
  ...
  return rows.filter(row => text(row.pedido_id_hint) === id || text(row.pedido_id) === id);
}

function matchByCodes(order, rows) {
  const codes = extractCodes(orderComment(order));
  if (!codes.length) return [];
  const set = new Set(codes);
  return rows.filter(row => set.has(pendingCode(row)));
}
...
async function matchByEmailProduct(order, rows) {
  ...
  }).sort((a, b) => new Date(b.aprovado_em || 0) - new Date(a.aprovado_em || 0));
...
async function syncMatchedOrder(order, matched, situation) {
  const id = orderId(order);
  if (!id || !matched.length) return { id, matched: 0, payment: 'pendente' };
  const payment = paymentState(situation);
  const status = commercialState(payment, situation);
  const now = nowIso();
  const items = [];
  ...
  for (const pending of matched) {
    ...
    if (!creation) {
      console.warn(`Criação ${code} não localizada para pedido ${id}.`);
      continue;
    }
    const source = sourceItem(order, pending);
    const art = creationArt(creation);
    items.push({
      id: `${id}-${code}`,
      produto_key: text(pending.produto_key || creation.modelo_key || creation.produto_key),
      codigo: text(pending.sku || itemSku(source)),
      sku: text(pending.sku || itemSku(source)),
      nome: itemName(source) || text(pending.modelo_nome || creation.modelo_nome || 'Caneca personalizada'),
      quantidade: 1,
      preco: itemPrice(source),
      criacao_id: code,
      codigo_criacao: code,
      personalizada: true,
  ...
  if (!items.length) return { id, matched: 0, payment };
  ...
  const payload = {
    ...
    cliente: { ...(existing?.cliente || {}), ...customer(order) },
    entrega: { ...(existing?.entrega || {}), ...shipping(order) },
    pagamento: {
      ...(existing?.pagamento || {}),
      status: payment,
      situacao_nome: text(situation.nome),
      situacao_codigo: text(situation.codigo),
      atualizado_em: now
    },
    pagamento_status: payment,
    itens,
    criacoes_ids: items.map(item => item.criacao_id),
    criacao_id: items.length === 1 ? items[0].criacao_id : '',
    comentario_loja_integrada: orderComment(order),
    loja_integrada: {
      ...(existing?.loja_integrada || {}),
      pedido_id: id,
      resource_uri: text(order.resource_uri),
      situacao: situation,
      sincronizado_em: now
    },
    criado_em: existing?.criado_em || orderDate(order) || now,
    atualizado_em: now
  };
  ...
  for (const item of items) {
    ...
    await Promise.all([
      fb(`canecas/personalizadas/${safeKey(code)}`, {
        method: 'PATCH',
        body: {
          status: payment === 'cancelado' ? 'pedido_cancelado' : 'encomendada',
          atendimento_status: payment === 'cancelado' ? 'novo' : 'encomendou',
          pedido_id: id,
          pedido_loja_integrada_id: id,
          encomenda: {
            status: payment === 'pago' ? 'paga' : payment === 'cancelado' ? 'cancelada' : 'pedido_criado',
            codigo_arte: code,
            pedido_id: id,
            loja_integrada_produto_id: item.loja_integrada_produto_id,
            pagamento_status: payment,
            atualizado_em: now,
      ...
      fb(`canecas/encomendas_pendentes/${safeKey(code)}`, {
        method: 'PATCH',
        body: { status: payment === 'pago' ? 'paga' : payment === 'cancelado' ? 'cancelada' : 'vinculada', pedido_id: id, pagamento_status: payment, atualizado_em: now }
      })
    ]);

    if (payment === 'pago' && item.arte_horizontal) {
      const jobId = safeKey(`PJ-${id}-${code}`);
      const existingJob = await fb(`canecas/print_jobs/${jobId}`).catch(() => null);
      if (!existingJob) {
        await fb(`canecas/print_jobs/${jobId}`, {
          method: 'PUT',
          body: {
            id: jobId,
            pedido_id: id,
            origem: 'canecafacil',
            origem_label: 'CANECAFÁCIL',
            cliente_nome: payload.cliente.nome,
            cliente_telefone: payload.cliente.telefone || payload.cliente.whatsapp,
            ...
            criacao_id: code,
            codigo_criacao: code,
            arte_aprovada: item.arte_aprovada,
            status: 'aguardando',
            pagamento_status: 'pago',
            criado_em: now,
            atualizado_em: now,
            tentativas_impressao: 0
          }
        });
      }
    }
  }
  return { id, matched: items.length, payment };
}

const pendingRaw = await fb('canecas/encomendas_pendentes').catch(() => ({}));
const pending = pendingRows(pendingRaw);
if (!pending.length) {
  console.log('SEM_ENCOMENDAS_PENDENTES');
  process.exit(0);
}
...
for (const summary of summaries) {
  ...
  if (!id) continue;
  let matched = matchByHint(summary, pending);
  let order = summary;
  if (!matched.length) {
    order = await li(`/pedido/${encodeURIComponent(id)}`).catch(() => summary);
    matched = matchByHint(order, pending);
  }
  ...
  const situation = await resolveSituation(order);
  const result = await syncMatchedOrder(order, matched, situation);
  linked += result.matched;
  if (result.payment === 'pago') queued += result.matched;
  handledOrders.add(id);
  console.log(`PEDIDO ${id} · artes=${result.matched} · pagamento=${result.payment}`);
}
...
for (const row of linkedPending.slice(0, 40)) {
  const id = text(row.pedido_id);
  const order = await li(`/pedido/${encodeURIComponent(id)}`).catch(() => null);
  if (!order) continue;
  const sameOrder = pending.filter(item => text(item.pedido_id) === id);
  const situation = await resolveSituation(order);
  const result = await syncMatchedOrder(order, sameOrder, situation);
  if (result.payment === 'pago') queued += result.matched;
  console.log(`REVALIDADO ${id} · artes=${result.matched} · pagamento=${result.payment}`);
}

console.log(`RESUMO artes_vinculadas=${linked} · filas_pagas=${queued}`);
