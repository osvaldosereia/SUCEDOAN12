...
must(editor.includes('Authorization:`Bearer ${a.access_token}`'),'editor must send bearer JWT to protected backend');
must(!/fetch\([^\n]+graph\.facebook|fetch\([^\n]+pinterest|fetch\([^\n]+googleapis|api\.openai\.com/i.test(editor),'editor must not call external provider APIs');
...
must(!/graph\.facebook|api\.pinterest|googleapis\.com|api\.openai\.com/i.test(workflow),'workflow must not call external provider APIs');
